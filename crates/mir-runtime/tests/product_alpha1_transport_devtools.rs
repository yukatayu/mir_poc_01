...
#[test]
fn product_alpha1_operational_two_shard_hard_boundary_devtools_bundle_surfaces_bounded_shard_runtime()
 {
    let report = run_product_alpha1_local_session_path(
    ...
    let bundle = export_product_alpha1_devtools(&report.session)
        .expect("operational two-shard hard-boundary devtools should export");

    assert_eq!(
        bundle.panels.shard_map_future.current_status,
        "bounded_two_shard_runtime"
    );
    assert!(
        bundle
            .panels
