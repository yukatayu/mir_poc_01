...
fn assert_actual_adoption_matches_threshold(
    actual_adoption: &CurrentL2SourceSampleOrderHandoffSurfaceActualAdoption,
    ...
    assert_eq!(
        actual_adoption.secondary_surface_lines,
...
fn order_handoff_surface_actual_adoption_reaches_late_join_default() {
    ...
    .unwrap();
    let actual_adoption = build_current_l2_source_sample_order_handoff_surface_actual_adoption(
        sample_path.to_str().unwrap(),
        prototype_host_plan(&sample_path),
    )
    .unwrap();
    ...
    assert_actual_adoption_matches_threshold(
        ...
        &[
            "guard:surface_actual_adoption_only",
...
#[test]
fn order_handoff_surface_actual_adoption_keeps_guarded_chain_not_reached() {
    ...
    assert_actual_adoption_matches_threshold(
        ...
        &["guard:order_handoff_surface_actual_adoption_not_reached"],
    );
}
