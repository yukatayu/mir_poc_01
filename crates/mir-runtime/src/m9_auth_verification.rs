...
use crate::m8_runtime_admission::{
    ...
    M8ReadOnlyProviderEffectBindingContext, M8ReadOnlyProviderEffectComponent, M8RuntimeAdmission,
    M8RuntimeInstance, materialize_m9_resolved_base,
    materialize_read_only_provider_effect_component, prepare_deferred_m9_base,
    prepare_deferred_m9_base_for_provider_coverage, read_only_provider_effect_binding_context,
};
...
use crate::sys3_projection::ReadOnlyProviderEffectStaticProjection;
...
/// boundary.  It names only source-declared identity and residual selections;
/// it carries no membership reference, credential, capability, witness,
/// route, state, or semantic result.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum M9FiniteLocalAdmissionFact {
    AnchorMembership {
        principal: String,
        locus: String,
        epoch: String,
    ...
    SourceDeclaredMembership {
        principal: String,
        locus: String,
        epoch: String,
        incarnation: String,
    },
    RelationBootstrapFreshAtAdmission {
        relation: String,
    },
    AuthDischarge {
        name: String,
    },
    OptionalVerificationDischarge {
        name: String,
    },
}

impl M9FiniteLocalAdmissionFact {
    pub(crate) fn anchor_membership(
        principal: impl Into<String>,
        locus: impl Into<String>,
        epoch: impl Into<String>,
        incarnation: impl Into<String>,
    ) -> Self {
        Self::AnchorMembership {
    ...
    pub(crate) fn source_declared_membership(
        principal: impl Into<String>,
        locus: impl Into<String>,
        epoch: impl Into<String>,
        incarnation: impl Into<String>,
    ) -> Self {
        Self::SourceDeclaredMembership {
            principal: principal.into(),
            locus: locus.into(),
            epoch: epoch.into(),
            incarnation: incarnation.into(),
    ...
    pub(crate) fn relation_bootstrap_fresh_at_admission(relation: impl Into<String>) -> Self {
        Self::RelationBootstrapFreshAtAdmission {
            relation: relation.into(),
        }
    }

    pub(crate) fn auth_discharge(name: impl Into<String>) -> Self {
        Self::AuthDischarge { name: name.into() }
    }

    pub(crate) fn optional_verification_discharge(name: impl Into<String>) -> Self {
        Self::OptionalVerificationDischarge { name: name.into() }
    }
}

#[derive(Debug, Clone, PartialEq, Eq)]
struct M9FiniteLocalMembershipIdentity {
    principal: String,
    locus: String,
    epoch: String,
    incarnation: String,
...
impl M9FiniteLocalMembershipIdentity {
    fn key(&self) -> (String, String) {
        (self.principal.clone(), self.locus.clone())
    }
...
/// The public factory marker for an opaque validated finite-local candidate.
/// It has no value constructor: `from_checked` is the only route and performs
/// all source/projection/fact validation before a candidate can exist.
pub(crate) struct M9FiniteLocalAdmissionCandidate;

/// A fail-closed finite-local admission validation result.  All fields are
/// private to M9 so no caller can forge a checked cut, normalized membership,
/// or discharge set and hand it to the authority issuer.
#[derive(Debug)]
pub(crate) struct M9ValidatedFiniteLocalAdmissionCandidate {
    checked: CheckedSurfaceV0,
    projection: crate::sys3_projection::GlobalProjectionResult,
    anchor: M9FiniteLocalMembershipIdentity,
    memberships: BTreeMap<(String, String), M9FiniteLocalMembershipIdentity>,
    relation_bootstraps: BTreeSet<String>,
    auth_discharge: String,
    verification_discharge: String,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum M9FiniteLocalAdmissionErrorKind {
    UnknownPrincipalOrLocus,
    InvalidMembershipIdentity,
    DuplicateMembership,
    ConflictingMembership,
    MissingOrUnexpectedMembership,
    PrincipalPolicyMismatch,
    RelationBootstrapMismatch,
    ResidualMismatch,
    CheckedProjectionIdentityMismatch,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct M9FiniteLocalAdmissionError {
    kind: M9FiniteLocalAdmissionErrorKind,
}

impl M9FiniteLocalAdmissionError {
    fn new(kind: M9FiniteLocalAdmissionErrorKind) -> Self {
        Self { kind }
    }

    pub(crate) const fn kind(&self) -> M9FiniteLocalAdmissionErrorKind {
        self.kind
    }

    pub(crate) const fn rejected_before_validated_candidate(&self) -> bool {
        true
    }

    pub(crate) const fn rejected_before_execution_seam(&self) -> bool {
        true
    }

    pub(crate) const fn rejected_before_authority_issuance(&self) -> bool {
        true
    }

    pub(crate) const fn partial_execution_seam(&self) -> Option<()> {
        None
    }
}
...
impl M9AdmissionRuntime {
    ...
    /// Dedicated Stage 2b base seam.  It admits no legacy M9 runtime or M8
    /// execution path: complete provider coverage must be present and the
    /// resulting base remains consumed only by the scoped composite.
    pub(crate) fn admit_read_only_provider_effect_composite_base(
        &self,
        checked: CheckedSurfaceV0,
        provider_coverage: &M9ReadOnlyProviderEffectCoverage,
        provider_binding_context: M8ReadOnlyProviderEffectBindingContext,
        m8_admission: M8RuntimeAdmission,
        envelope: M9AdmissionEnvelope,
    ) -> Result<M9AdmittedBase, M9AdmissionDiagnostics> {
        validate_outer_for_provider_coverage(&checked, &envelope, Some(provider_coverage))?;
        let outer = M9OuterAdmission {
            program_identity: checked.program_identity().clone(),
            source_artifact: envelope
                .original_source_artifact
                .expect("validated composite outer envelope retains source artifact"),
            residual_bindings: M9ResidualBindings(envelope.residual_bindings),
        };
        let embedded = prepare_deferred_m9_base_for_provider_coverage(
            &checked,
            &m8_admission,
            Some(provider_coverage),
            Some(provider_binding_context),
        )
        .map_err(|diagnostics| {
            let kind = match diagnostics.primary().kind() {
                M8AdmissionDiagnosticKind::MissingResidualEvidence => {
    ...
    pub fn admit_runtime(
        ...
        if base._embedded_m8_base.has_read_only_provider_effect_scope() {
            return Err(M9AdmissionDiagnostics::one(
...
impl M9FiniteLocalAdmissionCandidate {
    /// Validate one finite local admission against the exact retained checked
    /// cut and projection.  This is the sole constructor for the opaque
    /// candidate consumed by the M9 execution seam.
    pub(crate) fn from_checked(
        checked: &CheckedSurfaceV0,
        projection: &crate::sys3_projection::GlobalProjectionResult,
        facts: Vec<M9FiniteLocalAdmissionFact>,
    ) -> Result<M9ValidatedFiniteLocalAdmissionCandidate, M9FiniteLocalAdmissionError> {
        let known_principals = checked
            .static_environment()
            .principals()
            .iter()
            .map(|principal| principal.name())
            .collect::<BTreeSet<_>>();
        let checked_loci = checked
            .static_environment()
            .loci()
            .iter()
        ...
        if projection.checked_program_identity() != checked.program_identity()
            || projection_loci != checked_loci
        {
            return Err(M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::CheckedProjectionIdentityMismatch,
            ));
        }

        let mut anchor = None;
        let mut memberships = BTreeMap::new();
        let mut relation_bootstraps = Vec::new();
        let mut auth_discharges = Vec::new();
        let mut verification_discharges = Vec::new();
        for fact in facts {
            match fact {
                M9FiniteLocalAdmissionFact::AnchorMembership {
                    principal,
                    locus,
                    epoch,
                    incarnation,
                } => {
                    let identity = M9FiniteLocalMembershipIdentity {
                        principal,
                        locus,
                        epoch,
                        incarnation,
                    };
                    validate_finite_local_membership_identity(
                        &identity,
                        &known_principals,
                        &projection_loci,
                    )?;
                    if let Some(existing) = anchor.as_ref() {
                        return Err(M9FiniteLocalAdmissionError::new(if existing == &identity {
                            M9FiniteLocalAdmissionErrorKind::DuplicateMembership
                        } else {
                            M9FiniteLocalAdmissionErrorKind::ConflictingMembership
                        }));
                    }
                    insert_finite_local_membership(&mut memberships, identity.clone())?;
                    anchor = Some(identity);
                }
                M9FiniteLocalAdmissionFact::SourceDeclaredMembership {
                    principal,
                    locus,
                    epoch,
                    incarnation,
                    ...
                    insert_finite_local_membership(&mut memberships, identity)?;
                }
                M9FiniteLocalAdmissionFact::RelationBootstrapFreshAtAdmission { relation } => {
                    relation_bootstraps.push(relation);
                }
                M9FiniteLocalAdmissionFact::AuthDischarge { name } => auth_discharges.push(name),
                M9FiniteLocalAdmissionFact::OptionalVerificationDischarge { name } => {
                    verification_discharges.push(name);
                }
            }
        }
        let anchor = anchor.ok_or_else(|| {
            M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::MissingOrUnexpectedMembership,
            )
        })?;

        let owner_principals = checked
            .evaluations()
            .iter()
        ...
        if owner_principals.len() != 1
            || owner_principals
                .iter()
                .next()
                .is_none_or(|principal| *principal != anchor.principal)
        {
            return Err(M9FiniteLocalAdmissionError::new(
        ...
        let expected_memberships = finite_local_required_memberships(checked, &anchor);
        if memberships.keys().cloned().collect::<BTreeSet<_>>() != expected_memberships {
            return Err(M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::MissingOrUnexpectedMembership,
            ));
        }
        if memberships
            .values()
            .any(|membership| membership.principal != anchor.principal)
        {
            return Err(M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::PrincipalPolicyMismatch,
            ));
        ...
        let expected_relations = checked
            .evaluations()
            .iter()
            .filter(|evaluation| evaluation.relation_core().is_some())
            .map(|evaluation| evaluation.name().to_string())
            .collect::<BTreeSet<_>>();
        let provided_relations = relation_bootstraps.iter().cloned().collect::<BTreeSet<_>>();
        if relation_bootstraps.len() != provided_relations.len()
            || provided_relations != expected_relations
        {
            return Err(M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::RelationBootstrapMismatch,
            ));
        }

        let expected_auth = checked
        ...
        let expected_verify = checked
            ...
            .map(|residual| residual.name().to_string())
            .collect::<BTreeSet<_>>();
        let provided_auth = auth_discharges.iter().cloned().collect::<BTreeSet<_>>();
        let provided_verify = verification_discharges
            .iter()
            .cloned()
            .collect::<BTreeSet<_>>();
        if auth_discharges.len() != provided_auth.len()
            || verification_discharges.len() != provided_verify.len()
            || provided_auth != expected_auth
            || provided_verify != expected_verify
        {
            return Err(M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::ResidualMismatch,
            ));
        }
        let Some(auth_discharge) = provided_auth.into_iter().next() else {
            return Err(M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::ResidualMismatch,
            ));
        };
        let Some(verification_discharge) = provided_verify.into_iter().next() else {
            return Err(M9FiniteLocalAdmissionError::new(
                M9FiniteLocalAdmissionErrorKind::ResidualMismatch,
            ));
        };
        Ok(M9ValidatedFiniteLocalAdmissionCandidate {
            checked: checked.clone(),
            projection: projection.clone(),
            anchor,
            memberships,
            relation_bootstraps: provided_relations,
            auth_discharge,
...
fn validate_finite_local_membership_identity(
    identity: &M9FiniteLocalMembershipIdentity,
    known_principals: &BTreeSet<&str>,
    known_loci: &BTreeSet<&str>,
) -> Result<(), M9FiniteLocalAdmissionError> {
    if !known_principals.contains(identity.principal.as_str())
        || !known_loci.contains(identity.locus.as_str())
    {
        return Err(M9FiniteLocalAdmissionError::new(
    ...
    if !identity.is_valid() {
        return Err(M9FiniteLocalAdmissionError::new(
            M9FiniteLocalAdmissionErrorKind::InvalidMembershipIdentity,
        ));
    }
    Ok(())
}

fn insert_finite_local_membership(
    memberships: &mut BTreeMap<(String, String), M9FiniteLocalMembershipIdentity>,
    identity: M9FiniteLocalMembershipIdentity,
) -> Result<(), M9FiniteLocalAdmissionError> {
    ...
    memberships.insert(key, identity);
    Ok(())
}

fn finite_local_required_memberships(
    checked: &CheckedSurfaceV0,
    anchor: &M9FiniteLocalMembershipIdentity,
) -> BTreeSet<(String, String)> {
    let mut required = BTreeSet::from([(anchor.principal.clone(), anchor.locus.clone())]);
...
/// T0-produced finite-local facts for the one provider composite.  The
/// nonce is an opaque setup association, not a membership credential.
pub(crate) struct M9ReadOnlyProviderCompositeBootstrap {
    facts: Vec<M9FiniteLocalAdmissionFact>,
    program_identity: CheckedProgramIdentity,
...
impl std::fmt::Debug for M9ReadOnlyProviderCompositeBootstrap {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("M9ReadOnlyProviderCompositeBootstrap(..)")
    }
}

impl M9ReadOnlyProviderCompositeBootstrap {
    pub(crate) fn resource_runtime_nonce(&self) -> &[u8; 32] {
        &self.resource_runtime_nonce
    }
}

pub(crate) fn trusted_read_only_provider_composite_bootstrap(
    checked: &CheckedSurfaceV0,
    static_plan: &ReadOnlyProviderEffectStaticProjection,
    resource_runtime_nonce: [u8; 32],
) -> Result<M9ReadOnlyProviderCompositeBootstrap, M9AdmissionDiagnostics> {
    let coverage = static_plan.provider_coverage().clone();
    if static_plan.checked_program_identity() != checked.program_identity()
    ...
    let owner_principals = checked
        ...
        .map(|evaluation| evaluation.actor_authority_origin())
        .collect::<BTreeSet<_>>();
    let Some(principal) = owner_principals.iter().next() else {
        return Err(M9AdmissionDiagnostics::one(
    ...
    let Some(anchor_locus) = checked
        .evaluations()
        .iter()
        .find_map(|evaluation| evaluation.owner_rmw_core().map(|core| core.owner_locus()))
    else {
        return Err(M9AdmissionDiagnostics::one(
            M9AdmissionErrorKind::InvalidMembershipLineage,
        ));
    };
    let anchor = M9FiniteLocalMembershipIdentity {
        principal: (*principal).to_string(),
        locus: anchor_locus.to_string(),
        epoch: "i3-read-only-provider-bootstrap-epoch".to_string(),
        incarnation: "i3-read-only-provider-bootstrap-incarnation".to_string(),
    };
    let mut facts = vec![M9FiniteLocalAdmissionFact::anchor_membership(
        anchor.principal.clone(),
        anchor.locus.clone(),
        anchor.epoch.clone(),
        anchor.incarnation.clone(),
    )];
    for (membership_principal, membership_locus) in
        finite_local_required_memberships(checked, &anchor)
    {
        if membership_principal == anchor.principal && membership_locus == anchor.locus {
            continue;
        }
    ...
    for residual in checked.residual_obligations().entries() {
        match residual.kind() {
            ResidualObligationKind::AuthDeferred => {
                facts.push(M9FiniteLocalAdmissionFact::auth_discharge(residual.name()));
            }
            ResidualObligationKind::VerifyDeferred => {
                facts.push(M9FiniteLocalAdmissionFact::optional_verification_discharge(
                    residual.name(),
                ));
            }
            _ => {}
    ...
    // The established validator owns the authoritative exact-membership and
    // residual checks; this factory only creates T0's source-bound input.
    M9FiniteLocalAdmissionCandidate::from_checked(checked, static_plan.projection(), facts.clone())
        .map_err(|_| M9AdmissionDiagnostics::one(M9AdmissionErrorKind::InvalidMembershipLineage))?;
    Ok(M9ReadOnlyProviderCompositeBootstrap {
        facts,
        program_identity: checked.program_identity().clone(),
        coverage,
...
/// One opaque policy proof issued only for an authenticated composite.  It is
/// non-Clone and consumed by the M9 issuer, so it cannot be replayed across
/// a different checked identity, static coverage, or trusted binding nonce.
pub(crate) struct M9ReadOnlyProviderPolicyProof {
    program_identity: CheckedProgramIdentity,
    coverage: M9ReadOnlyProviderEffectCoverage,
    resource_runtime_nonce: [u8; 32],
    generation_ref: String,
...
impl std::fmt::Debug for M9ReadOnlyProviderPolicyProof {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("M9ReadOnlyProviderPolicyProof(..)")
    }
}
...
pub(crate) fn verify_read_only_provider_composite(
    checked: &CheckedSurfaceV0,
    static_plan: &ReadOnlyProviderEffectStaticProjection,
    bootstrap: M9ReadOnlyProviderCompositeBootstrap,
    composite_discharge: &M9CompositeFiniteRefinementDischarge,
    binding_nonce: [u8; 32],
) -> Result<M9VerifiedReadOnlyProviderComposite, M9AdmissionDiagnostics> {
    let coverage = static_plan.provider_coverage();
    if bootstrap.program_identity != *checked.program_identity()
        || bootstrap.coverage != *coverage
        || bootstrap.resource_runtime_nonce != binding_nonce
        || coverage.program_identity() != checked.program_identity()
        || !coverage.matches_checked(checked)
    {
        return Err(M9AdmissionDiagnostics::one(
    ...
    let expected_discharge = M9CompositeFiniteRefinementChecker::default()
        .discharge_composite_candidate(
            ...
            M9CompositeContractCandidate::from_checked_surface(checked)
                .map_err(|_| {
                    M9AdmissionDiagnostics::one(M9AdmissionErrorKind::MissingVerifyDischarge)
                })?
                .membership_auth_strengthening(),
        )
        .map_err(|_| M9AdmissionDiagnostics::one(M9AdmissionErrorKind::MissingVerifyDischarge))?;
    if &expected_discharge != composite_discharge
        || composite_discharge.provider_coverage() != coverage
    {
        return Err(M9AdmissionDiagnostics::one(
            M9AdmissionErrorKind::MissingVerifyDischarge,
        ));
    ...
    let validated = M9FiniteLocalAdmissionCandidate::from_checked(
        checked,
        static_plan.projection(),
        bootstrap.facts,
    )
    .map_err(|_| M9AdmissionDiagnostics::one(M9AdmissionErrorKind::InvalidMembershipLineage))?;
    let M9ValidatedFiniteLocalAdmissionCandidate {
    ...
    let auth_residual = validated_checked
        .residual_obligations()
        .entries()
        .iter()
        .find(|residual| {
            residual.kind() == ResidualObligationKind::AuthDeferred
                && residual.name() == auth_discharge
        })
        .ok_or_else(|| {
            M9AdmissionDiagnostics::one(M9AdmissionErrorKind::InvalidMembershipLineage)
        })?;
    let relation_lifecycle_facts = finite_local_fresh_at_admission_lifecycle_facts(
        &validated_checked,
        &anchor.principal,
        &membership_rows,
        &relation_bootstraps,
    ...
    let base = m9.admit_read_only_provider_effect_composite_base(
        validated_checked.clone(),
        coverage,
        read_only_provider_effect_binding_context(binding_nonce),
        finite_local_m8_admission_for_provider_coverage(
            &validated_checked,
            &relation_lifecycle_facts,
            Some(coverage),
        )?,
        finite_local_m9_envelope_for(&validated_checked),
    )?;
    let mut authority = base.authority_runtime();
    let mut memberships = BTreeMap::new();
    for membership_row in membership_rows.values() {
        let membership =
            finite_local_membership_for(&mut authority, membership_row, auth_residual)?;
        memberships.insert(
            (
                membership_row.principal.clone(),
                membership_row.locus.clone(),
            ),
            membership,
        );
    }
    let primary_membership = finite_local_required_membership(
        &memberships,
        coverage.contract().requester_principal(),
        coverage.contract().requester_locus(),
    )?;
    let (contract_capability, contract_witness) = finite_local_capability_and_witness(
        &mut authority,
        &primary_membership,
        "read-only-provider-composite-contract-capability",
        M9CapabilityScope::contract_update(
            checked.program_identity().module(),
            format!("{M9_AUTH_CONTRACT_PREFIX}{}", auth_residual.name()),
        ),
        auth_residual.source_ref(),
    )?;
    if !contract_authority_lineage_matches_base(
        &base,
        &authority,
        &primary_membership,
        &contract_capability,
    ...
    Ok(M9VerifiedReadOnlyProviderComposite {
        ...
        composite_discharge: composite_discharge.clone(),
        resource_runtime_nonce: binding_nonce,
    })
}
...
impl std::fmt::Debug for M9InactiveReadOnlyProviderComposite {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("M9InactiveReadOnlyProviderComposite(..)")
    }
}

impl M9VerifiedReadOnlyProviderComposite {
    pub(crate) fn resource_runtime_nonce_matches(&self, nonce: &[u8; 32]) -> bool {
        &self.resource_runtime_nonce == nonce
    }

    pub(crate) fn issue_fixed_policy_proof(
        &self,
        resource_runtime_nonce: &[u8; 32],
    ) -> Result<M9ReadOnlyProviderPolicyProof, M9AdmissionDiagnostics> {
        if !self.resource_runtime_nonce_matches(resource_runtime_nonce)
            || !self.membership_is_current()
        ...
        Ok(M9ReadOnlyProviderPolicyProof {
            ...
            generation_ref: m9_opaque_ref(&format!(
                "read-only-provider-fixed-policy:{}:{}:{}",
    ...
    fn seal_with_fixed_policies_impl(
        ...
        if policy.program_identity != *self.coverage.program_identity()
            ...
            || policy.resource_runtime_nonce != self.resource_runtime_nonce
            || policy.generation_ref.is_empty()
            || !self.membership_is_current()
        ...
        let witness = self.authority.materialize_witness(
            M9WitnessRequest::new(format!(
                "read-only-provider-effect-witness:{}",
                capability.ref_id()
            ))
        ...
        if !provider_final_evidence_matches_base(
            &self.base,
            &self.authority,
            &self.primary_membership,
            &capability,
    ...
    fn membership_is_current(&self) -> bool {
        self.authority
            .snapshot
            .memberships
            .get(self.primary_membership.ref_id())
...
impl M9InactiveReadOnlyProviderComposite {
    ...
    pub(crate) fn effect_authorization_is_current(&self) -> bool {
        ...
        let Some(capability) = self
            ...
            .snapshot
            .capabilities
            .get(self.capability.ref_id())
        else {
            return false;
        };
        let Some(witness) = self.authority.snapshot.witnesses.get(self.witness.ref_id()) else {
            return false;
        };
        membership == &self.membership
            ...
            && !self
                ...
                .snapshot
                .revoked_capabilities
                .contains(self.capability.ref_id())
            && self
                .authority
                .snapshot
                .current_memberships
...
impl M9RuntimeExecutionSeam {
    ...
    fn test_real_admitted_owner_kernel_seam<const N: usize>(
        ...
        .and_then(M9RuntimeAdmitted::into_m10_execution_seam)
        .map_err(|diagnostics| {
            format!(
...
impl M9AuthorityGeneration {
    ...
    /// An M9-owned opaque digest used only to attest that a sealed SYS-4
    /// admission covers its checked source inventory.  It is deliberately
    /// separate from a public carrier or any authority constructor.
    pub(crate) fn observer_safe_inventory_digest(&self) -> String {
    ...
    /// Read one M9-issued relation transition authority use.  The returned
    /// M8 carrier was translated from the sealed M9 inventory; this query
    /// cannot issue or alter relation authority.
    pub(crate) fn relation_authority_use(
        &self,
        relation: &str,
        transition: &str,
...
impl M9RuntimeExecutionSeam {
    /// Materialize the M9 execution seam only from a previously validated
    /// finite-local candidate.  There is intentionally no raw facts or
    /// caller-provided identity route through this authority-issuing boundary.
    pub(crate) fn admit_validated_finite_local_candidate(
        candidate: M9ValidatedFiniteLocalAdmissionCandidate,
    ) -> Result<Self, M9AdmissionDiagnostics> {
        let M9ValidatedFiniteLocalAdmissionCandidate {
            checked,
            projection,
        ...
        let auth_residuals = checked
            ...
            .filter(|residual| residual.kind() == ResidualObligationKind::AuthDeferred)
            .collect::<Vec<_>>();
        let verify_residuals = checked
            .residual_obligations()
            .entries()
            .iter()
            .filter(|residual| residual.kind() == ResidualObligationKind::VerifyDeferred)
            .collect::<Vec<_>>();
        let Some(auth_residual) = auth_residuals
            .iter()
            .copied()
            .find(|residual| residual.name() == auth_discharge)
        else {
            return Err(M9AdmissionDiagnostics::one(
                M9AdmissionErrorKind::InvalidMembershipLineage,
            ));
        };
        if auth_residuals.len() != 1
            || verify_residuals.len() != 1
            || verify_residuals[0].name() != verification_discharge
        {
            return Err(M9AdmissionDiagnostics::one(
                M9AdmissionErrorKind::MissingVerifyDischarge,
            ));
        ...
        let m9 = M9AdmissionRuntime::default();
        let base = m9.admit_source_bound_base(
            checked.clone(),
            finite_local_m8_admission_for(&checked, &relation_lifecycle_facts)?,
            finite_local_m9_envelope_for(&checked),
        )?;
        let mut authority = base.authority_runtime();
        let mut memberships = BTreeMap::new();
        for membership_row in membership_rows.values() {
        ...
        let primary_membership =
            finite_local_required_membership(&memberships, &anchor.principal, &anchor.locus)?;
        let (contract_capability, contract_witness) = finite_local_capability_and_witness(
            &mut authority,
            &primary_membership,
            "finite-local-contract-capability",
            M9CapabilityScope::contract_update(
                checked.program_identity().module(),
                format!("{M9_AUTH_CONTRACT_PREFIX}{auth_discharge}"),
            ),
            auth_residual.source_ref(),
        )?;
        ...
        if discharge.residual_name() != verification_discharge {
            return Err(M9AdmissionDiagnostics::one(
                M9AdmissionErrorKind::MissingVerifyDischarge,
            ));
        }
...
fn finite_local_membership_for(
    authority: &mut M9AuthorityRuntime,
    membership_row: &M9FiniteLocalMembershipIdentity,
    auth_residual: &mir_semantics::surface_v0_pipeline::ResidualObligation,
) -> Result<M9MembershipAuth, M9AdmissionDiagnostics> {
    let attestation = authority.issue_membership_attestation(
        &membership_row.principal,
        &membership_row.locus,
        &membership_row.epoch,
        &membership_row.incarnation,
        auth_residual.name(),
        auth_residual.source_ref().clone(),
    )?;
    authority.authenticate_membership(
        M9MembershipRequest::new(
            &membership_row.principal,
            &membership_row.locus,
            &membership_row.epoch,
        )
        .with_incarnation(&membership_row.incarnation)
        .with_auth_residual(auth_residual.name(), auth_residual.source_ref().clone())
        .with_issued_provider_attestation(attestation),
    )
}

fn finite_local_required_membership(
    memberships: &BTreeMap<(String, String), M9MembershipAuth>,
    principal: &str,
    locus: &str,
) -> Result<M9MembershipAuth, M9AdmissionDiagnostics> {
    memberships
        ...
        .cloned()
        .ok_or_else(|| M9AdmissionDiagnostics::one(M9AdmissionErrorKind::InvalidMembershipLineage))
}

/// Fixed local bootstrap evidence for one accepted relation lifetime.  It is
/// intentionally derived from a checked binding plus an explicit membership
/// identity, but it neither creates a Core fact nor grants authority.  M8
/// receives only an opaque, bounded lifecycle label rather than a caller
/// supplied lifecycle handle.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl M9FiniteFreshAtAdmissionLifecycleFact {
    fn bounded_local_bootstrap_label(&self) -> String {
        m9_opaque_ref(&format!(
    ...
    const fn core_derived(&self) -> bool {
        false
    }

    const fn grants_authority(&self) -> bool {
        false
    }
}

fn finite_local_fresh_at_admission_lifecycle_facts(
    checked: &CheckedSurfaceV0,
    admission_principal: &str,
    memberships: &BTreeMap<(String, String), M9FiniteLocalMembershipIdentity>,
    relation_bootstraps: &BTreeSet<String>,
) -> Result<BTreeMap<String, M9FiniteFreshAtAdmissionLifecycleFact>, M9AdmissionDiagnostics> {
    let mut facts = BTreeMap::new();
    for evaluation in checked.evaluations() {
        let Some(relation) = evaluation.relation_core() else {
            continue;
        };
        let Some(binding_frontier) = relation.binding_frontier().as_slice().first() else {
            return Err(M9AdmissionDiagnostics::one(
                M9AdmissionErrorKind::M8BaseEvidenceMismatch,
            ));
        };
        let Some(membership) = memberships.get(&(
            admission_principal.to_string(),
            relation.owner_locus().to_string(),
        )) else {
            return Err(M9AdmissionDiagnostics::one(
                M9AdmissionErrorKind::InvalidMembershipLineage,
        ...
        if !relation_bootstraps.contains(evaluation.name())
            || facts.insert(evaluation.name().to_string(), fact).is_some()
        {
            return Err(M9AdmissionDiagnostics::one(
                M9AdmissionErrorKind::M8BaseEvidenceMismatch,
            ));
...
fn finite_local_capability_and_witness(
    authority: &mut M9AuthorityRuntime,
    membership: &M9MembershipAuth,
    reference: &str,
    scope: M9CapabilityScope,
    source_ref: &SourceRef,
) -> Result<(M9CapabilityAuth, M9WitnessAuth), M9AdmissionDiagnostics> {
    let capability = authority.authorize_capability(
        M9CapabilityGrantRequest::new(reference)
            .with_membership_ref(membership.ref_id())
            .with_scope(scope)
            .with_lineage_epoch(membership.epoch())
            .with_source_ref(source_ref.clone()),
    )?;
    let witness = authority.materialize_witness(
        M9WitnessRequest::new(format!("finite-local-witness:{reference}"))
            .with_membership_ref(membership.ref_id())
            .with_capability_ref(capability.ref_id())
            .with_source_ref(source_ref.clone()),
    )?;
...
fn finite_local_m8_admission_for(
    checked: &CheckedSurfaceV0,
    relation_lifecycle_facts: &BTreeMap<String, M9FiniteFreshAtAdmissionLifecycleFact>,
) -> Result<M8RuntimeAdmission, M9AdmissionDiagnostics> {
    finite_local_m8_admission_for_provider_coverage(checked, relation_lifecycle_facts, None)
}

fn finite_local_m8_admission_for_provider_coverage(
    checked: &CheckedSurfaceV0,
    relation_lifecycle_facts: &BTreeMap<String, M9FiniteFreshAtAdmissionLifecycleFact>,
        ...
        match residual.kind() {
            ...
            ResidualObligationKind::RelationLifetime => {
                let relation = checked
                    ...
                    .ok_or_else(|| {
                        M9AdmissionDiagnostics::one(M9AdmissionErrorKind::M8BaseEvidenceMismatch)
                    })?;
                let binding_frontier = relation
                    .binding_frontier()
                    .as_slice()
                ...
                if lifecycle_fact.relation != residual.name()
                    || lifecycle_fact.binding_frontier != binding_frontier
                    || lifecycle_fact.core_derived()
                    || lifecycle_fact.grants_authority()
                {
                    return Err(M9AdmissionDiagnostics::one(
                        M9AdmissionErrorKind::M8BaseEvidenceMismatch,
                    ));
                ...
                admission = admission.with_evidence(M8AdmissionEvidence::RelationLifetime {
                    relation: residual.name().to_string(),
                    live_lease: lifecycle_fact.bounded_local_bootstrap_label(),
                    binding_frontier,
                    source_ref: residual.source_ref().clone(),
                });
            }
            ...
            ResidualObligationKind::ReadOnlyProviderEffectRuntimeUnsupported => {
                if provider_coverage.is_none_or(|coverage| {
                    coverage.program_identity() != checked.program_identity()
                        || !coverage.matches_checked(checked)
                }) {
                    return Err(M9AdmissionDiagnostics::one(
...
fn finite_local_m9_envelope_for(checked: &CheckedSurfaceV0) -> M9AdmissionEnvelope {
    let mut envelope =
...
impl M9RuntimeAdmitted {
    ...
    pub(crate) fn into_m10_execution_seam(
        self,
    ) -> Result<M9M10ExecutionSeam, M9AdmissionDiagnostics> {
    ...
    pub(crate) fn into_m8_execution_seam(
        self,
    ) -> Result<M8RuntimeInstance, M9AdmissionDiagnostics> {
        self.into_m10_execution_seam()
            .map(M9M10ExecutionSeam::into_parts)
            .map(|parts| parts.0)
    }
}
...
/// Shared final M9 lineage check for an actual authenticated membership,
/// capability, and witness. Callers supply the only capability scope that may
/// satisfy their sealed admission profile; this helper never creates an M8
/// execution carrier.
fn final_authority_lineage_matches_base(
    base: &M9AdmittedBase,
    authority_runtime: &M9AuthorityRuntime,
    membership_ref: &str,
    capability_ref: &str,
    witness_ref: &str,
    expected_scope: &M9CapabilityScope,
) -> bool {
    let auth_binding = base.m9_residual_bindings.0.iter().find(|binding| {
        binding.kind == ResidualObligationKind::AuthDeferred
            && binding.module.as_deref() == Some(base.program_identity.module())
    ...
    let (Some(membership), Some(capability), Some(witness)) = (
        authority_runtime.snapshot.memberships.get(membership_ref),
        authority_runtime.snapshot.capabilities.get(capability_ref),
        authority_runtime.snapshot.witnesses.get(witness_ref),
    ) else {
        return false;
    };
    outer == &base.outer_admission
        ...
        && capability.policy_version == "m9-policy-v1"
        && capability.source_ref.as_ref() == Some(auth_ref)
        && capability.scope == *expected_scope
        && witness.membership_ref == membership.reference
        && witness.capability_ref == capability.reference
        && witness.source_ref.as_ref() == Some(auth_ref)
}

fn contract_authority_lineage_matches_base(
    base: &M9AdmittedBase,
    authority_runtime: &M9AuthorityRuntime,
    membership: &M9MembershipAuth,
    capability: &M9CapabilityAuth,
    ...
    let Some(auth_binding) = base.m9_residual_bindings.0.iter().find(|binding| {
        binding.kind == ResidualObligationKind::AuthDeferred
            && binding.module.as_deref() == Some(base.program_identity.module())
            && binding.contract.as_deref()
                == Some(format!("{M9_AUTH_CONTRACT_PREFIX}{}", binding.name).as_str())
    ...
    let (Some(module), Some(contract)) = (
        auth_binding.module.as_deref(),
        auth_binding.contract.as_deref(),
    ) else {
        return false;
    };
    let expected_scope = M9CapabilityScope::contract_update(module, contract);
    final_authority_lineage_matches_base(
        base,
        authority_runtime,
        membership.ref_id(),
        capability.ref_id(),
...
fn provider_final_evidence_matches_base(
    base: &M9AdmittedBase,
    authority_runtime: &M9AuthorityRuntime,
    membership: &M9MembershipAuth,
    capability: &M9CapabilityAuth,
    ...
    let expected_scope = M9CapabilityScope::read_only_provider_effect(
        coverage,
        resource_runtime_nonce,
        policy_generation_ref,
    );
    final_authority_lineage_matches_base(
        ...
        witness.ref_id(),
        &expected_scope,
    ) && composite_discharge.program_identity() == coverage.program_identity()
        && composite_discharge.provider_coverage() == coverage
        && composite_discharge.activation_pending()
        && !composite_discharge.grants_authority()
        && !composite_discharge.permits_effect_use()
...
fn validate_outer(
    ...
    validate_outer_for_provider_coverage(checked, envelope, None)
}

fn validate_outer_for_provider_coverage(
    checked: &CheckedSurfaceV0,
    envelope: &M9AdmissionEnvelope,
    provider_coverage: Option<&M9ReadOnlyProviderEffectCoverage>,
) -> Result<(), M9AdmissionDiagnostics> {
...
/// Opaque provider capability scope.  Its fields and constructor remain M9
/// private, so naming the public capability enum cannot manufacture a scope
/// accepted by the dedicated issuer.
#[doc(hidden)]
#[derive(Clone, PartialEq, Eq)]
...
impl std::fmt::Debug for M9ReadOnlyProviderEffectScope {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("M9ReadOnlyProviderEffectScope(..)")
    }
}

impl M9ReadOnlyProviderEffectScope {
    fn from_admitted_composite(
        coverage: &M9ReadOnlyProviderEffectCoverage,
        resource_runtime_nonce: [u8; 32],
        policy_generation_ref: &str,
    ) -> Self {
...
pub enum M9CapabilityScope {
    ...
    /// Private Stage 2b capability.  Generic capability authorization always
    /// rejects this scope; only the checked composite policy seam may issue it.
    ReadOnlyProviderEffect(M9ReadOnlyProviderEffectScope),
}
...
impl M9CapabilityScope {
    ...
    fn read_only_provider_effect(
        ...
        Self::ReadOnlyProviderEffect(M9ReadOnlyProviderEffectScope::from_admitted_composite(
            coverage,
            resource_runtime_nonce,
            policy_generation_ref,
        ))
...
impl M9AuthorityRuntime {
    ...
    /// The only issuer for the provider-effect capability.  It is private to
    /// the composite path and requires a current authenticated membership plus
    /// exact checked coverage; callers cannot obtain this through the generic
    /// `authorize_capability` request surface.
    fn issue_read_only_provider_effect_capability(
        &mut self,
        membership: &M9MembershipAuth,
        coverage: &M9ReadOnlyProviderEffectCoverage,
        resource_runtime_nonce: [u8; 32],
        ...
        let contract = coverage.contract();
        if membership.principal() != contract.requester_principal()
        ...
        let scope = M9CapabilityScope::read_only_provider_effect(
            coverage,
            resource_runtime_nonce,
            policy_generation_ref,
        );
        let reference = m9_opaque_ref(&format!(
            "read-only-provider-effect-capability:{}:{}:{}",
            coverage.program_identity().stable_key(),
            membership.ref_id(),
            contract.operation(),
        ));
    ...
    fn permits_capability_scope(
        ...
        match scope {
            ...
            // A provider-effect scope is never available through this generic
            // request API.  Its issuer below additionally checks the sealed
            // composite coverage and the separate fixed policy decision.
            M9CapabilityScope::ReadOnlyProviderEffect(_) => false,
        }
    }
...
impl M9ContractAuthorityUse {
    pub fn from_grant_and_witness(capability: &M9CapabilityAuth, witness: &M9WitnessAuth) -> Self {
        let target_module = match &capability.scope {
            ...
            | M9CapabilityScope::DesignatedRemoteInputRelease { .. }
            | M9CapabilityScope::DesignatedConsumption { .. }
            | M9CapabilityScope::ReadOnlyProviderEffect(_) => None,
        };
        Self {
...
impl M9ObserverRequest {
    pub fn from_grant_and_witness(capability: &M9CapabilityAuth, witness: &M9WitnessAuth) -> Self {
        let observer_principal = match &capability.scope {
            ...
            | M9CapabilityScope::DesignatedRemoteInputRelease { .. }
            | M9CapabilityScope::DesignatedConsumption { .. }
            | M9CapabilityScope::ReadOnlyProviderEffect(_) => String::new(),
        };
        Self {
            observer_principal,
...
#[cfg(test)]
#[path = "m9_auth_verification_i3_provider_composite_tests.rs"]
mod m9_auth_verification_i3_provider_composite_tests;
