...
pub struct CleanNearEndCloseout {
    ...
    pub message_envelope_scope: String,
    ...
    pub transport_mediums: Vec<String>,
    pub reserved_transport_mediums: Vec<String>,
    ...
    pub retention_scope_names: Vec<String>,
    pub reserved_retention_scope_names: Vec<String>,
    pub layer_signatures: Vec<LayerSignature>,
...
fn message_envelope(
    ...
    MessageEnvelope {
        ...
        transport_medium: transport_medium.map(|value| value.to_string()),
        ...
        membership_epoch,
        member_incarnation,
        freshness_checks: collect(freshness_checks),
...
fn visualization_view(
    ...
    VisualizationView {
        ...
        source_refs: collect(source_refs),
...
fn reserved_visualization_view_names() -> Vec<String> {
    ["label_authority_redaction_retention_grid"]
        .into_iter()
        .map(|name| name.to_string())
        .collect()
}

fn retention_scope_names(
    visualization_views: &[VisualizationView],
    telemetry_rows: &[TelemetryRow],
) -> Vec<String> {
    visualization_views
        .iter()
        .map(|view| view.retention_scope.clone())
        .chain(telemetry_rows.iter().map(|row| row.retention_scope.clone()))
        .collect::<BTreeSet<_>>()
        .into_iter()
        .collect()
}
        ...
        "05_delegated_rng_service" => vec![
            message_envelope(
                ...
                principal_claim(
                    ...
                    &["UseDelegatedRng(Alice)", "ObserveProviderReceipt"],
                ),
                "Alice",
                0,
                0,
                &[
                    "membership_epoch matches request frontier",
                    "member_incarnation matches live participant",
                ],
    ...
    match spec.id.as_str() {
        "05_delegated_rng_service" => vec![
            visualization_view(
                ...
                "message_route",
                "report:provider-boundary",
                "ObserveProviderBoundary(ProviderPlace[AuthorityRng])",
                "named_witness_only",
                "report_local_inventory",
                &[
                    "message_envelopes[provider_request#1]",
                    "message_envelopes[provider_receipt#1]",
                    "layer_signatures[transport_provider_boundary]",
                ],
                &layer_signature_refs,
                &message_envelope_refs,
            ...
            visualization_view(
                ...
                "projection_view",
                "report:projection",
                "InspectProjection(DelegatedRngSource#1)",
        ...
        "06_auditable_authority_witness" => vec![visualization_view(
            ...
            "report_local_inventory",
            &[
                "message_envelopes[audit_trace_request#1]",
                "layer_signatures[auth_authority_witness]",
            ],
            &layer_signature_refs,
            &message_envelope_refs,
...
pub fn build_clean_near_end_closeout() -> Result<CleanNearEndCloseout, CleanNearEndError> {
    ...
    Ok(CleanNearEndCloseout {
        ...
        message_envelope_scope: "clean_near_end_canonical_inventory".to_string(),
        ...
        transport_mediums: vec![],
        reserved_transport_mediums: vec![
        ...
        reserved_transport_seams: vec![
            "attach_point_boundary".to_string(),
            "game_action_boundary".to_string(),
            "membership_registry_boundary".to_string(),
            "published_history_boundary".to_string(),
        ],
        visualization_views,
        ...
        reserved_retention_scope_names: vec!["helper_local_ephemeral".to_string()],
