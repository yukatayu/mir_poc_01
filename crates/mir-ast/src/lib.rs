#![doc = r#"
# mir-ast

Syntax trees and normalized language structures for Mir.

This crate currently exposes a **non-production current L2 parser carrier floor**
for the Phase 6 front-half compile-ready checkpoint.

The current first tranche keeps the accepted surface narrow:
- stage 1 option/chain declarations
- stage 2 try/fallback structural shape

Stage 3 admit / request / predicate helpers remain in this crate as
retained-later spike support for comparison and bridge evidence. They are not
the current first-tranche parser carrier, and they are not a final public
parser API.

Stage 3 request clause suites, perform head publicization, span-rich
diagnostics, and final grammar remain later work.
"#]

pub mod current_l2;

pub fn crate_name() -> &'static str {
    "mir_ast"
}
