...
#[test]
fn proof_notebook_review_unit_support_emits_runtime_review_rows() {
    ...
    let review_unit = build_proof_notebook_review_unit_artifact(&formal_hook).unwrap();

    assert_eq!(review_unit.subject_kind, "runtime_try_cut_cluster");
    assert_eq!(review_unit.subject_ref, "e2_try_fallback");
    assert_eq!(review_unit.review_rows.len(), 1);
    assert_eq!(
        review_unit.review_rows[0].obligation_kind,
        "rollback_cut_non_interference"
    );
    assert_eq!(
        review_unit.review_rows[0].evidence_refs,
        vec![
    ...
    assert!(
        review_unit.review_rows[0].goal_text.contains("rollback"),
        "unexpected goal text: {}",
        review_unit.review_rows[0].goal_text
    );
    assert!(
        review_unit.review_rows[0]
            .goal_text
            .contains("e2_try_fallback"),
        "unexpected goal text: {}",
        review_unit.review_rows[0].goal_text
    );
...
#[test]
fn proof_notebook_review_unit_support_emits_static_review_rows() {
    ...
    assert_eq!(review_unit.subject_kind, "fixture_static_cluster");
    assert_eq!(review_unit.subject_ref, "e5_underdeclared_lineage");
    assert_eq!(review_unit.review_rows.len(), 2);
    assert_eq!(
        review_unit.review_rows[0].obligation_kind,
    ...
    assert_eq!(
        review_unit.review_rows[1].obligation_kind,
        "no_re_promotion"
    );
    assert!(
        review_unit.review_rows[1]
            .goal_text
            .contains("not re-promoted"),
        "unexpected goal text: {}",
        review_unit.review_rows[1].goal_text
    );
    assert_eq!(review_unit.checklist.len(), 4);
}
...
fn proof_notebook_review_unit_support_rejects_wrong_schema_or_artifact_kind() {
    ...
    let schema_error = build_proof_notebook_review_unit_artifact(&wrong_schema).unwrap_err();
    assert!(
    ...
    let kind_error = build_proof_notebook_review_unit_artifact(&wrong_kind).unwrap_err();
    assert!(
