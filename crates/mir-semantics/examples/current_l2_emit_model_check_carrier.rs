...
#[allow(dead_code)]
#[path = "support/current_l2_model_check_carrier_support.rs"]
...
fn usage(program: &str) -> String {
    format!(
        "usage: {program} <formal-hook-path> [--output <artifact-path>]\n\
         ...
         /tmp/e2.formal-hook.json --output /tmp/e2.model-check.json"
    )
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let program = env::args()
        .next()
        .unwrap_or_else(|| "current_l2_emit_model_check_carrier".to_string());
    ...
    let carriers = build_model_check_concrete_carrier_artifacts(&formal_hook)?;
