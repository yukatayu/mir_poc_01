...
| Mir computational core | first executable row plus planned widening queue | `python3 scripts/mir_computational_samples.py check-all --format json` | `specs/28` / `plan/53`, `samples/product-alpha1/computational/add-one-pure-mir/package.mir.json`, helper, unit tests, and runtime tests now prove `ReadInt -> add_one -> WriteInt` while other rows remain planned | widen to `P-COMP-03..04` without changing legacy adapter-owned `typed_host_io.add_one` |
...
| autonomous execution contract | boundary-fixed, no sample claim | docs validation plus package helpers | `specs/32` / `plan/57` define front-half closeout, implementation half, package cadence, and close protocol | continue `P-COMP-03..04` / `P-POSE-02` implementation packages |
...
| `samples/product-alpha1/computational/` | one executable Mir-owned computation root plus planned widening roots | `python3 scripts/mir_computational_samples.py check-all --format json` | `add-one-pure-mir/` is executable via `package.mir.json`; remaining representative `.mir` rows stay planned-only |
...
| Mir-owned computation | bounded runnable anchor plus widening queue | `P-COMP-02` now proves one runnable Mir-owned row; `P-COMP-03..04` remain; current legacy `AddOne` is still host-boundary evidence only |
