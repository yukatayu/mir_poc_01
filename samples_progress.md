...
Last updated: 2026-05-21 20:53 JST

Current repo-local focus: current-L2 runnable floor, practical alpha-1 evidence, bounded operational α-0.5 / α-0.8 / α-0.9 workflows, product alpha-1 release candidate, installed-binary adoption probe, canonical operational product sample suite, and the implementation-half queue after `P-COMP-03` closeout. `samples/alpha/` remains alpha-0 evidence; `samples/practical-alpha1/` remains first-floor fixture evidence; `samples/product-alpha1/` is the current product alpha root. Docker skip paths are partial local probes, not release-candidate evidence.
...
| Mir computational core | bounded first-floor evidence plus remaining effect-boundary package | `python3 scripts/mir_computational_samples.py check-all --format json` | `specs/28` / `plan/53`, `samples/product-alpha1/computational/add-one-pure-mir/package.mir.json`, helper, unit tests, and runtime tests now prove one direct `ReadInt -> add_one -> WriteInt` row plus 5 accepted and 5 expected runtime rejection first-floor rows | `P-COMP-04` effect / failure / capability boundary without changing legacy adapter-owned `typed_host_io.add_one` |
...
| autonomous execution contract | boundary-fixed, no sample claim | docs validation plus package helpers | `specs/32` / `plan/57` define front-half closeout, implementation half, package cadence, and close protocol | continue `P-COMP-04` / `P-POSE-02` implementation packages |
...
| `samples/product-alpha1/computational/` | one direct runtime row, ten helper-executable first-floor rows, one planned-only row | `python3 scripts/mir_computational_samples.py check-all --format json` | `add-one-pure-mir/` is executable via `package.mir.json`; `comp-03` rows execute through helper package contracts; `comp-04` remains planned-only |
...
| Mir-owned computation | bounded first-floor evidence plus remaining effect boundary | `P-COMP-02` proves one runnable Mir-owned row and `P-COMP-03` proves positive / negative first-floor rows; `P-COMP-04` remains; current legacy `AddOne` is still host-boundary evidence only |
