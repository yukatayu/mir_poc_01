...
Last updated: 2026-04-28 21:45 JST
Current repo-local focus: clean near-end current layer と Sugoroku world / avatar follow representative slice の runnable floor、viewer typed public prototype inventory、storage/backend current first-cut closeout、`R2` AttachPoint minimal-contract closeout memory と `R3` FAIRY-05 carrier-bundling closeout memory を保ったまま、`R4` hot-plug real migration / rollback boundary で current minimal contract row の先に残る kept-later hot-plug workを docs-first に揃える
Current active packages: `R4` hot-plug real migration / rollback boundary (`R3` `FAIRY-05` visibility-return carrier bundling は close 済み。next は current minimal contract row の先に残る kept-later hot-plug boundary を整理する)
...
| Hot-plug package | 100 | helper-local lifecycle canary + package-manager first-cut closeout + minimal-contract closeout closed | `hotplug_scope` / `hotplug_anchor_samples` / `hotplug_package_concerns` / `hotplug_lifecycle_lanes` / `hotplug_anchor_envelopes` / `hotplug_view_ids` / `hotplug_telemetry_row_ids` / `hotplug_kept_later_gates` / `hotplug_validation_floor` を closeout に固定しつつ、`R2` で `compatibility / activation_cut / detach_boundary / migration_contract` minimal contract row と grounding anchor を `plan/30` に整理し、real migration を deferred に保つ | `python3 scripts/sugoroku_world_samples.py closeout --format json` |
...
|---|---|---|---|---:|---|---|---|---|
| `PH0` | repository memory | `samples_progress.md`, `docs/reports/`, `scripts/check_source_hierarchy.py` | dashboard / hierarchy check | 90 | mixed | 2026-04-28 22:05 JST | `0913`, `0920`, `0943`, `0945`, `0955`, `0956`, `0957`, `0958`, `0959`, `0960`, `0961`, `0962`, `0963`, `0964`, `0965`, `0966`, `0967`, `0968`, `0969`, `0970`, `0971`, `0972`, `0973`, `0974`, `0975`, `0976`, `0977`, `0978` | source hierarchy と report discipline の current floor。`R3` closeout memory と `R4` promoted-next wording まで同期済み |
...
| `SUG-01` | Sugoroku runtime attach | `samples/clean-near-end/sugoroku-world/01_runtime_attach_game.mir` | active runnable | 90 | positive | 2026-04-28 21:19 JST | `0909`, `0916`, `0931`, `0945`, `0955`, `0977` | runtime attach floor + helper-local attachpoint compatibility / activation evidence。`R2` closeout では `compatibility` / `activation_cut` minimal contract row の primary anchor になり、`world_surface` と `MembershipRegistry` source-of-truth の anchor でもある |
...
| `SUG-09` | hot-plug preview | `samples/clean-near-end/sugoroku-world/09_detach_todo.mir` | active TODO boundary | 75 | explicit TODO + rejection evidence | 2026-04-28 21:19 JST | `0909`, `0916`, `0925`, `0931`, `0955`, `0977` | completion evidence ではなく stop line。`R2` closeout では `detach_boundary` / `migration_contract` minimal contract row と `detached_roll_request#1` rejection evidence の anchor であり、completed migration ではない |
...
| `PH14` | hot-plug package | `plan/21-hotplug-attachpoint-roadmap.md`, `plan/30-attachpoint-detach-minimal-contract.md`, `samples/clean-near-end/sugoroku-world/09_detach_todo.mir` | helper-local lifecycle canary + package-manager first-cut closeout + minimal-contract closeout | 100 | positive + negative | 2026-04-28 21:19 JST | `0925`, `0931`, `0955`, `0968`, `0969`, `0977` | helper closeout inventory に加え、`R2` で `compatibility / activation_cut / detach_boundary / migration_contract` minimal contract row、grounding envelope / view / telemetry anchor、kept-later migration / rollback gate を fixed。real migration / rollback / attachpoint runtime remain deferred |
