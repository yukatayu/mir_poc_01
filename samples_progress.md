...
Last updated: 2026-05-22 14:46 JST

Current repo-local focus: current-L2 runnable floor, practical alpha-1 evidence, bounded operational α-0.5 / α-0.8 / α-0.9 workflows, product alpha-1 release candidate, installed-binary adoption probe, canonical operational product sample suite, `P-PAT-01` minimal alpha-1 pattern verification, and the first `P-MIR-01..04` plus `P-POSE-03..04` plus `P-PROJ-03` Full System V1 parser+checker+bounded-effectful-runtime+PoseGraph-runtime+pose-save/devtools+projection-schema lane with `P-PROJ-04` promoted. `samples/alpha/` remains alpha-0 evidence; `samples/practical-alpha1/` remains first-floor fixture evidence; `samples/product-alpha1/` is the current product alpha root. `samples/full-system-v1/computational/`、`samples/full-system-v1/avatar-pose/`、`samples/full-system-v1/projection/` は evidence-closed だが、wider Full System V1 roots remain non-workflow-ready until later packages add role execution and provider evidence. Docker skip paths are partial local probes, not release-candidate evidence.
...
| projection / engine adapter boundary | bounded projection IR + boundary-schema anchor exists; engine/provider still inventory-only | `P-PROJ-01` / `P-ENG-01` scaffolds remain as comparison anchors, while `P-PROJ-03` actualizes `samples/full-system-v1/projection/`; no engine execution yet |
