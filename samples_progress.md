...
| operational product sample suite | workflow-ready canonical suite, not final product | `python3 scripts/operational_product_samples.py check-all --format json` | `samples/product-alpha1/operational/world-core`, `membership-chat`, `sugoroku-world`, `portal-worldlink`, shared debug/auth/rate-limit/object/avatar packages, local/Docker deployment profiles, schema-backed projection profile, retained portal blueprint / future shard inventory, `scripts/operational_product_samples.py`, `docs/hands_on/operational_product_sample_01.md`, `docs/research_abstract/operational_product_sample_01.md`, `membership-chat` bounded `EchoText("Taro") -> "Hello, Taro!"` lane, `sugoroku-world` bounded roll / publish / witness / handoff / stale membership reject scenario, `portal-worldlink` bounded resolve / handoff offer / witness emit / destination admit scenario, observer-safe devtools bundle with source/dependency/projection/portal/shard panels, helper semantic check for projection inventory and portal runtime evidence | two-shard hard-boundary model-check |
