...
Last updated: 2026-04-28 16:00 JST
Current repo-local focus: clean near-end current layer と Sugoroku world / avatar follow representative slice の runnable floor を維持したまま、`P12` external adapter / host boundary tranche の current first cut closeout を保持し、その次の promoted line として `P13` network transport minimal alpha を helper-local `process_boundary` closeout に留めて transport floor を tighten する
Current active packages: `P13` network transport minimal alpha (`promoted next`; safest first cut = helper-local `process_boundary` closeout)
...
| Network transport | 75 | helper-local canary family + promoted next closeout | `NET-01..05` helper-local canary を維持しつつ `P13` first cut では `helper_local_process_boundary` floor、stale reconnect reject、typed failure family、observer-safe redacted route trace を tighten し、real socket / durable replay は deferred に保つ | `python3 scripts/network_transport_samples.py closeout --format json` |
...
|---|---|---|---|---:|---|---|---|---|
| `PH0` | repository memory | `samples_progress.md`, `docs/reports/`, `scripts/check_source_hierarchy.py` | dashboard / hierarchy check | 90 | mixed | 2026-04-28 16:00 JST | `0913`, `0920`, `0943`, `0945`, `0955`, `0956`, `0957`, `0958`, `0959`, `0960`, `0961`, `0962`, `0963`, `0964`, `0965`, `0966` | source hierarchy と report discipline の current floor |
...
| `PH13` | network transport | `scripts/network_transport_samples.py`, `samples/not_implemented/network-transport/` | helper-local canary family + planned source family | 75 | positive + negative | 2026-04-28 03:27 JST | `0926`, `0929`, `0932`, `0945` | local queue / loopback / reconnect / typed failure / redacted route trace。`P13` promoted next line は real transport alpha ではなく helper-local `process_boundary` closeout |
