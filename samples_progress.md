...
Last updated: 2026-05-07 12:25 JST
Current repo-local focus: current-L2 base source corpus, clean near-end runnable floor, Lean foundations / generated theorem stubs, practical alpha-1 first-floor toolchain, bounded operational α-0.5 / α-0.8 / α-0.9 runtime lines, the bounded practical α-1 integrated workflow carrier, the product alpha-1 release-candidate workflow, the installed-binary adoption probe, the front-door grammar/ABI hardening scope, the narrowed alpha replay shipped-surface boundary, the narrowed broader-distribution stance, the narrowed operational room-chat scope, the narrowed portal/shard starter stance, the narrowed operational Sugoroku scope, the hardened widening-queue stance, the user-final decision gate scoping, and the canonical operational product sample suite. `samples/alpha/` remains the alpha-0 evidence root; `samples/practical-alpha1/` remains the practical first-floor fixture root; `samples/product-alpha1/` now has product alpha schema/check, local same-session run/session/attach, local R0/R2 save evidence, local/Docker transport, non-final devtools/viewer evidence, native host launch bundle evidence, CLI `demo`, release check evidence, `scripts/product_alpha1_installed_binary_check.py` installed-binary evidence plus compatibility-scope / `shipped_surface` / `distribution_scope` output, and the `operational/` suite for `WorldCore -> MembershipChat -> SugorokuWorld -> PortalWorldLink -> TwoShardHardBoundary -> TwoShardGradientObservation` with bounded room-oriented `MembershipChat` `ChatText("hello room") -> "room#lobby message accepted: hello room"` evidence plus helper-reported `room_chat_scope`, `SugorokuWorld` bounded runtime evidence plus helper-reported `sugoroku_scope`, schema-backed projection inventory, bounded portal discrete handoff evidence, bounded two-shard hard-authority evidence, bounded observer-only gradient runtime evidence, a `template_only` `world-core` / `membership-chat` / `sugoroku-world` starter catalog plus guide, helper-reported `portal_shard_starter_scope`, helper-reported `widening_queue_scope`, helper-reported `user_final_decision_scope`, the docs-first boundary that keeps portal/shard authoring on active roots rather than starter duplicates, a docs-first backend comparison inventory, and a non-executable `gradient-observation.profile.json` future profile paired with the separate runtime root. Docker skip paths are partial local probes, not release-candidate evidence.
...
|---|---|---|---|
| 2026-05-07 12:25 JST | `P-OPS-26` later user-final distribution decision scoping | pass | helper に machine-readable `user_final_decision_scope` を追加し、current delivery unit を developer-built binary + generated host launch bundle、current catalog scope を bounded product alpha-1 narrow showcase に固定した。broader final distribution / final shared-space catalog breadth は user-spec-required gate のままとし、runtime widening は行わなかった |
| 2026-05-07 12:03 JST | `P-OPS-25` broader Sugoroku reopening queue-state hardening | pass | helper の machine-readable `widening_queue_scope` を更新し、room-chat reopening、portal/shard starter reopening、broader Sugoroku reopening を non-promoted line に戻した。next reopen point は later user-final distribution decision に進め、runtime widening は行わなかった |
| 2026-05-07 11:37 JST | `P-OPS-24` later room-chat reopening queue-state hardening | pass | helper に machine-readable `widening_queue_scope` を追加し、later room-chat reopening と portal/shard starter reopening を non-promoted line に戻した。next reopen point は broader Sugoroku reopening に進め、runtime widening は行わなかった |
| 2026-05-07 11:20 JST | `P-OPS-23` broader Sugoroku revisit | pass | helper に machine-readable `sugoroku_scope` を追加し、current `sugoroku-world` carrier を bounded deterministic same-session roll / publish / witness / handoff / stale-membership reject scenario に固定した。interactive turn choice / broader negative rows / networked multi-participant control は current line では未定義のままとし、next reopen point を later room-chat reopening に進めた |
| 2026-05-07 11:03 JST | `P-OPS-22` portal/shard starter revisit | pass | helper に machine-readable `portal_shard_starter_scope` を追加し、validated starter catalog が `templates/sugoroku-world-starter` で止まり portal/shard authoring は active executable roots を study/copy boundary に使う current lineを固定した。starter duplicates と executable `future/` roots は current line では未定義のままとし、next reopen point を broader Sugoroku revisit に進めた |
| 2026-05-07 10:42 JST | `P-OPS-21` broader room-chat revisit | pass | helper に machine-readable `room_chat_scope` を追加し、current `membership-chat` lane を bounded single-message room-oriented `ChatText` に固定した。multi-message / transport-coupled / room-history / stdio は current line では未定義のままとし、next reopen point を portal/shard starter revisit に進めた |
| 2026-05-07 10:22 JST | `P-OPS-20` broader public distribution narrowing | pass | helper に machine-readable `distribution_scope` を追加し、current delivery unit を developer-built `mirrorea-alpha` binary + locally generated native host launch bundle のみに固定した。archive / installer / system-package / auto-update / hosted-service shape は current line では未定義とし、next reopen point を broader room-chat revisit に進めた |
| 2026-05-07 10:09 JST | `P-OPS-19` shipped-surface hardening | pass | helper / bundle stdout / `manifest.json` / `verification-report.json` に machine-readable `shipped_surface` block を追加し、built-binary `check` / `build-native-bundle` / `demo`、bundle replay `run.sh check` / `run.sh view`、bundled CLI / package root / `manifest.json` / `launch.json` / `run.sh` / `README.md` / observer-safe supporting artifacts を current alpha replay bundle surface に固定した。next reopen point は broader public distribution narrowing |
| 2026-05-07 09:57 JST | `P-OPS-18` final grammar / ABI scoping | pass | `specs/25` と installed-binary helper output に current hardening target を追加し、versioned `package.mir.json`、documented `mirrorea-alpha` command family、native host launch bundle replay surface を alpha-stable front door として整理した。next reopen point は shipped-surface hardening |
| 2026-05-07 09:28 JST | `P-OPS-17` installed binary adoption probe first cut | pass | `scripts/product_alpha1_installed_binary_check.py` で built `mirrorea-alpha` binary と generated bundle `run.sh check/view` probe を追加し、current first public-ish adoption candidate を installed binary + native host launch bundle に固定した。next reopen point は final grammar / ABI scoping |
| 2026-05-07 09:12 JST | `P-OPS-16` final-public gate scoping | pass | docs/roadmap/dashboard queue を再整理し、next promoted line を public packaging adoption target scoping に絞った。installed binary + native host launch bundle を first public-ish candidate とし、grammar / ABI / WAN / durability は後段 gate に戻した |
