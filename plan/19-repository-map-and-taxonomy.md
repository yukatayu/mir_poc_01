...
- `crates/mirrorea-core`, `crates/mirrorea-control`
  Mirrorea boundary marker の placeholder
- `crates/prism-*`
...
| `mirrorea-core` | Mirrorea runtime substrate placeholder | medium |
...
- `mirrorea-*` / `prism-*` / `engine-abi` は placeholder だが、名前自体が subsystem separation を表しているので、早い rename はしない
