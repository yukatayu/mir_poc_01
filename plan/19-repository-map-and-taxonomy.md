...
| `mirrorea-core` | Mirrorea minimal carrier substrate (`LayerSignature` / `PrincipalClaim` / `AuthEvidence` / `MessageEnvelope`) | medium |
...
- `mirrorea-core` は current ownership cut が入り始めたが、まだ final public crate shape ではないので rename / rebucket しない
