...
- `specs/examples/520` により、current self-driven queue は next active line を
  ...
  に reopened して読む。
- `specs/examples/521` と `522` により、`Lean formal skeleton / proof obligations` の first slice と IFC secret valid/invalid concrete example は `samples/lean/foundations/CurrentL2ProofSkeleton.lean`、`CurrentL2IfcSecretExamples.lean`、representative theorem quartet committed Lean corpus、export/sync helper に actualize 済みである。current live queue はそこから先の source-side explicit authority / label-flow corpus と helper/CLI hardening と later mixed gate に narrowed すると読む。
