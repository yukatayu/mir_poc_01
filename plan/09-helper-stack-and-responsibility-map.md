...
- current sample-visible message/auth seam cut では、
  ...
  - `P19` current closeout 後は、engine-neutral `HotPlugRequest` / `HotPlugVerdict` と `hotplug_request_lanes` / `hotplug_verdict_lanes` も `crates/mirrorea-core` に actualize してよい。ただし helper-local `hotplug_lifecycle`、sample-grounded attach/detach IDs、attach/detach view/telemetry IDs は kept-later preview ownership に残し、`mir-runtime` orchestration skeleton や completed engine state machine と collapse しない。
...
- current sample-visible verifier preview cut では、
  ...
  - `R1` current line では theorem bridge / runtime policy / visualization-telemetry を `verification_preview` や downstream consumer lane に残し、active emitted `LayerSignature` row へはまだ上げない。
