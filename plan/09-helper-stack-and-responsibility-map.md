...
この later ordering は維持するが、current roadmap では **その前段に sample-visible theorem/model-check milestone を置き、そのうち first actual carrier までは actualize 済みで、current line は source-sample emitted verification artifact wiring にある。その後に docs-first の host-facing I/O / adapter boundary を別 gate として切る** のが自然である。ここでいう `host-facing port` は working label であり、final terminology は OPEN に残す。
