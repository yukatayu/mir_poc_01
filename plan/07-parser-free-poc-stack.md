...
この文書に出てくる current next-step / reopen wording は parser-boundary の technical memory であり、`progress.md` / `tasks.md` が current promoted workline を maintenance と読む場面では queue authority ではない。
...
parser-free stack の上に重ねる narrow reopen memory として読む。current snapshot が maintenance-only line を示す間、この wordingは active package order を意味しない。
...
- current actualization として、`crates/mir-ast/src/current_l2.rs` の `parse_stage3_request_clause_suite_text()` と `crates/mir-ast/tests/current_l2_stage3_request_clause_suite_spike.rs` で fixed two-slot suite bridge first tranche を crate-local non-production carrier actualization として通してよい
...
- current actualization として、その `Stage3RequestContractSubset` helper-local / test-only first tranche は `crates/mir-ast/tests/support/current_l2_stage3_predicate_fragment_spike_support.rs` と `crates/mir-ast/tests/current_l2_stage3_request_clause_suite_spike.rs` で source-backed に通してよい
  - fixture-side compare は `contract.require` / `contract.ensure` の 2 slot に留める
  - clause array が 2 個以上の predicate row を持つ widen は `outside stage 3 first tranche` として still later に残す
- current guard judgment としては、その後の next step でも contract-only compare surface を row-list carrier へすぐ widen するのではなく、still 0-or-1 guard に留めるのが自然である
  - current fixture corpus には multi-row clause array anchor がない
  - explicit `and` による grouped predicate fragment は already compare できる
- current sequencing judgment としては、その widening guard comparison の次に source-side suite bridge widening 条件をさらに積むより、この request contract subset family は current tranche で一旦 freeze し、Phase 3 主線を parser boundary staging と first checker cut 接点の re-sweep へ戻す方が自然である
- current side-selection judgment としては、その re-sweep の戻り先は parser boundary staging 側より first checker cut connection 側を先に取る方が自然である
  - parser-side accepted cluster widening をすぐ再開するより、existing parser evidence を checker-side inventory へどう reconnect するかを先に比べる
- current reconnect-family judgment としては、その first reconnect family は stage 1 chain / declaration structural floor family を先に取る方が自然である
  - `specs/examples/45-current-l2-first-checker-cut-regression-baseline.md` の same-lineage / missing-option / capability floor baseline と最も近い
  - `specs/examples/73-current-l2-first-parser-spike-staging.md` の checker-led staged spike と順序が一致する
- current actualization cut としては、その first tranche は existing `e4` / `e7` working set を維持しつつ、`e13` と `e16` を representative fixture compare に追加し、`Stage1ReconnectClusters` summary を helper-local / test-only に置く方が自然である
  - `e17` の direct reconnect は、current stage 1 surface で structural predecessor が chain progression に固定されるため still later に残す
- current sequencing judgment としては、その後の next reconnect step は stage 1 `e18` / `e19` / `e20` widening より先に stage 2 `try` / rollback structural floor reconnect を取る方が自然である
  - `e19` が current `Stage1ReconnectClusters` summary contract を押し広げやすい一方、stage 2 側には `checked_try_rollback_structural_*` dedicated contract と `e23` / `e24` pair が already あるためである
- current actualization cut としては、その stage 2 first tranche は `crates/mir-ast/tests/support/current_l2_stage2_try_rollback_spike_support.rs` と `crates/mir-ast/tests/current_l2_stage2_try_rollback_spike.rs` で helper-local / test-only に actualize してよい
  - input surface は inline test string の single `try { ... } fallback { ... }` block に留める
  - statement head は `atomic_cut` と opaque `Other` だけを区別する
  - compare surface は fixture-side `checked_try_rollback_structural_verdict` / `checked_try_rollback_structural_findings` に直接合わせる
  - `e23` / `e24` malformed pair と valid one-shot `atomic_cut` in try body の `no_findings` smoke を first tranche に入れてよい
...
- current next widening judgment としては、stage 2 `E21` / `E22` runtime contrast を先に parser-side reconnect へ mirror するより、current summary contract を保ったまま stage 1 `e18` / `e20` widening を先に actualize する方が自然である
  - `E21` / `E22` の差は runtime / proof boundary により近く、current `checked_try_rollback_structural_*` compare surface では new reconnect evidence が薄いためである
- current actualization としては、その stage 1 summary-preserving widening tranche は support helper widening なしに `crates/mir-ast/tests/current_l2_stage1_parser_spike.rs` の focused fixture compare と summary compare を追加する cut に留めてよい
  - `e18` は `same_lineage_floor + missing_option_structure_floor`
  - `e20` は `same_lineage_floor + capability_strengthening_floor`
  - `e19` の direct reconnect は summary redesign pressure があるため still later に残す
- current threshold judgment としては、その後も `e19` を reconnect summary へ追加したり、stage 2 `E21` / `E22` runtime contrast を parser-side reconnect へ mirror したりするより、reconnect subline 自体を current tranche で freeze する方が自然である
  - `e19` は already declared target edge pair family の typed static reason anchor を持つ
  - `E21` / `E22` は runtime / proof boundary に近く、current `checked_try_rollback_structural_*` compare surface を widen しない限り reconnect helper に自然に乗らない
  - したがって current reconnect line は、stage 1 representative widening と stage 2 malformed pair + valid one-shot smoke までで一区切りとしてよい
- current parser subset freeze judgment としては、Phase 6 front-half actual parser first tranche の accepted cluster は
  - stage 1 chain / declaration structural floor
  - stage 2 try / rollback structural floor
  に留めるのが自然である
  - truly malformed な first-tranche reject family は `missing edge-local lineage metadata`、`missing fallback body`、`atomic_cut` fallback placement に留める
  - stage 3 `admit` / request clause / predicate fragment line は helper-local evidence を維持したまま retained-later floor に残す
- current reading では、stage 3 helper が返す spillover wording は permanent parser rejection ではなく **out-of-tranche evidence** と読む
- current parser-to-checker reconnect freeze judgment としては、Phase 3 -> 5 bridge は
  - stage 1 support helper の `Stage1ReconnectClusters` summary floor
  - stage 2 support helper の `checked_try_rollback_structural_*` floor
  に留めるのが自然である
  - stage 3 request / predicate reconnect helper line
  - `e19` direct target mismatch redesign
  - `E21` / `E22` runtime contrast
  は retained-later / runtime-proof boundary 側へ残す
