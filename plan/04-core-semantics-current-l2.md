...
- current parser-free PoC では rollback frame が whole `place_store` snapshot を保持する。
- したがって current reading では、locality は user-facing statement であり、restore scope の exact shapeは runtime / proof boundary に残る。
...
- frontier update 自体は `place_anchor == current_place` の runtime gate を持ち、nested place の `AtomicCut` は event-only で終わりうる。
