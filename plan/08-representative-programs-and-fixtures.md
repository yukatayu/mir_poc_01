...
- corrected runnable prototype set は `p01-dice-publication-handoff`、`p02-dice-publication-fallback`、`p03-avatar-controller-attach-detach`、`p04-dice-owner-duplicate-declaration`、`p05-dice-owner-guarded-chain`、`p06-typed-proof-owner-handoff`、`p07-dice-late-join-visible-history`、`p08-dice-stale-reconnect-refresh`、`p09-dice-delegated-rng-provider-placement`、`p10-typed-authorized-fingerprint-declassification`、`p11-typed-unauthorized-fingerprint-release`、`p12-typed-classified-fingerprint-publication-block`、`p13-dice-late-join-missing-publication-witness`、`p14-dice-late-join-handoff-before-publication`、`p15-typed-capture-escape-rejected`、`p16-typed-remote-call-budget-exceeded` である。
  ...
  - `p13 / p14` は authoritative-room late-join visibility line の helper-local negative static-stop pair であり、final parser grammar や final source wordingを意味しない。
  - `p15` は capture / lifetime negative first slice、`p16` は simple cost negative first slice であり、どちらも checker-adjacent finite-index first layer の corrected prototype であって final typed source principal や final public verifier contract を意味しない。
...
| `e14-malformed-duplicate-option-declaration` | duplicate option declaration | `malformed` | `not_evaluated` | runtime に入らない | なし | E14, `0157`, `0713` |
| `e15-malformed-duplicate-chain-declaration` | duplicate chain declaration | `malformed` | `not_evaluated` | runtime に入らない | なし | E15, `0157`, `0713` |
