...
- current static-only corpus baseline では、same-lineage floor `4`、capability floor `1`、missing-option structure floor `3` まで source-backed な regression baseline が見え始めているため、heavy workstream 前の checker-boundary整理は次段 mainline に戻してよい。
