...
- したがって current next pressure は low-level memory-order の即時導入ではなく、concrete theorem consumer bridge に必要な minimum contract rows をどこまで docs-only で書くかを比較することである。
