...
- したがって current next pressure は low-level memory-order の即時導入ではなく、proof-obligation matrix を docs 正本に置いたうえで、external handoff artifact を mixed row bundle にするか boundary-specific artifact に割るかをどこまで narrow に比較するかである。
