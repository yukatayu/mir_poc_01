...
- したがって current next pressure は low-level memory-order の即時導入ではなく、concrete notebook workflow pressure と `proof_assistant_adapter` consumer pressure のどちらを先に narrow に扱うかを比較することである。
