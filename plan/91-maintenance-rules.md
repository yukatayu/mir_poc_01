...
- sample / script / helper taxonomy が変わる task では、`samples/README.md` と `scripts/README.md` の更新要否を同じ task で確認する。
- active / planned / prototype / archive / generated の分類が変わった task では、`samples_progress.md` と `plan/19-repository-map-and-taxonomy.md` の更新要否も確認する。
- high-risk な path move をしない task でも、docs-first layer map と staged migration plan が変われば report に残す。
...
- `tasks.md` の **「次に自走で進める順番と rough estimate」** には、各 task package がどの大局 phase の前半 / 中盤 / reserve path かを短く添える。
- `tasks.md` では、long chain を毎回 exhaustively 再列挙せず、**current checkpoint / current promoted line / next reopen point** が読める粒度に圧縮してよい。
- blocker 項目では、少なくとも
  - 概要
  - 影響範囲
  - 主要な選択肢
