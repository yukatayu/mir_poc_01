...
- current promoted next line は `R2` `AttachPoint` compatibility / detach minimal contract であり、`R1` で `VerificationLayer` widening threshold を docs-first に固定した後、helper-local `hotplug_lifecycle` / explicit detach TODO boundary を current evidence floor に保ったまま minimal contract row を narrow に整理する
...
- current next line は `R2` `AttachPoint` compatibility / detach minimal contract であり、helper-local `hotplug_lifecycle` / `hotplug_lifecycle_lanes` / explicit detach TODO boundary を current evidence floor に保ったまま、compatibility / activation_cut / detach_boundary / migration_contract の minimal contract row を docs-first に整理すると読む
