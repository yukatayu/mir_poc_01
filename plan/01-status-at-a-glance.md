...
- current promoted next line は `R5` runtime-crate hot-plug engine ownership cut であり、`R4` で kept-later boundary matrix を current memory に置いた後、helper-local preview / crate-side carrier / runtime orchestration の owner split を narrow に整理する
...
- current next line は `R5` runtime-crate hot-plug engine ownership cut であり、helper-local preview / crate-side carrier / runtime orchestration の owner split を docs-first に整理すると読む
