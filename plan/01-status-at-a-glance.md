...
- current promoted next line は implementation-side runtime-crate hot-plug engine actualization remains later であり、`R5` で owner split を current memory に置いた後の exact package decomposition は OPEN のまま残す
...
- post-`R5` current next line は implementation-side runtime-crate hot-plug engine actualization remains later と読み、exact package decomposition は OPEN のまま残す
