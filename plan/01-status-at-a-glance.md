...
- current promoted next line は `R4` hot-plug real migration / rollback boundary であり、`R2` で `AttachPoint` current minimal contract row を docs-first に固定し、`R3` で `FAIRY-05` carrier-choice matrix と provisional recommendation を current memory に置いた後、その先に残る kept-later hot-plug boundary を narrow に整理する
...
- current next line は `R4` hot-plug real migration / rollback boundary であり、current minimal contract row の先に残る real migration / rollback / runtime-crate hot-plug engine / distributed activation ordering の kept-later boundary を docs-first に整理すると読む
