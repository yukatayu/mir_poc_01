...
- repo-level current line は stable malformed missing-option first source-backed widening actualization に置き、その後に public operational CLI second gate と final public parser/checker/runtime thin-facade later support actualization を reserve に並べるのが current reading である。

## current mainline

1. stable malformed missing-option first source-backed widening actualization
2. reserve public operational CLI second later gate actualization comparison
3. reserve final public parser/checker/runtime thin-facade later support actualization
