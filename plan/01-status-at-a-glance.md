...
- repo-level current line は model-check concrete carrier actualization comparison に置き、その後に model-check concrete carrier first actualization、source-sample emitted verification artifact wiring、sample-facing theorem/model-check evidence summary and bless/review flow、later docs-first I/O / host-facing port boundary（working label）を並べるのが current reading である。
