...
- `P-A1-14` は closed であり、`VIS-A1-07` retention/on-demand trace は current exact carriers では widen しないと固定した。current exact reports にあるのは `retention_scope` label と `retained_later_refs` inventory までであり、exact retained-artifact catalog や on-demand retrieval request/result trace はまだ無い
...
- current practical devtools floor は `VIS-A1-01/02/03/04/05/06` に限られ、remaining reopen point は `VIS-A1-07` または broader save-load widening に縮小した
