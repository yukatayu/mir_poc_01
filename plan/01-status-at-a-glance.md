...
- `P19` は close 済みであり、current promoted next line は `P20` `mir-runtime` hot-plug orchestration skeleton first tranche である。completed engine、rollback、durable migration、distributed activation ordering、final public hot-plug ABI は still later に残す
...
- post-`P19` current next line は `P20` `mir-runtime` hot-plug orchestration skeleton first tranche と読む。completed engine、rollback、durable migration、distributed activation ordering、final public hot-plug ABI を扱う package-level reopen next はまだ昇格させない
