...
- current promoted next line は `P19` `mirrorea-core` hot-plug request/verdict carrier tranche であり、`R6` で first admissible family と queue split を current memory に置いた後も、actual hot-plug engine actualization は still later に残す
...
- post-`R6` current next line は `P19` `mirrorea-core` hot-plug request/verdict carrier tranche と読み、reopen next は `P20` `mir-runtime` hot-plug orchestration skeleton first tranche に残す
