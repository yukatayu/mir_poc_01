...
- current active package は `P11` logical multi-place runtime tranche であり、first cut は `MembershipRegistry` / `PlaceCatalog` substrate actualization である
...
- current next line は `P11` logical multi-place runtime tranche first cut、その reopen point は `P12` external adapter / host boundary tranche と読む
