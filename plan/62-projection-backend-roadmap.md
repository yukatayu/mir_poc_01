...
Current projection/backend state after `P-PROJ-04`:
...
- `samples/full-system-v1/server-client/` is actualized as a bounded same-binary local role-split floor with 1 accepted row and 1 undeclared-entry rejection row.
...
- `crates/mir-runtime::full_system_v1_local_split` now launches admitted target entries from one accepted projection manifest without widening target authority, and rejects undeclared entry overrides before runtime execution.
...
- `crates/mir-runtime::full_system_v1_projection`, `crates/mir-runtime::full_system_v1_local_split`, the `mir_full_system_v1_projection` and `mir_full_system_v1_local_split` examples, `cargo test -p mir-runtime --test projection_ir -- --nocapture`, and `cargo test -p mirrorea-cli --test full_system_v1_cli -- --nocapture` expose and validate the same bounded floor.
- deployment planner and LLVM/native backend remain later.
...
| `P-PROJ-04` | server/client local split | closed: same-binary local roles run from projection manifest with explicit undeclared-entry rejection and generated local-split inventory reports |
...
Next validation target: preserve the same projection/runtime/CLI anchor set while adding provider-admission rows on top of the now-actualized local role-split floor, and keep `python3 scripts/projection_boundary_samples.py check-all --format json` as the inventory comparison anchor.
