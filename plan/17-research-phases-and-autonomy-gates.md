...
3. **Phase 5 current package は checkpoint close として維持する**
   ...
   - `specs/examples/132...` により、public checker migration は concrete theorem consumer pressure が出たときだけ reopen する threshold に留め、`specs/examples/133...` で minimum contract row core、`specs/examples/134...` で `proof_notebook` first consumer、`specs/examples/135...` で `goal_text` attachment、`specs/examples/136...` で notebook bridge artifact を docs-only derived view に留める threshold、`specs/examples/137...` で next reopen order を notebook workflow first / `proof_assistant_adapter` second に固定し、`specs/examples/138...` で workflow pressure の first threshold を review checklist / walkthrough に置き、`specs/examples/139...` で review unit を docs-only named bundle に寄せ、`specs/examples/140...` で bridge-level の docs-only sketch に寄せ、`specs/examples/141...` で compare basis refs を、`specs/examples/142...` で bless decision state を、`specs/examples/143...` で review-note refs を、`specs/examples/144...` で retained-notebook ref を、`specs/examples/145...` で review-session ref を、`specs/examples/146...` で `reviewed_by_ref + reviewed_at_ref` を、`specs/examples/147...` で `review_session_state` を、`specs/examples/148...` で `retention_state` を、`specs/examples/149...` で `retained_path_policy_ref` を、`specs/examples/150...` で `emitted_artifact_ref` を、`specs/examples/151...` で `handoff_emitter_ref` を、`specs/examples/152...` で `consumer_adapter_ref` を、`specs/examples/153...` で `exchange_rule_ref` を、`specs/examples/154...` で `adapter_validation_ref` を、`specs/examples/155...` で `consumer_invocation_surface_ref` を、`specs/examples/156...` で `exchange_rule_body_ref` を、`specs/examples/157...` で `runtime_coupling_ref` を、`specs/examples/158...` で `transport_protocol_ref` を、`specs/examples/159...` で `failure_body_ref` を、`specs/examples/160...` で `actual_invocation_protocol_ref` を、`specs/examples/161...` で `consumer_host_binding_ref` を、`specs/examples/162...` で `failure_wording_ref` を、`specs/examples/163...` で `actual_runtime_handoff_ref` を、`specs/examples/164...` で `emitted_invocation_receipt_ref` を、`specs/examples/165...` で `runtime_transcript_ref` を、`specs/examples/166...` で `materialized_runtime_handoff_ref` を、`specs/examples/167...` で `concrete_payload_ref` を、`specs/examples/168...` で `concrete_transcript_body_ref` を、`specs/examples/169...` で `serialized_channel_body_ref` を、`specs/examples/170...` で `emitted_attachment_body_ref` を足したうえで、next practical reopen は actual emitted attachment blob / file materialization を theorem-line bridge にどこまで足すかの comparison に寄せる
