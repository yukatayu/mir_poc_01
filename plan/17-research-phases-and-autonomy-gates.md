...
| `Macro 5` | typed / theorem / model-check bridge | repo-local near-end completion closeout active | 重 | self-driven up to narrow mixed gate | final public contract / full type calculus / concrete production binding が必要になる |
...
- model-check second-line first concretization（initial package は close 済み、current reopen lane は still active）
