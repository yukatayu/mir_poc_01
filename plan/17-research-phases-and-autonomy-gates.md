...
- 現在地: `specs/examples/126...187` までで theorem-line later package close、**next promoted line は actual bless-side / update-side row split comparison**
- 重さ: とても重い
- autonomy gate:
...
1. **Phase 5 later reopen の current promoted line**
   - actual bless-side / update-side row split comparison
...
- **current promoted line は Phase 5 actual bless-side / update-side row split comparison** と読むのが自然である。
