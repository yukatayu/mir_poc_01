...
- **前半の authoritative baseline closeout 済み。catalog comparison の first cut として working subset row を切り、`auditable_authority_witness` の minimal witness core まで整理済み。current mainline は provider placement / causal stop line の refinement に移行**
...
2. **consistency / fairness / causal metadata catalog を working subset として比較する**
   ...
   - next narrow step は `delegated_rng_service` を authoritative room 側でも provider-placement candidate としてどこまで practical に読むかを比べる
