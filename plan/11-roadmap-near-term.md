...
- Phase 5 は `specs/examples/126...179` までで theorem-line later package close であり、**next promoted line は `retained archive payload comparison`** である。
...
### 1. retained archive payload comparison を Phase 5 later reopen の current promoted line として扱う

- `archive_bless_update_policy_ref` の次段として、retained archive payload をどこまで theorem-side retained bridge に近づけるかを比較する
- actual retained payload body / bless actualization は current core に上げず、threshold comparison に留める
- rough weight: 中〜重
...
|---|---|---|
| retained archive payload comparison | 1〜3 task | current promoted line |
