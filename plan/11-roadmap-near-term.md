...
|---|---|---|---|---|---|
| 1 | `105` parser companion inspector | `Macro 6/7` | representative parser companion sample の parse result を final parser API にせず repo-local inspector として inspectable にする | `M` | `p06 / p07 / p08` companion sample の parse result を JSON / CLI で読める repo-local command と test が揃う |
| 2 | `106` parser companion mapping matrix | `Macro 6/7` | original prototype / parser companion / guided bundle / Lean artifact / anchor spec-report の対応を drift なく readable にする | `S-M` | representative slice の mapping matrix が docs / helper / traceability の 3 点で同じ読みになる |
...
- current source:
  ...
  `specs/examples/578`
- current recommendation:
  `p06 / p07 / p08` companion sample の parse result を final parser/public API に上げず、repo-local JSON / pretty inspector command と focused test で inspectable にする。
- stop line:
  final public parser / checker / runtime API
  full `Program` lowering
...
- current source:
  ...
  `specs/examples/578`
- current recommendation:
  representative slice について original prototype / parser companion / guided bundle / Lean artifact / anchor spec-report の mapping matrix を docs / helper / traceability に actualize し、bundle helper だけに依存しない readable cut を作る。
- stop line:
...
- current self-driven queue は queue zero ではなく、Package 105 / 106 parser-side readable-inspection package と later mixed/user-spec residual を含む。
