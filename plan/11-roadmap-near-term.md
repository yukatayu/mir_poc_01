...
|---|---|---|---|---|---|
| 1 | `90` phase6-perform-head-final-public-parser-api comparison | `Macro 6/7` | Package 89 close 後の次段として、request clause suite bridge を保ったまま `perform` head をどこまで parser-side non-production carrier に上げるか | `M` | `perform` head line の current first compare floor を narrow に切り、request clause suite / diagnostics / final grammar へ premature に広げずに current first choice を置ける |
...
- current self-driven queue は queue zero ではなく、phase6-perform-head-final-public-parser-api comparison を含む。
