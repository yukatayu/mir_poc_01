...
|---|---|---|---|---|---|
| 1 | `73` verifier-handoff-surface ratchet | `Macro 5/7` | public-checker-boundary threshold の次段として verifier handoff surface ready sketch をどこまで helper-local summary に近づけるか | `M` | actual emitted verifier handoff artifact や dedicated consumer split を凍らせず、docs-only mixed-row bridge を narrow に actualize できる |
...
- current self-driven queue は queue zero ではなく、verifier-handoff-surface ratchet を含む。
