...
- current macro phase mainline は、`Macro 6` shared-space docs-first follow-up checkpoint を `specs/examples/375...376` までで一旦閉じたうえで、`specs/examples/377...382` の sequencing judgment / first actual carrier / emitted route actualization に従い、`Macro 5` sample-facing theorem/model-check evidence summary and bless/review flow を current line として詰める line である。
...
### 2. docs-first I/O / host-facing port boundary comparison（working label）
...
- sample-visible milestone の current package は
  ...
  の 2 本に分けるのが最小であり、前者までを `specs/examples/381...382` で actualize 済み、current line は後者である。
