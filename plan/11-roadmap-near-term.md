...
### 1. public operational surface actualization gate
...
- `Macro 6`
- Mirrorea/shared-space docs-first re-entry fixed 後の next docs-first reopen として、membership core と auth / admission / projection layering の cut を整理する。

### model-check concrete carrier first actualization gate

- `Macro 5`
- proof-notebook first pilot と public-checker docs-only reserve line を崩さずに、machine-facing actualization をどの model-check carrier から再開するかを整理する。

### stable static malformed broader follow-up

- `Macro 4`
- edge-pair fixed 後の broader malformed follow-up を missing-option / capability family、duplicate cluster、try/rollback malformed-static familyの順でどう reopen するかは later inventory に残す。
