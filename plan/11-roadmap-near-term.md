...
- Phase 5 は `specs/examples/126...185` までで theorem-line later package close であり、**next promoted line は `actual retained payload body materialization carrier detail comparison`** である。
...
### 1. actual retained payload body materialization carrier detail comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_payload_ref` の次段として、actual retained payload body materialization carrier detail をどこまで theorem-side retained bridge に近づけるかを比較する
- bless-side row / update-side row split と actual retained payload body materialization carrier payload は current core に上げず、threshold comparison に留める
- rough weight: 中〜重
...
|---|---|---|
| actual retained payload body materialization carrier detail comparison | 1〜3 task | current promoted line |
