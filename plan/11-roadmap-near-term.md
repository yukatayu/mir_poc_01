...
- current macro phase mainline は、`Macro 6` shared-space docs-first follow-up checkpoint を `specs/examples/375...376` までで一旦閉じたうえで、`specs/examples/377...378` の sequencing judgment に従い、`Macro 5` model-check concrete carrier first actualization を入口とする sample-visible theorem/model-check line（actual carrier + 2 package）を詰める line である。
...
### 4. docs-first I/O / host-facing port boundary comparison（working label）
...
- sample-visible milestone の current package は
  ...
  の 3 本に分けるのが最小であり、その sequencing は `specs/examples/377...378` で fixed 済みである。
