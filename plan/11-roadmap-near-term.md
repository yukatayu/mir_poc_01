...
- Phase 5 は `specs/examples/126...186` までで theorem-line later package close であり、**next promoted line は `actual retained payload body materialization carrier payload comparison`** である。
...
### 1. actual retained payload body materialization carrier payload comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_carrier_detail_ref` の次段として、actual retained payload body materialization carrier payload をどこまで theorem-side retained bridge に近づけるかを比較する
- bless-side row / update-side row split と actual retained payload body materialization carrier payload actualization は current core に上げず、threshold comparison に留める
- rough weight: 中〜重
...
|---|---|---|
| actual retained payload body materialization carrier payload comparison | 1〜3 task | current promoted line |
...
- current lineは inventory と theorem-side symbolic retained bridge comparison に留める
- actual external contract finalization はまだ後段である
...
- theorem-line retained bridge は symbolic threshold comparison として扱い、actual external contract とは区別する
