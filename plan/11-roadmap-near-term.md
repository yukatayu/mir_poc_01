...
- Phase 5 は `specs/examples/126...206` までで theorem-line later package close であり、**next promoted line は `theorem-export-checker-payload-pressure-ready actual-exported-checker-payload comparison`** である。
...
### 1. theorem-export-checker-payload-pressure-ready actual-exported-checker-payload comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_theorem_export_checker_payload_pressure` の次段として、actual exported checker payload をどこまで current line に近づけるかを比較する
- exported checker payload pressure marker 自体は current first choice に入ったまま、actual exported checker payload は still 後段に残す
- rough weight: 中〜重
...
|---|---|---|
| theorem-export-checker-payload-pressure-ready actual-exported-checker-payload comparison | 1〜3 task | current promoted line |
