...
|---|---|---|---|---|---|
| 1 | `110` representative problem bundle failure-focused smoke diagnostics | `Macro 6/7` | `smoke-all` failure 時に failed step と captured output を compact に追える diagnostics を足す | `S-M` | aggregate smoke が失敗しても compact summary を壊さず failing point を追える |
...
- current source:
  `specs/examples/583`
- current recommendation:
  `smoke-all` を compact summary のまま保ちつつ、failure 時だけ failed step と captured output を narrow に surfacing する diagnostics を追加する。
- stop line:
  exhaustive workflow automation
  aggregate CI / installed-binary contract
  final public CLI / tutorial surface
...
- current self-driven queue は queue zero ではなく、Package 110 representative problem bundle failure-focused smoke diagnostics package と later mixed/user-spec residual を含む。
