...
|---|---|---|---|---|---|
| 1 | `69` public-checker entry-criteria ratchet | `Macro 5/7` | public checker API sketch threshold の次段として public-checker comparison 専用の entry criteria をどこまで helper-local summary に近づけるか | `M` | actual command surface や parser-front public checker boundary を凍らせず、command-surface pressure present / heavier boundary deferred の current cut を整理できる |
...
- current self-driven queue は queue zero ではなく、public-checker entry-criteria ratchet を含む。
