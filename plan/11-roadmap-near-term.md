...
|---|---|---|---|---|---|
| 1 | `63` checker payload row-family ratchet | `Macro 5/7` | sample-local actual checker payload family threshold を checker payload row family へどこまで近づけるか | `M` | final public checker artifact や final public verifier contract を凍らせず、`p10 / p11 / p12` の helper-local preview を `payload_family_ref + row_family_kind` まで整理できる |
...
- current self-driven queue は queue zero ではなく、checker payload row-family ratchet を含む。
