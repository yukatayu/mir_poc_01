...
- Phase 5 は `specs/examples/126...187` までで theorem-line later package close であり、**next promoted line は `actual bless-side / update-side row split comparison`** である。
...
### 1. actual bless-side / update-side row split comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_carrier_payload_ref` の次段として、actual bless-side / update-side row split をどこまで theorem-side retained bridge に近づけるかを比較する
- bless/update split 自体を current core に上げず、threshold comparison に留める
- rough weight: 中〜重
...
|---|---|---|
| actual bless-side / update-side row split comparison | 1〜3 task | current promoted line |
