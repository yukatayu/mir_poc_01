...
|---|---|---|---|---|---|
| 1 | `65` checker payload row-body ratchet | `Macro 5/7` | sample-local checker payload row-detail threshold を checker payload row body へどこまで近づけるか | `M` | final public checker artifact や final public verifier contract を凍らせず、`p10 / p11 / p12` の helper-local preview を checker payload row body まで整理できる |
...
- current self-driven queue は queue zero ではなく、checker payload row-body ratchet を含む。
