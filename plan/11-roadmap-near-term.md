...
|---|---|---|---|---|---|
| 1 | `92` first strong typing finite-index layer | `Macro 5` | finite decidable index fragment を current sample / static carrier / docs に実装する | `L` | IFC / capture / lifetime / simple cost sample 群と docs default が drift なく揃う |
| 2 | `93` Lean-first formal skeleton hardening | `Macro 5` | actual small proof / generated stub / explanation の役割差を保ったまま proof placeholder 群を tighten する | `M` | Lean corpus と docs wording が Package 92/94 を受けられる状態で揃う |
| 3 | `94` theorem-first and model-check second-line carrier | `Macro 5/7` | notebook-first theorem line と row-local model-check carrier を current samples / artifacts / previews へ narrow に同期する | `M` | theorem/model-check bridge current floor と stop line が current corpus 上で一貫する |
| 4 | `95` order/handoff source surface and artifacts | `Macro 5/6` | explicit edge-row principal / stage-block secondary / negative corpus / artifact reading を tighten する | `M` | valid/invalid source surface family と helper artifact reading が drift なく読める |
| 5 | `96` authoritative-room first scenario | `Macro 6` | authoritative-room first default profile を current CLI / tests / artifacts / examples に tighten する | `M` | representative run と negative pair が drift なく読める |
...
- current source:
  ...
  `faq_010.md`
- current recommendation:
  current active line。finite decidable index fragment を checker-adjacent first layer として actualize し、IFC / taint、capture / lifetime、simple cost を current sample-visible static carrier に落とす。stronger typed surface は still non-principal に保つ。
- stop line:
...
- current self-driven queue は queue zero ではなく、Package 92...98 の once-through sequence を含む。
