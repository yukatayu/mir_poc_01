...
- Phase 5 は `specs/examples/126...202` までで theorem-line later package close であり、**next promoted line は `second-consumer-pressure-ready proof-assistant-adapter contract comparison`** である。
...
### 1. second-consumer-pressure-ready proof-assistant-adapter contract comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_external_contract_second_consumer_pressure` の次段として、actual `proof_assistant_adapter` contract をどこまで current line に近づけるかを比較する
- second consumer pressure 自体は current first choice に入ったまま、actual adapter-facing contract は still 後段に残す
- rough weight: 中〜重
...
|---|---|---|
| second-consumer-pressure-ready proof-assistant-adapter contract comparison | 1〜3 task | current promoted line |
