...
|---|---|---|---|---|---|
| 1 | `88` fixed-subset-source-sample-corpus-scope-and-file-layout ratchet | `Macro 6/7` | Package 87 close の次段として、repo-root `samples/current-l2/` flat `.txt` layer の scope / directory / naming / non-goal minimum をどこまで actualize するか | `M` | source corpus を第 3 層として actualize しつつ、mapping matrix / actual sample content / parser-to-`Program` lowering は retained-later に残せる |
...
- current source:
  ...
  `specs/examples/316`
- current recommendation:
  next active line。repo-root `samples/current-l2/` flat `.txt` layer を第 3 層 source corpus として保つ minimum を actualize し、initial cluster / directory / extension / naming / non-goal を narrow に固定する。
- stop line:
...
- current self-driven queue は queue zero ではなく、fixed-subset-source-sample-corpus-scope-and-file-layout ratchet を含む。
