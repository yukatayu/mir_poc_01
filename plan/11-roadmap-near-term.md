...
- Phase 5 は `specs/examples/126...201` までで theorem-line later package close であり、**next promoted line は `consumer-hint-ready second-consumer-pressure comparison`** である。
...
### 1. consumer-hint-ready second-consumer-pressure comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_external_contract_consumer_hint` の次段として、second consumer pressure をどこまで current line に近づけるかを比較する
- `consumer_hint` enrichment 自体は current first choice に入ったまま、second consumer pressure は still 後段に残す
- rough weight: 中〜重
...
|---|---|---|
| consumer-hint-ready second-consumer-pressure comparison | 1〜3 task | current promoted line |
