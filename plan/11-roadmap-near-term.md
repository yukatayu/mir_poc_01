...
|---|---|---|---|---|---|
| 1 | `86` phase6-parser-side-follow-up-package-sequencing ratchet | `Macro 6/7` | phase6 reserve formal tool binding inventory close の次段として shared single attachment frame を next parser-side package に置き、request clause suite / perform head / source-sample path をどう deferred reopen に押し戻すか | `M` | shared single attachment frame を next package に置き、request clause suite / perform head / richer diagnostics / final grammar / source-sample path を later に残せる |
...
- current source:
  ...
  `specs/examples/97`
- current recommendation:
  next active line。shared single attachment frame を next parser-side package に固定し、request clause suite / perform head / source-sample path を deferred reopen として helper-local manifest と snapshot docs に narrow に actualize する。
- stop line:
...
- current self-driven queue は queue zero ではなく、phase6-parser-side-follow-up-package-sequencing ratchet を含む。
