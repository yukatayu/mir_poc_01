...
|---|---|---|---|---|---|
| 1 | `66` checker payload supported-kind-summary ratchet | `Macro 5/7` | checker payload row-body threshold の次段として supported kind summary をどこまで helper-local summary に近づけるか | `M` | final public checker artifact や final public verifier contract を凍らせず、`payload_row_family_ref + supported_kind_scope + supported_kind_refs` の helper-local preview を整理できる |
...
- current self-driven queue は queue zero ではなく、checker payload supported-kind-summary ratchet を含む。
