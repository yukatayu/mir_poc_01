...
- current later reopen package は `specs/examples/146-current-l2-theorem-line-session-linked-retained-bridge-review-observation-threshold.md` と `specs/examples/147-current-l2-theorem-line-observed-session-lifecycle-threshold.md` までで切ってよく、session-linked retained bridge には `reviewed_by_ref + reviewed_at_ref` を、observed session-linked retained bridge には `review_session_state` を足す current first choice までを固定した。next later candidate は、retention state / actual retained path policy / emitted artifact threshold の comparison であり、`proof_assistant_adapter` pressure は still second practical candidate に残す
- rough weight: 重
