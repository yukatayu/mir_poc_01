...
- Phase 5 は `specs/examples/126...191` までで theorem-line later package close であり、**next promoted line は `actual bless-side row ref / update-side row strict dual-field comparison`** である。
...
### 1. actual bless-side row ref / update-side row strict dual-field comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_bless_update_dual_ref_bundle_ref` の次段として、actual bless-side row ref / update-side row strict dual-field shape をどこまで theorem-side retained bridge に近づけるかを比較する
- strict dual-field shape 自体を current core に上げず、threshold comparison に留める
- rough weight: 中〜重
...
|---|---|---|
| actual bless-side row ref / update-side row strict dual-field comparison | 1〜3 task | current promoted line |
