...
- current macro phase mainline は、まず `Macro 6` shared-space follow-up 2 package を進め、その後に `Macro 5` model-check concrete carrier actualization comparison を入口とする sample-visible theorem/model-check line（comparison + 3 package）を詰める line である。
...
- current near-term では、public operational surface inventory と later orderingを既存固定点として尊重しつつ、shared-space docs-first follow-up を先に 2 package 進め、その後に theorem/model-check side を **sample-visible milestone** まで進める。
