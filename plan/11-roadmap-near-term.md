...
|---|---|---|---|---|---|
| 1 | `64` checker payload row-detail ratchet | `Macro 5/7` | sample-local checker payload row-family threshold を checker payload row detail へどこまで近づけるか | `M` | final public checker artifact や final public verifier contract を凍らせず、`p10 / p11 / p12` の helper-local preview を `payload_row_family_ref + row_source_ref + row_reason_kind` まで整理できる |
...
- current self-driven queue は queue zero ではなく、checker payload row-detail ratchet を含む。
