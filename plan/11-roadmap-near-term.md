...
|---|---|---|---|---|---|
| 1 | `103` parser-side companion surface bundle | `Macro 6/7` | representative sample line を thin experimental surface / non-production parser-side carrier まで narrow に actualize する | `M` | representative companion text が parser-side carrier に入り、current bundle docs と drift なく結び付く |
| 2 | `104` parser-side bundle-to-helper bridge | `Macro 6/7` | parser-side carrier から guided helper / current sample reading への thin bridge を actualize する | `M` | representative parser-side slice が helper summary / guided bundle と一本道で辿れる |
...
- current source:
  ...
  `specs/examples/576`
- current recommendation:
  `p06` と `p07 / p08` representative slice を、final grammar へ上げずに thin experimental companion surface / non-production parser-side carrier まで narrow に actualize する。
- stop line:
  final grammar
  final public parser / checker / runtime surface
...
- current source:
  ...
  `specs/examples/576`
- current recommendation:
  parser-side carrier を current `bundle problem1 / problem2`、guided sample、helper summary へ thin bridge し、representative slice が docs / helper / parser-side carrier の 3 点で一本道になる minimum を actualize する。
- stop line:
...
- current self-driven queue は queue zero ではなく、Package 103 / 104 parser-side actualization と later mixed/user-spec residual を含む。
