...
- historical sync_v3 recovery は numbered self-driven queue を reopen しない。回収した論点は `plan/10` / `plan/12` / `plan/13` / `plan/18` の owner split に従って読む。
- reserve package summary index 4 本
  （theorem-first external pilot、`auditable_authority_witness`、`delegated_rng_service`、model-check second line）
  は actualize 済みである。reserve integration reopen / later mixed gate reopen / true user-spec hold line は later reopen bands として保持する。
- current defaults:
...
| `139 model-check-second-line reserve package summary index actualization` | `docs/reports/0894` + `specs/examples/612` | `emit-reserve model-check-second-line` と `target/current-l2-guided/reserve-packages/model-check-second-line/package-summary.md|json` を actualize し、`p06` representative bridge / `p10` positive carrier / `p11 / p12 / p15 / p16` rejection pair を reserve package 単独の repo-local summary index に圧縮した。reserve package summary index 4 本はこれで揃い、next reopen line は later mixed gate と true user-spec hold line に移った |

## active self-driven packages

- current numbered self-driven queue は closed である。
- next reopen bands は later mixed gate と true user-spec hold line に分けて読む。
