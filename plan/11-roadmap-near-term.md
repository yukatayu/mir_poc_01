...
|---|---|---|---|---|---|
| 1 | `89` phase6-request-clause-suite-publicization comparison | `Macro 6/7` | Package 88 close 後の次段として、request clause suite を multiline frame cut の上にどこまで publicize してよいか | `M` | request-local clause suite compare floor を narrow に切り、perform head / span-rich diagnostics / final grammar へ premature に広げずに current first choice を置ける |
...
- current self-driven queue は queue zero ではなく、phase6-request-clause-suite publicization comparison を含む。
