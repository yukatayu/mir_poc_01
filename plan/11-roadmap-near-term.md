...
|---|---|---|---|---|---|
| 1 | `112` representative problem quickstart CLI mirror | `Macro 6/7` | bundle doc に置いた 4 段 quickstart を `scripts/current_l2_guided_samples.py` からも problem ごとに読めるようにする | `S-M` | sample-side quickstart と CLI summary が同じ導線を返す |
...
- current source:
  `specs/examples/585`
- current recommendation:
  bundle doc 側の 4 段 quickstart を `scripts/current_l2_guided_samples.py` からも problem ごとに表示できるようにし、doc-only ではなく helper-side summary にも mirrored quickstart を持たせる。
- stop line:
  exhaustive tutorial surface
  final public CLI / tutorial surface
  final public parser / checker / runtime API
...
- current self-driven queue は queue zero ではなく、Package 112 representative problem quickstart CLI mirror package と later mixed/user-spec residual を含む。
