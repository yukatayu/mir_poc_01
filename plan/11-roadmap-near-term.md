...
|---|---|---|---|---|---|
| 1 | `94` theorem-first and model-check second-line carrier | `Macro 5/7` | notebook-first theorem line と row-local model-check carrier を current samples / artifacts / previews へ narrow に同期する | `M` | theorem/model-check bridge current floor と stop line が current corpus 上で一貫する |
| 2 | `95` order/handoff source surface and artifacts | `Macro 5/6` | explicit edge-row principal / stage-block secondary / negative corpus / artifact reading を tighten する | `M` | valid/invalid source surface family と helper artifact reading が drift なく読める |
| 3 | `96` authoritative-room first scenario | `Macro 6` | authoritative-room first default profile を representative run / CLI / artifact へ揃える | `M` | first room profile の practical meaning が sample / docs / helper で一致する |
| 4 | `97` reserve strengthening | `Macro 6/7` | first completion line を壊さずに reserve strengthening を narrow に切り分ける | `M` | `auditable_authority_witness` / `delegated_rng_service` / model-check second line が reserve lane として明示される |
| 5 | `98` documentation/report closeout | `Macro 0/5` | once-through sequence を docs / plan / progress / tasks / traceability に閉じる | `S` | queue drift が消え、remaining mixed gate / true user-spec residual だけが残る |
...
- current source:
  ...
  `specs/examples/532`
- current recommendation:
  current active line。notebook-first theorem line、Lean-first experimental binding、row-local model-check carrier、brand-neutral reserve を current strong typing / Lean / sample floor に narrow に再接続する。
- stop line:
  final public theorem/model-check contract
...
- current self-driven queue は queue zero ではなく、Package 94...98 の once-through sequence を含む。
