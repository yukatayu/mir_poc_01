...
|---|---|---|---|---|---|
| 1 | `109` representative problem bundle aggregate smoke summary | `Macro 6/7` | Problem 1 / Problem 2 の `smoke` 実行結果をまとめて compact に俯瞰できる repo-local summary helper を作る | `S-M` | representative 2 問題の smoke 成否と最小 step inventory を 1 コマンドで読める |
...
- current source:
  `specs/examples/582`
- current recommendation:
  `smoke problem1` / `smoke problem2` の上に、representative 2 問題の smoke 成否と step inventory を 1 コマンドで compact に読める repo-local summary helper を追加する。
- stop line:
  exhaustive workflow automation
...
- current self-driven queue は queue zero ではなく、Package 109 representative problem bundle aggregate smoke summary package と later mixed/user-spec residual を含む。
