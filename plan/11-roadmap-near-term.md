...
|---|---|---|---|---|---|
| 1 | `113` representative problem quickstart parity checks | `Macro 6/7` | sample bundle doc と `quickstart problem1|problem2` helper が同じ 4-step 導線を保っていることを focused test / helper で確認する | `S-M` | sample-side quickstart と CLI mirror の drift を早めに拾える |
...
- current source:
  ...
  `specs/examples/586`
- current recommendation:
  sample bundle doc と `quickstart problem1|problem2` helper が同じ 4-step 導線を保っていることを focused test / helper で確認し、doc-side と helper-side の quickstart drift を早めに拾えるようにする。
- stop line:
  exhaustive tutorial surface
  final public CLI / tutorial surface
  final public parser / checker / runtime API
...
- current self-driven queue は queue zero ではなく、Package 113 representative problem quickstart parity checks package と later mixed/user-spec residual を含む。
