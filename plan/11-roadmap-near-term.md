...
- current macro phase mainline は、`Macro 6` shared-space docs-first follow-up checkpoint を `specs/examples/375...376` までで一旦閉じたうえで、`specs/examples/377...380` の sequencing judgment と first actual carrier に従い、`Macro 5` source-sample emitted verification artifact wiring を入口とする sample-visible theorem/model-check line（emitted route + sample-facing summary）を詰める line である。
...
### 3. docs-first I/O / host-facing port boundary comparison（working label）
...
- current near-term では、public operational surface inventory と later orderingを既存固定点として尊重しつつ、shared-space docs-first follow-up の checkpoint close を保持したまま、その後に theorem/model-check side を **sample-visible milestone** まで進める。
- sample-visible milestone の current package は
  ...
  の 2 本に分けるのが最小であり、その前段の sequencing と first actual carrier は `specs/examples/377...380` で fixed / actualize 済みである。
