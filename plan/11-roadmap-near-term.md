...
- Phase 5 は `specs/examples/126...207` までで theorem-line later package close であり、**next promoted line は `theorem-export-checker-payload-ready checker-result-materialization-family comparison`** である。
...
### 1. theorem-export-checker-payload-ready checker-result-materialization-family comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_theorem_export_checker_payload` の次段として、checker result materialization family をどこまで current line に近づけるかを比較する
- actual exported checker payload 自体は current first choice に入ったまま、checker result materialization family は still 後段に残す
- rough weight: 中〜重
...
|---|---|---|
| theorem-export-checker-payload-ready checker-result-materialization-family comparison | 1〜3 task | current promoted line |
