...
|---|---|---|---|---|---|
| 1 | `107` explained representative problem sample bundles | `Macro 6/7` | 二大問題それぞれの representative sample を `samples/` 側で簡潔な日本語解説付き bundle として辿れるようにする | `S-M` | Problem 1 / Problem 2 の explained sample bundle が `samples/` 導線と helper / Lean / parser-side line を同じ読みに揃える |
...
- current source:
  ...
  `specs/examples/580`
- current recommendation:
  Problem 1 / Problem 2 の representative sample を `samples/` 側へ簡潔な日本語解説付き bundle として戻し、runner / Lean artifact / parser companion / guided helper の 4 本を bundle README から辿れるようにする。
- stop line:
...
- current self-driven queue は queue zero ではなく、Package 107 explained representative problem sample bundle package と later mixed/user-spec residual を含む。
