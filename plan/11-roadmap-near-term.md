...
- Phase 5 は `specs/examples/126...180` までで theorem-line later package close であり、**next promoted line は `archive retention layout comparison`** である。
...
### 1. archive retention layout comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_archive_payload_ref` の次段として、archive retention layout をどこまで theorem-side retained bridge に近づけるかを比較する
- actual retained payload body / bless-side row / update-side row split は current core に上げず、threshold comparison に留める
- rough weight: 中〜重
...
|---|---|---|
| archive retention layout comparison | 1〜3 task | current promoted line |
