...
|---|---|---|---|---|---|
| 1 | `91` phase6-perform-head-request-clause-bundle-attachment comparison | `Macro 3/5` | perform head structural carrier と request clause suite carrier をどの combined carrier で narrow に束ねるか | `M` | `Stage3RequestHeadClauseBundle` thin wrapper first を compare floor に置き、flatten/generic attachment を retained alternative に押し戻せる |
| 2 | `92` first strong typing finite-index layer | `Macro 5` | finite decidable index fragment を current sample / static carrier / docs に実装する | `L` | IFC / capture / lifetime / simple cost sample 群と docs default が drift なく揃う |
| 3 | `93` Lean-first formal skeleton hardening | `Macro 5` | actual small proof / generated stub / explanation の役割差を保ったまま proof placeholder 群を tighten する | `M` | Lean corpus と docs wording が Package 92/94 を受けられる状態で揃う |
| 4 | `94` theorem-first and model-check second-line carrier | `Macro 5/7` | notebook-first theorem line と row-local model-check carrier を current samples / artifacts / previews へ narrow に同期する | `M` | theorem/model-check bridge current floor と stop line が current corpus 上で一貫する |
| 5 | `95` order/handoff source surface and artifacts | `Macro 5/6` | explicit edge-row principal / stage-block secondary / negative corpus / artifact reading を tighten する | `M` | valid/invalid source surface family と helper artifact reading が drift なく読める |
| 6 | `96` authoritative-room first scenario | `Macro 6` | authoritative-room first default profile を representative run / CLI / artifact へ揃える | `M` | first room profile の practical meaning が sample / docs / helper で一致する |
| 7 | `97` reserve strengthening | `Macro 6/7` | first completion line を壊さずに reserve strengthening を narrow に切り分ける | `M` | `auditable_authority_witness` / `delegated_rng_service` / model-check second line が reserve lane として明示される |
| 8 | `98` documentation/report closeout | `Macro 0/5` | once-through sequence を docs / plan / progress / tasks / traceability に閉じる | `S` | queue drift が消え、remaining mixed gate / true user-spec residual だけが残る |
...
- current recommendation:
  next active line。perform head structural carrier と request clause suite carrier を current entry criteria に保ったまま、どの combined carrier で narrow に束ねるかを比較する。current first compare choice は `Stage3RequestHeadClauseBundle { perform_head, clause_suite, attachment_frame_kind }` thin wrapper である。
- current source:
  `specs/examples/562`
  `specs/examples/563`
  `specs/examples/564`
...
- current source:
  ...
  `faq_010.md`
- current recommendation:
  finite decidable index fragment を checker-adjacent first layer として actualize し、IFC / taint、capture / lifetime、simple cost を current sample-visible static carrier に落とす。stronger typed surface は still non-principal に保つ。
- stop line:
...
- current source:
  ...
  `samples/lean/foundations/CurrentL2ProofSkeleton.lean`
- current recommendation:
  actual small proof fragment と generated stub corpus の役割差を保ち、Package 92 の first layer と first theorem placeholder 群を drift なく日本語 explanation に同期する。
- stop line:
...
- current source:
  ...
  `specs/examples/532`
- current recommendation:
  notebook-first theorem line、Lean-first experimental binding、row-local model-check carrier、brand-neutral reserve を current strong typing / Lean / sample floor に narrow に再接続する。
- stop line:
  final public theorem/model-check contract
...
- current source:
  ...
  `specs/examples/533`
- current recommendation:
  explicit edge-row principal、stage-block secondary、serial sugar reserve を current artifact reading / negative corpus / helper surface に揃え、low-level exact source surface へは戻さない。
- stop line:
...
- current recommendation:
  authority-ack / single room authority / authoritative serial transition / authority_rng / late join visible past / stale reconnect fail-then-refresh / replay invalidation を first room default profile として representative run / CLI / artifact / docs に揃える。
- stop line:
...
- current source:
  ...
  `specs/examples/478`
- current recommendation:
  first completion line を壊さず、`auditable_authority_witness`、`delegated_rng_service`、model-check second-line concretization を reserve strengthening lane として narrow に切り分ける。
- stop line:
...
- current self-driven queue は queue zero ではなく、Package 91...98 の once-through sequence を含む。
