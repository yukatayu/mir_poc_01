...
|---|---|---|---|---|---|
| 1 | `108` representative problem bundle smoke runner | `Macro 6/7` | representative sample bundle guide に書いた主要 command 群を repo-local helper でまとめて smoke 実行できるようにする | `S-M` | Problem 1 / Problem 2 の representative smoke helper が runner / bundle / mapping / parser-side inspection の最小導線を再現する |
...
- current source:
  ...
  `specs/examples/581`
- current recommendation:
  Problem 1 / Problem 2 の representative sample bundle guide に書いた主要 command 群を repo-local helper でまとめて smoke 実行し、guide / helper / runnable evidence の 3 点が drift していないことを 1 コマンドで見られるようにする。
- stop line:
...
- current self-driven queue は queue zero ではなく、Package 108 representative problem bundle smoke runner package と later mixed/user-spec residual を含む。
