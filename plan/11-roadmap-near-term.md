...
- current later reopen package は `specs/examples/148-current-l2-theorem-line-lifecycle-ready-retention-state-threshold.md`、`specs/examples/149-current-l2-theorem-line-retention-ready-path-policy-threshold.md`、`specs/examples/150-current-l2-theorem-line-path-ready-emitted-artifact-threshold.md`、`specs/examples/151-current-l2-theorem-line-emitted-ready-handoff-emitter-threshold.md` までで切ってよく、lifecycle-ready retained bridge には `retention_state` を、retention-ready retained bridge には `retained_path_policy_ref` を、path-ready retained bridge には `emitted_artifact_ref` を、emitted-ready retained bridge には `handoff_emitter_ref` を足す current first choice までを固定した。next later candidate は、consumer adapter threshold の comparison であり、`proof_assistant_adapter` pressure は still second practical candidate に残す
- rough weight: 重
