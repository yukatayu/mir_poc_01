...
- current later reopen package は `specs/examples/148-current-l2-theorem-line-lifecycle-ready-retention-state-threshold.md` と `specs/examples/149-current-l2-theorem-line-retention-ready-path-policy-threshold.md` までで切ってよく、lifecycle-ready retained bridge には `retention_state` を、retention-ready retained bridge には `retained_path_policy_ref` を足す current first choice までを固定した。next later candidate は、actual emitted notebook artifact threshold の comparison であり、`proof_assistant_adapter` pressure は still second practical candidate に残す
- rough weight: 重
