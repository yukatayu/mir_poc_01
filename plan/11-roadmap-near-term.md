...
- Phase 5 は `specs/examples/126...205` までで theorem-line later package close であり、**next promoted line は `theorem-export-checker-contract-ready exported-checker-payload-pressure comparison`** である。
...
### 1. theorem-export-checker-contract-ready exported-checker-payload-pressure comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_theorem_export_checker_contract` の次段として、exported checker payload pressure をどこまで current line に近づけるかを比較する
- actual checker-facing contract 自体は current first choice に入ったまま、exported checker payload は still 後段に残す
- rough weight: 中〜重
...
|---|---|---|
| theorem-export-checker-contract-ready exported-checker-payload-pressure comparison | 1〜3 task | current promoted line |
