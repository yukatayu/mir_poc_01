...
|---|---|---|---|---|---|
| 1 | `101` theorem-first pilot bundle | `Macro 5/7` | theorem-first external integration target を repo-local emitted artifact / Lean sample corpus / guided helper の one-bundle reading へ寄せる | `M` | Problem 1 sample bundle から theorem-first pilot artifact 群まで一本道で辿れる |
| 2 | `102` authoritative-room scenario bundle | `Macro 6/7` | authoritative-room first scenario を representative pair / reserve route / negative pair / artifact refs を含む one-bundle reading へ寄せる | `M` | Problem 2 sample bundle から first line / reserve / negative / artifact route まで一本道で辿れる |
...
- current source:
  ...
  `specs/examples/573`
- current recommendation:
  guided sample `problem1`、`matrix problem1`、`samples/lean/current-l2/`、repo-local emitted theorem artifact refs を 1 本の repo-local theorem-first pilot bundle として再編する。
- stop line:
...
- current source:
  ...
  `specs/examples/574`
- current recommendation:
  guided sample `problem2`、`matrix problem2`、authoritative-room first scenario helper summary、reserve lane helper summary、negative static-stop pair を 1 本の repo-local scenario bundle として再編する。
- stop line:
  final public witness/provider/artifact contract
  exhaustive shared-space catalog
...
- current self-driven queue は queue zero ではなく、Package 101 / 102 bundle actualization と later mixed/user-spec residual を含む。
