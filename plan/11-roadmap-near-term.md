...
|---|---|---|---|---|---|
| 1 | `96` authoritative-room first scenario | `Macro 6` | authoritative-room first default profile を representative run / CLI / artifact へ揃える | `M` | first room profile の practical meaning が sample / docs / helper で一致する |
| 2 | `97` reserve strengthening | `Macro 6/7` | first completion line を壊さずに reserve strengthening を narrow に切り分ける | `M` | `auditable_authority_witness` / `delegated_rng_service` / model-check second line が reserve lane として明示される |
| 3 | `98` documentation/report closeout | `Macro 0/5` | once-through sequence を docs / plan / progress / tasks / traceability に閉じ、二大問題それぞれの簡潔な日本語解説付き sample を `samples/` に追加する | `S` | queue drift が消え、remaining mixed gate / true user-spec residual だけが残り、二大問題の current first line を sample でも追える |
...
- current self-driven queue は queue zero ではなく、Package 96...98 の once-through sequence を含む。
