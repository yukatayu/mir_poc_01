...
|---|---|---|---|---|---|
| 1 | `133` reserve integration entrypoint sync | `Macro 5/6/7` | reserve integration lane を current reopen order 付きで再構成する | `M` | reserve integration lane が final public contract 群と混ざらずに読める |
| 2 | `134` parser-side residual closeout sync | `Macro 5/7` | parser-side residual を repo-local once-through closeout line と分離して保つ | `M` | parser-side residual が mixed gate として独立に読める |
| 3 | `135` true user-spec residual freeze sync | `Macro 7/8` | true user-spec residual を explicit hold line に固定する | `S` | user-spec residual が reserve / mixed gate と分離して読める |
...
- current source:
  ...
  `specs/examples/571`
- current recommendation:
  theorem-first external pilot、`auditable_authority_witness`、`delegated_rng_service`、model-check second-line reserve を、closeout helper 後の next reopen order として再圧縮する。
- stop line:
  final public theorem contract
  final public witness/provider/artifact contract
  concrete theorem/model-check production binding
...
- current source:
  ...
  `specs/examples/520`
- current recommendation:
  packaging / FFI / engine adapter / exhaustive shared-space catalog / upper-layer application target を explicit hold line に固定し、self-driven queue と混ぜない。
- stop line:
...
- current self-driven queue は queue zero ではなく、Package 133...135 next closeout / reserve / user-spec freeze sync と later mixed/user-spec residual を含む。
