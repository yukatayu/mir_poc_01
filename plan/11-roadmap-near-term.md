...
|---|---|---|---|---|---|
| 1 | `74` minimal-parser-subset-freeze ratchet | `Macro 5/7` | verifier-handoff-surface threshold の次段として minimal parser subset freeze ready sketch をどこまで helper-local summary に近づけるか | `M` | accepted cluster / reject cluster / retention floor を narrow に actualize しつつ、final parser grammar や parser-to-checker reconnect freeze を still later に残せる |
...
- current self-driven queue は queue zero ではなく、minimal-parser-subset-freeze ratchet を含む。
