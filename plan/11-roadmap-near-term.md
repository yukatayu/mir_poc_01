...
- Phase 6 front-half の compile-ready minimal PoC では、`mir-ast` stage 1 / stage 2 carrier と `mir-semantics` / `mir-runtime` checker/runtime first trancheは actualize 済みである。残る主線は formal hook と checkpoint sweep である。

## いまから数 task の主眼

近い数 task の目的は、fixed 済みの Phase 1 / 2 / 3 / 4 / 5 entry criteria と parser + checker/runtime first tranche を前提に、Phase 6 front-half actualization を残り 1〜2 package で compile-ready checkpoint まで進めることである。
...
|---|---|---|
| compile-ready verification and formal hook | 1 task | cargo gate と tool-neutral or narrow tool first cut を揃える |
...
- current recommendation は **near-term Task 1 で tool-neutral first cut を優先して narrow に選ぶ** である
...
- proof / model-check line は Phase 5 closeout fixed と parser + checker/runtime first tranche を entry criteria に、near-term Task 1 で narrow に actualize する
