...
|---|---|---|---|---|---|
| 1 | `68` checker payload public-checker-api sketch ratchet | `Macro 5/7` | checker payload public-schema sketch threshold の次段として public checker API sketch をどこまで helper-local summary に近づけるか | `M` | actual command surface や final public verifier contract を凍らせず、`checker_subject + public_checker_payload_schema_ref` の helper-local preview を整理できる |
...
- current self-driven queue は queue zero ではなく、checker payload public-checker-api sketch ratchet を含む。
