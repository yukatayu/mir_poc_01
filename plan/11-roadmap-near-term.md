...
|---|---|---|---|---|---|
| 1 | `84` phase6-parser-second-tranche-attached-slot-and-predicate-fragment-first-package ratchet | `Macro 6/7` | phase6 next-reopen sequencing close の次段として attached-slot / predicate fragment first package を `mir_ast::current_l2` にどこまで narrow に actualize するか | `M` | stage3 declaration-side admit attached slot と shared isolated predicate fragment だけを first package に取り、attachment frame / request suite / perform head / formal binding は reserve/later line に残せる |
...
- current source:
  ...
  `specs/examples/555`
- current recommendation:
  next active line。stage3 declaration-side admit attached slot と shared isolated predicate fragment だけを `mir_ast::current_l2` へ actualize し、shared single attachment frame / request clause suite / perform head / richer diagnostics / formal binding は still later に残す。
- stop line:
...
- current self-driven queue は queue zero ではなく、phase6-parser-second-tranche-attached-slot-and-predicate-fragment-first-package ratchet を含む。
