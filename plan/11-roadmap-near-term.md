...
- Phase 5 は `specs/examples/126...189` までで theorem-line later package close であり、**next promoted line は `actual bless-side row / update-side row ref family comparison`** である。
...
### 1. actual bless-side row / update-side row ref family comparison を Phase 5 later reopen の current promoted line として扱う

- `retained_payload_body_materialization_bless_update_row_pair_ref` の次段として、actual bless-side row ref / update-side row ref family をどこまで theorem-side retained bridge に近づけるかを比較する
- bless/update row ref family 自体を current core に上げず、threshold comparison に留める
- rough weight: 中〜重
...
|---|---|---|
| actual bless-side row / update-side row ref family comparison | 1〜3 task | current promoted line |
