...
### 1. minimal-checker-payload-row-family-ready checker-payload-row-detail comparison を Phase 5 の current promoted line として扱う
...
- そのうえで、first checker cut の 6 cluster を docs-only checker-cluster matrix として見せる line は `coverage_state` で止め、actual checker payload family を `payload_family_kind + source_refs`、checker payload row family を `payload_family_ref + row_family_kind` minimal bundle まで切った current package を前提に、checker payload row detail をどこまで docs-first に比較すべきかを narrow に整理する
- rough weight: 中
...
|---|---|---|
| minimal-checker-payload-row-family-ready checker-payload-row-detail comparison | 1〜3 task | current promoted line |
