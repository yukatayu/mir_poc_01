...
- ただし、source text sample を fixed subset で持ち、`static gate` / `interpreter` / `formal hook` を sample 単位で通す経路は policy と theorem-first first pilot まで fixed した一方、bridge sketch と wider authored row 側は still staged である。current authored quintet `e1` / `e2` / `e21` / `e4` / `e23` には reached-stage row と repo-local policy helper があり、remaining authored-row widen line は `e3` guard comparison に置く。

## いまから数 task の主眼

近い数 task の目的は、fixed 済みの Phase 1 / 2 / 3 / 4 / 5 entry criteria と parser + checker/runtime + formal-hook checkpoint close を前提に、

1. third-slot guard line `e3` へ進む
2. theorem-side plain bridge sketch actualization を authored-row line の後に置く
3. compare-ready bridge sketch second reopen をその次段に置く

ことである。

## 次に自走で進める順番

### 1. third widened row guard comparison (`e3`)

- `e3` widening を current theorem-side / formal-hook family のどこで guard するかを narrow に比較する
- rough weight: 中
- rough 所要: 1〜2 task / 数日

### 2. plain bridge sketch actualization

- review-unit current cut を維持したまま plain docs-only bridge sketch を narrow actual package に落とす
- rough weight: 中
- rough 所要: 1〜2 task / 数日

### 3. compare-ready bridge sketch second reopen
...
|---|---|---|
| authored-row widen actualization + theorem-side plain bridge sketch actualization | 1〜3 task | widen sequencing と bridge-sketch reopen ordering は fixed 済みであり、remaining `e3` line と theorem-side next reopen timing を narrow に接続する |
