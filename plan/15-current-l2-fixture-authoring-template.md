...
6. future typed carrier 候補 row を narrow に見たいときは `suggest-reason-codes` を使い、helper-local / reference-only `detached_noncore.reason_codes` を display-only で確認する
7. 必要なら既存 artifact と `checker_core` を比較する
8. helper は fixture JSON を更新しないので、採用する場合は hand-edit して `run_bundle()` と detached static gate compare の両方で確認する
9. runtime artifact も参考に見たいときだけ `emit-fixture` を併用する
10. 1 command でまとめたいときは `scripts/current_l2_detached_loop.py smoke-static-gate` を non-production convenience として使ってよい
