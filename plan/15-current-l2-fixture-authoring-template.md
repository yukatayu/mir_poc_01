...
7. current tranche で fixture schema へ上げてよいのは stable inventory 8 kind に限り、duplicate declaration cluster は reference-only suggestion / actual wording のままにする
8. 必要なら既存 artifact と `checker_core` を比較する
9. helper は fixture JSON を更新しないので、採用する場合は hand-edit して `run_bundle()` と detached static gate compare の両方で確認する
10. runtime artifact も参考に見たいときだけ `emit-fixture` を併用する
11. 1 command でまとめたいときは `scripts/current_l2_detached_loop.py smoke-static-gate` を non-production convenience として使ってよい
11. directory-level の差を見たいときは `emit-aggregate` のあと `compare-aggregates` で `summary_core` を比較する
