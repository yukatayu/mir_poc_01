...
6. 必要なら既存 artifact と `checker_core` を比較する
7. helper は fixture JSON を更新しないので、採用する場合は hand-edit して `run_bundle()` と detached static gate compare の両方で確認する
8. runtime artifact も参考に見たいときだけ `emit-fixture` を併用する
9. 1 command でまとめたいときは `scripts/current_l2_detached_loop.py smoke-static-gate` を non-production convenience として使ってよい
10. directory-level の差を見たいときは `emit-aggregate` のあと `compare-aggregates` で `summary_core` を比較する
