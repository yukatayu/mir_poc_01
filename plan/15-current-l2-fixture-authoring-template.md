...
4. detached validation loop に入れるなら `emit-fixture` で artifact を保存する
5. 必要なら既存 artifact と payload core を比較する
...
4. selection / profile の妥当性は batch / profiled run 側で確認する
5. profile alias の意味が変わるなら docs / tests / code mirror を同 task で更新する
...
6. batch / profile の論点があるときだけ別に `run_directory_profiled` / named profile 側を見る
7. fixture を追加してまた回す
