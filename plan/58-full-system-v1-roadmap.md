...
Current repo state after `P-FSV1-99`:
...
- Full System V1 source-first typed checker floor is `actualized` with crate-local typed IR lowering, explicit accepted/residual obligations, imported-module semantic closure, ambiguous import rejection, and a 3-positive / 18-negative sample matrix through `scripts/full_system_v1_samples.py`. Its private host-adapter policy binds the existing exact `read_int@host_input` / `write_int@host_output` pairs to their signature and minimum capability, requires an ambient transition row, rejects duplicate record construction fields, and admits equality only for the bounded scalar floor. This is LAB implementation evidence, not a public effect ABI or trusted runtime authorization mechanism.
...
- `P-FSV1-99` has closed the current autonomous chain with final validation, docs/report cleanup, and claim/non-claim audit.
...
2026-07-04 maintenance note: `scripts/provider_admission_samples.py`,
`scripts/renderer_pose_backend_samples.py`, and the representative Full System
V1 CLI commands planned by `scripts/full_system_v1_release_check.py` now pass
in-repo CLI inputs as repo-relative `samples/...` argv. The renderer-pose
generated nested `provider-admission-report.json` files also store
repo-relative path fields rather than host absolute paths, and the
release-check reports / bundle / viewer display output-root paths relatively.
This is portability hardening for existing evidence, not a new runtime status,
ABI, execution-scope, or final-product claim.
...
The individual roots remain evidence-closed rather than workflow-ready, while the line-level bounded release-check workflow is actualized and the current autonomous chain is closed through `P-FSV1-99`.
