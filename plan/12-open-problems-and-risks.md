...
### 5. malformed duplicate-cluster source-authored static-stop pair actualization cut

- duplicate-cluster source-sample widening comparison は fixed 済みであり、current execution line は `e14/e15` source-authored static-stop pair actualization comparison である。
- current reading:
  **次の open problem は、duplicate pair widening の exact actualization cut と non-promotion guard をどこまで narrow に取るかである。**
