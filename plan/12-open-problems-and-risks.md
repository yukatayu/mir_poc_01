...
- 引き続き OPEN なのは、actual boundary-specific handoff pair shape をどの field で切るか、symbolic handoff pair ref を retained bridge のまま維持するか boundary-specific artifact row へ actualize するか、handoff pair shape を actual external contract へ actualize する concrete pressure を何とみなすか、`proof_assistant_adapter` consumer pressure を second practical candidate のまま維持する条件がいつ崩れるかである。
