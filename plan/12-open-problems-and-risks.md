...
- 引き続き OPEN なのは、checker verdict payload family をどの field / row / payload family で切るか、`retained_payload_body_materialization_theorem_export_checker_verdict_carrier_detail` を retained bridge のまま維持するか checker verdict payload family へ actualize するか、checker verdict witness / transport line をどの concrete threshold で呼ぶかである。
