...
- 引き続き OPEN なのは、checker verdict carrier detail をどの field / row / payload family で切るか、`theorem_export_checker_result_payload` を retained bridge のまま維持するか checker verdict carrier detail へ actualize するか、checker verdict witness / transport line をどの concrete threshold で呼ぶかである。
