...
### 1. typed-surface family unification keep/drop

- request / predicate / `try` cluster typed-surface reserve note は fixed 済みである。
- current reading:
  **次の open problem は、capability / lineage / request / predicate / `try` cluster をどこまで shared family として読むかである。**

### 2. notebook-consumer threshold and discharge reserve

- theorem pilot order、lemma wording floor、proof artifact / bridge stop line、admissible evidence contractionは fixed 済みである。
- current reading:
  **次の open problem は、notebook-first consumer threshold と discharge reserve をどこで切るかである。**

### 3. model-check small-cluster projection keep/drop

- projection / property-family inventory、bridge grain、sample-facing summary wording、tool-binding stop lineは fixed 済みである。
- current reading:
  **次の open problem は、small-cluster projection の keep/drop をどこまで narrowing するかである。**

### 4. order / handoff source-surface wording reserve

- cut / order / witness decomposition、negative falsifier coverage、candidate reduction、property-language bridge、emitted-artifact schema reserve は fixed 済みである。
- current reading:
  **次の open problem は、final source-surface handoff syntax を fixed せずに wording reserve をどこまで整理するかである。**

### 5. modality internalization trigger
