...
- live な first-line integration package はもう不要であり、`M1/M2/M3` threshold package、theorem contract shape threshold、theorem coupled later gate、theorem review-unit actual adoption、theorem public-seam compression、model-check public-seam compression、order-handoff-witness-provider final public seam compression、representative Lean sample set actual Lean execution も close 済みである。`specs/examples/520`、`521`、`522`、`523` により、current remaining self-driven work は actual Lean execution helper/CLI hardening だけではなく、layered strong typing / IFC first-fragment、source-side explicit authority / label-flow corpus、later mixed gate tracking に整理され、Lean formal skeleton / proof obligations first slice と IFC secret valid/invalid concrete example、source-side authority pair は committed `samples/lean/` corpus と source-side prototype corpus として fixed 済みである。remaining work 全体は later mixed gate / user-spec residual に分かれる。
