...
- 引き続き OPEN なのは、actual checker result payload をどの field / row / payload family で切るか、`theorem_export_checker_result_payload` family marker を retained bridge のまま維持するか actual checker result payload へ actualize するか、checker verdict carrier detail をどの concrete threshold で呼ぶかである。
