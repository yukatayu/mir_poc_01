...
| dynamic membership / causal metadata | shared space / fabric | OPEN / FUTURE | participant churn を plain vector clock deletion だけで扱うと membership change と causal history が混線しやすい。participant 集合を plain array の source of truth にすると、identity / activation / incarnation / ordering が混ざりやすい | shared-space / Mirrorea workstream 側で、membership reconfiguration と causal metadata を分離し、session-scoped membership registry + derived snapshot view を第一候補として比較する。activation rule は authoritative room では `authority-ack` を最小候補にし、full-coverage-like / quorum-like は policy option に残す。causal carrier は plain vector deletion を避け、epoch / incarnation split を first practical candidate、control-plane separated carrier を next stronger candidate として比較する |
| shared-space authority / resource ownership | shared space / fabric | OPEN / FUTURE | participant carrier、resource owner、delegated capability、consistency mode、fairness source を同じ carrier に潰すと、authoritative room と append-friendly room で必要な invariants が混線しやすい | shared-space workstream 側で、resource owner slot と delegated capability を分け、authoritative room / append-friendly room / relaxed room の相性を docs-first に比較する。authority placement は current phase では `single room authority` を最小候補にし、`replicated authority` と `relaxed projection authority` は comparison option に残す。consistency mode は `authoritative serial transition` と `append-friendly room` を first practical catalog にし、`relaxed merge-friendly room` は future comparison に残す。RNG / fairness source は `authority_rng` を最小候補にし、`delegated_rng_service` を next practical candidate、`distributed_randomness_provider` を future comparison に残す。authoritative game room の concrete profile は `authority-ack` + `single room authority` + `authoritative serial transition` + `authority_rng` を current minimal bundle に置き、RNG だけ `delegated_rng_service` へ差し替える形を next practical bundle に残す。reconnect / late leave / in-flight action は room profile に全部入れず、`member_incarnation` と uncommitted action invalidation だけを minimal room-profile rule とし、timeout / retry / resend は external policy layer に残す |
