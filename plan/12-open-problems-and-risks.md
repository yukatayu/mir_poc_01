...
### 1. request / predicate / `try` cluster typed-surface reserve

- checker attachment から handoff row への migration note は fixed 済みである。
- current reading:
  **次の open problem は、request / predicate / `try` cluster をどこまで同じ typed-surface reserve line に寄せるかである。**

### 2. admissible evidence contraction

- theorem pilot order、lemma wording floor、proof artifact / bridge stop lineは fixed 済みである。
- current reading:
  **次の open problem は、admissible evidence contraction をどこまで narrowing するかである。**

### 3. model-check tool-binding stop line

- projection / property-family inventory、bridge grain、sample-facing summary wording は fixed 済みである。
- current reading:
  **次の open problem は、concrete tool-binding stop line をどこまで explicit にするかである。**

### 4. order / handoff emitted-artifact schema reserve

- cut / order / witness decomposition、negative falsifier coverage、candidate reduction、property-language bridge は fixed 済みである。
- current reading:
  **次の open problem は、final property language を fixed せずに emitted-artifact schema reserve をどこまで整理するかである。**

### 5. modal stronger-candidate reduction timing

- `lambda-circle-box` partial basis、guarded / MDTT / MTT stronger family、promotion threshold note は fixed 済みである。
- current reading:
  **次の open problem は、guarded と MDTT/MTT cluster の reduction timing をどこで切るかである。**

### 6. shared-space fairness / replay strengthening reserve

- room-profile / host binding bridge-only note は fixed 済みである。
- current reading:
  **次の open problem は、fairness / replay strengthening reserve をどこまで docs-first に寄せるかである。**

### 7. public operational CLI packaging reserve

- current-L2 scoped Rust shell over thin facade は actualize 済みである。
- current reading:
  **次の open problem は、installed-binary / final packaging reserve をどこで切るかである。**
