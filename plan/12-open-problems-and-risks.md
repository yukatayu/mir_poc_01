...
### 1. stronger typed-surface promotion mixed gate

- request / predicate / `try` cluster typed-surface reserve note、typed-surface family split note、stronger typed-surface promotion threshold framing note は fixed 済みである。
- current reading:
  **次の open problem は、shared attachment shape と stronger typed surface promotion をどの mixed gate で actual adoption 判定へ送るかである。**

### 2. theorem discharge transport / public-contract concretization mixed gate

- theorem pilot order、lemma wording floor、proof artifact / bridge stop line、admissible evidence contraction、notebook-consumer threshold、theorem discharge transport / public-contract later-gate framingは fixed 済みである。
- current reading:
  **次の open problem は、abstract discharge-entry reserve の先で concrete discharge transport / public theorem contract をどの mixed gate で concretize するかである。**

### 3. model-check property-language / tool-binding concretization mixed gate

- projection / property-family inventory、bridge grain、sample-facing summary wording、tool-binding stop line、small-cluster projection keep/drop、property-language / tool-binding later-gate framingは fixed 済みである。
- current reading:
  **次の open problem は、first settled property language と concrete tool seam をどの mixed gate で concretize するかである。**

### 4. stronger modal foundation adoption mixed gate

- `lambda_circle_box` partial basis、guarded / MDTT / MTT stronger family、promotion threshold note、reduction timing note、modality internalization trigger note は fixed 済みである。
- current reading:
  **次の open problem は、place / authority / witness を stronger modality line へ internalize した後、どの mixed gate で foundation adoption 判定へ送るかである。**

### 5. shared-space final fairness / replay operational profile mixed gate

- room-profile / host binding bridge-only note、fairness / replay strengthening reserve note、shared-space fairness / replay mixed-gate boundary note は fixed 済みである。
- current reading:
...
### 6. public operational CLI installed packaging / success-criteria mixed gate

- current-L2 scoped Rust shell over thin facade、packaging reserve note、installed-binary / packaging success-criteria mixed-gate boundary note は fixed 済みである。
- current reading:
  **次の open problem は、installed-binary promotion と backend / tooling success criteria をどこで mixed gate concretization として扱うかである。**
