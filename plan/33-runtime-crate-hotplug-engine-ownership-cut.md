...
の owner split を repository memory として固定する。
...
follow-on reading in this memory:

- helper-local lifecycle / sample-grounded IDs / attach-detach view-telemetry IDs は preview ownership に残す
- reusable request / verdict carrier ownership は subsequent closeout memory では `P19` `mirrorea-core` closeout で actualize 済みである
- thin runtime/report assembly 上の orchestration skeleton は subsequent closeout memory では `P20` `mir-runtime` closeout で actualize 済みである
