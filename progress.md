...
最終更新: 2026-04-06（stage 1 parser spike entry criteria まで反映）
...
| parser / syntax finalization 準備 | 56% | 61% | 24% | 着手可能 | first parser cut に入れてよい semantic cluster の inventoryに加え、actual parser spike を切るなら chain / declaration structural floor、`TryFallback` / `AtomicCut` structural floor、request / admissibility cluster の checker-led staged order が source-backed に見えた。さらに stage 1 では declaration-side guard slot を opaque attached slot に留め、predicate fragment parse / option-local `admit` / request clause parse は later stage に残す cut まで docs-only で固定できた。ただし final grammar、exact lexical choice、predicate completion、actual parser API は未決 |
