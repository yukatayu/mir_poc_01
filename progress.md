...
| Phase 5 | static analysis / type / theorem prover / model checker boundary | third inventory package close | とても重い | 着手可能 | `specs/examples/126...` で 4-way split、`specs/examples/127...` で proof-obligation matrix と mixed handoff sketch、`specs/examples/128...` で mixed row default / split threshold / emitter defer まで固定。next は concrete consumer pressure が出たときだけ reopen |
...
- **inventory line**: Phase 5 third package close / later reopen candidate
...
2. Phase 5 current package は checkpoint close として維持し、mixed row default を保ったまま boundary-specific handoff artifact や actual emitter が本当に必要かを concrete pressure が出たときだけ reopen する
...
| static analysis / type / theorem prover / async-control boundary | 63% | 50% | 14% | 着手可能 | `specs/examples/126...` で 4-way split、`specs/examples/127...` で proof-obligation matrix と mixed handoff sketch、`specs/examples/128...` で mixed row default と boundary-specific split / actual emitter の reopen threshold まで current package close。next は concrete consumer pressure が出たときだけ reopen |
...
2. Phase 5 を reopen する場合は、mixed row default を維持したまま、どの concrete external consumer が boundary-specific handoff artifact や actual emitter を要求するかだけを narrow に選ぶ
