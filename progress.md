...
- **Mirrorea runtime / package line**
  `TermSignature`、`LayerSignature`、`MessageEnvelope`、`AuthEvidence`、`MembershipRegistry`、`PlaceCatalog`、`HotPlugRequest` / `HotPlugVerdict`、practical hot-plug / transport / save-load carriers、`practical_alpha05_session` session carrier に加えて、product alpha package kinds `world_core` / `membership_chat` / `sugoroku_world`、`membership-chat` bounded `EchoText` host boundary、`sugoroku-world` bounded roll/publish/witness/handoff/stale-reject scenario、schema-backed projection inventory summary、observer-safe operational devtools inventory、dependency-preserving native host launch bundle copy line、`samples/product-alpha1/operational/` suite がある
...
| `Macro 7` | toolchain / developer surface / public operational interface | α-0.5 / α-0.8 / α-0.9 operational line、bounded practical α-1 workflow、product alpha release-candidate workflow、operational product suite schema-backed projection inventoryまで到達。next は portal first cut / two-shard hard-boundary sample / package authoring guide | heavy | 着手可能 |
...
- operational suite では portal/world-link discrete handoff actualization、two-shard hard-boundary model-check、broader package authoring guide が未着手
