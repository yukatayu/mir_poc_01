...
| parser / syntax finalization 準備 | 48% | 54% | 20% | 着手可能 | first parser cut に入れてよい semantic cluster の inventory と、`TryFallback` / `AtomicCut` dedicated AST structural helper の entry criteria / malformed source placement / helper-local first tranche actualization までは見えたが、final grammar と exact lexical choice は未決 |
