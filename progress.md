...
| `P11` logical multi-place runtime tranche | 着手中 | `MembershipRegistry` / `PlaceCatalog` substrate、participant-place-kind-gated `LogicalPlaceRuntimeShell`、principal-derived `ParticipantPlace[{principal}]` shell-backed bootstrap / join / leave parity helper を actualize し、next は helper-local emulator の kept-later boundary を tighter にする |
...
- 2026-04-28 15:32 JST — `P11` logical multi-place runtime tranche の current third cut として principal-derived `ParticipantPlace[{principal}]` shell-backed `add_initial_participant` / `add_participant` / `leave_participant` parity helper を `mirrorea-core` に actualize した。review reports `0964` / `0965` の指摘を受けて helper parity を principal-derived に tightening し、新規 participant place registration の rollback により failure-atomic にした。`cargo test -p mirrorea-core`、`python3 -m unittest scripts.tests.test_sugoroku_world_samples`、`run 06_leave_non_owner`、`run 07_owner_leave_reassign`、Sugoroku closeout、`check_source_hierarchy.py`、`validate_docs.py`、`git diff --check` を fresh に通し、helper-side `membership_owner_reassignment` layer signature の `dice_owner:Bob->Carol` drift も `dice_owner:Bob->Alice` に修正した上で、report `0963`、`README.md`、`Documentation.md`、`progress.md`、`tasks.md`、`samples_progress.md`、`plan/01` / `plan/09` / `plan/16` / `plan/17` / `plan/19`、`specs/11`、`docs/hands_on/current_phase_closeout_01.md`、`docs/research_abstract/mirrorea_future_axis_01.md`、`docs/research_abstract/hands_on_sugoroku_05_join_leave.md`、`samples/clean-near-end/sugoroku-world/README.md` を current third-cut line に同期した。
