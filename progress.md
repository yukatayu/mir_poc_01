...
- next near-term path は、`stable malformed capability second reopen actualization comparison` を current line に置き、その後に `public operational CLI concrete shell naming` を reserve に置く。
...
| `Macro 4` | 89% | executable fixed-subset sample expansion | active | 自走可能 | current authored decet runnable。missing-option first source-backed widening は fixed 済みで、next malformed reserve は capability second reopen |
...
| `Macro 7` | 36% | toolchain / backend / developer surface | thin-facade support cut fixed | 一部自走可能 | public operational surface inventory、library-before-CLI later ordering、docs-first I/O / adapter boundary、runtime skeleton later support cut は fixed。next は CLI concrete shell naming |
...
1. **Macro 4 / stable malformed capability second reopen**
   - missing-option first source-backed wideningを巻き戻さず、capability family (`e13/e20`) の next reopen cut を整理する。
2. **Macro 7 / public operational CLI concrete shell naming**
...
- public operational CLI concrete command / flag / host-plan shell naming の最小 cut
