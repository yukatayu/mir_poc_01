...
- integration floor:
  `P0` current-state audit、`P1` repository layer map / `samples_progress.md` stabilization、`P2` typed external residual planned family review を close し、handoff 由来の queue numbering と source hierarchy 読み、typed external residual reopen matrix を current repo へ mirror した
...
2026-04-28 時点の repo は、**current-L2 / shared-space sample の runnable floor を維持したまま、Mirrorea future-axis の future-plan integration、next package queue stabilization、`P2` typed external residual planned family review を close し、次に `P3` projection emitted-program gate と `P4` signature hardening を整理する段階**です。
...
| `Macro 6` | shared-space / fabric boundary | `P3` promoted + `P4` reopen待ち | 92% | public contract / real transport 以外は自走可能 |
...
| typed external / projection / hot-plug / transport preview | `S5-S6` | typed external synthetic preview、typed external residual reopen matrix、projection preview、hot-plug lifecycle canary、`NET-01..05` helper-local canary | final host-facing contract、emitted program family、real migration / replay |
...
| `P3` projection emitted-program gate | next | preview floor と emitted program family を切り分ける | final IR / optimizer / equivalence checker を固定しない |
| `P4-P7` carrier hardening | queued; `P4` reopen next | signature / layer / envelope / visualization security を tighten する | helper-local inventory を public contract と誤読させない |
...
| `P3` Projection / placement residual emitted-program gate | 着手可能 | preview floor は actualize 済みで、emitted program / optimizer / equivalence line だけを mixed gate に残したまま切り分けを進められる |
| `P4-P7` carrier hardening | 着手可能 | `P4` は next reopen point であり、first cut 既存なので naming / law surface / stop line hardening を進められる |
