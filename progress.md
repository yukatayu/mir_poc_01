...
- current runnable prototype sample:
  ...
  （order/handoff family `p01 / p02 / p04 / p05 / p07 / p08`、dynamic attach/detach `p03`、typed/theorem/model-check `p06`）
...
| `Macro 4` | executable fixed-subset sample expansion | current scoped closeout fixed + prototype octet actualized | 99% | 重 | 着手可能 | current fixed-subset widening closeout まで self-driven |
...
| executable sample corpus | `S6` | authored sixteen、runner / ladder / regression helper、artifact fan-out、duplicate pair actualization、underdeclared source omission actualization、prototype runnable octet、rough-stimulus preservation bucket、helper-local `debug_outputs` / `verification_preview` / `artifact_preview` | broader try-rollback malformed-static family comparisonと mixed-gate reserve を保つ |
...
- 自走範囲:
  current self-driven queue は close 済みで、promoted self-driven package は `0 package` に戻った。以後の line は mixed gate / user specification に依存する。
...
- current promoted self-driven package は 0 本である。
...
- 2026-04-17 14:19 JST — `docs/reports/0725` で `p06-typed-proof-owner-handoff` を prototype sextet に追加し、typed marker (`admit` / `require` / `ensure`) が helper-local `verification_preview` / `artifact_preview` にどう映るかを sample-visible runtime prototype として固定した。
- 2026-04-17 14:26 JST — `docs/reports/0726` で `p07` late join visibility / `p08` stale reconnect refresh を prototype octet に追加し、order/handoff corrected prototype third tranche を close したうえで promoted self-driven queue を `0 package` に戻した。
