...
- **Phase 5** は `specs/examples/126...201` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory と theorem-side retained bridge は `retained_payload_body_materialization_external_contract_consumer_hint` まで current first choice を切れており、**next later reopen** は `consumer-hint-ready second-consumer-pressure comparison` に置いている。
...
| Phase 5 | static analysis / type / theorem prover / async-control boundary | theorem-line later package close | とても重い | 自走可能 | `126...201` まで current package close。next reopen は consumer-hint-ready second-consumer-pressure comparison |
...
1. **Phase 5 later reopen**
   - consumer-hint-ready second-consumer-pressure comparison を next promoted line に置く。
...
| static analysis / type / theorem prover / async-control boundary | 98% | 94% | 18% | 着手可能 | `126...201` まで current package close。next は consumer-hint-ready second-consumer-pressure comparison |
...
1. **Phase 5 later reopen** として `consumer-hint-ready second-consumer-pressure comparison` を扱う
