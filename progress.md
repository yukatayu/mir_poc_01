...
- **Phase 5** は `specs/examples/126...182` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory と symbolic retained bridge は `retained_archive_payload_body_family_ref` まで current first choice を切れており、**next later reopen** は `retained payload materialization family comparison` に置いている。
...
| Phase 5 | static analysis / type / theorem prover / async-control boundary | theorem-line later package close | とても重い | 自走可能 | `126...182` まで current package close。next reopen は retained payload materialization family |
...
1. **Phase 5 later reopen**
   - retained payload materialization family comparison を next promoted line に置く。
...
| static analysis / type / theorem prover / async-control boundary | 98% | 93% | 18% | 着手可能 | `126...182` まで current package close。next は retained payload materialization family |
...
1. **Phase 5 later reopen** として `retained payload materialization family comparison` を扱う
