...
- current main bottleneck は semantics の大崩れではなく、**sample ごとの reached stage を `static gate` / `interpreter` / `formal hook` でどう揃え、source-sample 更新フローへどう繋ぐか** である。mapping / lowering / runner first cut 自体は fixed 済み entry criteria として扱える。
...
| fixed-subset syntax-backed sample verification milestone | 79%前後 | ladder、policy | source text sample を `static gate` / `interpreter` / `formal hook` へ段階接続する |
...
| Phase 6 | 81% | actual parser / checker / runtime commitment と syntax-backed sample path | verification ladder wiring | 重い | 自走可能 | front-half compile-ready checkpoint、parser-side follow-up actualization、source corpus scope / layout、mapping、lowering、runner first cut は fixed 済みで、ladder / policy が残る |
| Phase 7 | 3% | higher-layer integration / domain realization | 未着手 | とても重い | 要仕様確認 | Mirrorea / Prism / 上位アプリは user specification が要る |

## 現在の主線

1. **Phase 6 verification ladder**
   - sample ごとの reached stage を `static gate` / `interpreter` / `formal hook` で揃える。
2. **Phase 6 source-sample authoring / bless policy**
...
| source-backed sample corpus / verification ladder | 78% | 80% | 69% | 着手可能 | representative prose、fixture corpus、source target path の matrix、helper-local lowering、helper-local runner first cut は fixed。ladder / policy が残る |
...
1. **Phase 6 verification ladder wiring** を immediate line として扱う
2. 続いて **source-sample authoring / bless policy** を固定する
3. その後に **theorem-first concrete tool pilot** を reserve reopen として比較する
4. 最後に checkpoint close 済み package の drift suppression を継続する
