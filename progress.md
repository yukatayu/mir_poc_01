...
- **Phase 5** は `specs/examples/126...179` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory と symbolic retained bridge は `archive_bless_update_policy_ref` まで current first choice を切れており、**next later reopen** は `retained archive payload comparison` に置いている。
...
| Phase 5 | static analysis / type / theorem prover / async-control boundary | theorem-line later package close | とても重い | 自走可能 | `126...179` まで current package close。next reopen は retained archive payload |
...
1. **Phase 5 later reopen**
   - retained archive payload comparison を next promoted line に置く。
...
| static analysis / type / theorem prover / async-control boundary | 97% | 90% | 18% | 着手可能 | `126...179` まで current package close。next は retained archive payload |
