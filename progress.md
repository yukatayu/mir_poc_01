...
- product alpha-1 release-candidate workflow と operational suite first cut に加えて installed-binary adoption probe と grammar / ABI scoping は入ったが、user-facing shipped surface の hardening depth はまだ未固定であり、WAN / distributed durable save-load はその後段に残る
- operational suite では product-side shipped-surface hardening が current queue にあり、broader multi-message room-chat widening と portal/shard starter revisit は later decision として残る
...
- WAN / distributed durable save-load / engine adapter scope は shipped-surface hardening 後の later gate のまま
...
- 2026-05-07 09:57 JST
  `P-OPS-18` で `specs/25` と installed-binary helper output に current hardening target を追加し、versioned `package.mir.json`、documented `mirrorea-alpha` command family、native host launch bundle replay surface を alpha-stable front door として整理した。next reopen point は shipped-surface hardening。
