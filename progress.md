...
- sample/progress/storage lane:
  `samples_progress.md` と storage guardrail script を追加し、phase-sample-progress-storage-foundation package を close した
...
| storage / external workdir guardrail | 54% | 62% | 41% | audit と safe scripts は入ったが、extra storage は未マウントで LLVM/backend path は未使用 |
...
| `Macro 7` | toolchain / backend / host integration | mixed | 59% | repo-local guardrail まで |
...
| storage / detachable workdir guardrail | `S2-S3` | audit、env script、detach prepare、cleanup script | actual mount cutover、cache relocation、LLVM probe |
