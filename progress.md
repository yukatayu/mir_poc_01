...
- product alpha-1 release-candidate workflow と operational suite first cut に加えて installed-binary adoption probe は入ったが、final public grammar / ABI はまだ未固定であり、WAN / distributed durable save-load はその後段に残る
- operational suite では product-side grammar / ABI scoping が current queue にあり、broader multi-message room-chat widening と portal/shard starter revisit は later decision として残る
...
- WAN / distributed durable save-load / engine adapter scope は grammar / ABI scoping 後の later gate のまま
