...
| Phase 4 | shared-space / membership / practical example boundary | 前半の authoritative baseline closeout 済み、catalog working subset と minimal witness core まで到達 | 重い | 着手可能 | baseline は `specs/examples/121...`、working subset は `specs/examples/122...`、minimal witness core は `specs/examples/123...`。current mainline は provider / causal stop line の refinement |
...
1. consistency / fairness / causal metadata catalog の current first cut と minimal witness core を前提に、`delegated_rng_service` を authoritative room 側でも provider-placement candidate としてどこまで practical に読むかを比較する
...
1. `delegated_rng_service` を authoritative room 側でも provider-placement candidate としてどこまで practical に読めるかを比べ、minimal witness core と working subset row の境界を保つ
2. static analysis / type / theorem prover / async-control boundary の small decidable core inventory を narrow に進める
