...
- **Phase 5** は `specs/examples/126...208` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory と theorem-side retained bridge は `retained_payload_body_materialization_theorem_export_checker_result_materialization_family` まで current first choice を切れており、**next later reopen** は `actual-exported-checker-payload-ready actual-checker-result-payload comparison` に置いている。
...
| Phase 5 | static analysis / type / theorem prover / async-control boundary | theorem-line later package close | とても重い | 自走可能 | `126...208` まで current package close。next reopen は actual-exported-checker-payload-ready actual-checker-result-payload comparison |
...
1. **Phase 5 later reopen**
   - actual-exported-checker-payload-ready actual-checker-result-payload comparison を next promoted line に置く。
...
| static analysis / type / theorem prover / async-control boundary | 98% | 95% | 18% | 着手可能 | `126...208` まで current package close。next は actual-exported-checker-payload-ready actual-checker-result-payload comparison |
...
1. **Phase 5 later reopen** として `actual-exported-checker-payload-ready actual-checker-result-payload comparison` を扱う
