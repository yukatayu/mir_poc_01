...
- **Phase 5** は `specs/examples/126...198` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory と theorem-side retained bridge は `retained_payload_body_materialization_external_contract` まで current first choice を切れており、**next later reopen** は `actual-external-contract-ready consumer-specific external contract payload comparison` に置いている。
...
| Phase 5 | static analysis / type / theorem prover / async-control boundary | theorem-line later package close | とても重い | 自走可能 | `126...198` まで current package close。next reopen は actual-external-contract-ready consumer-specific external contract payload |
...
1. **Phase 5 later reopen**
   - actual-external-contract-ready consumer-specific external contract payload comparison を next promoted line に置く。
...
| static analysis / type / theorem prover / async-control boundary | 98% | 93% | 18% | 着手可能 | `126...198` まで current package close。next は actual-external-contract-ready consumer-specific external contract payload |
...
1. **Phase 5 later reopen** として `actual-external-contract-ready consumer-specific external contract payload comparison` を扱う
