...
- current reading:
  repo は **theory / first-floor carriers / evidence closeout** に加えて **bounded operational α-0.5 local observable runtime** を得たが、same-session hot-plug/runtime-wide live devtools は未完成
- self-driven status:
  `P-A1-21` は自走可能。α-0.8 same-session hot-plug runtime は `P-A1-19` carrier と `P-A1-20` host-I/O lane を前提にする
...
| 実装 / 運用 | 69 | current-L2 active floor と practical alpha-1 first floors に加えて bounded operational α-0.5 session carrier と typed host-I/O direct execution lane は actualize したが、same-session hot-plug と live/session-bound full devtools は未実装 |
...
| operational α-0.5 | operational line | 100 | `P-A1-20` で `practical_alpha05_session` carrier 上に `crates/mir-runtime::practical_alpha05_host_io`、example `host-io` subcommand、`OA05-07` `AddOne` direct execution lane を接続し、bounded local observable runtime を actualize | α-0.8 same-session hot-plug runtime |
...
- **Observability / devtools**
  export-side first floors に加え、α-0.5 local session から event DAG / observer-safe summary / host-I/O receipt summary を引く lane はあるが、route trace / membership timeline / witness relation まで live/session 化したわけではない
...
| `Macro 6` | distributed fabric / shared-space / runtime evolution boundary | 現在の主戦場。`P-A1-21` same-session hot-plug runtime が次に入る | heavy | 着手可能 |
| `Macro 7` | toolchain / developer surface / public operational interface | α-0.5 session surface と minimal host-I/O lane は actualize 済み。next promoted line は `P-A1-21`、その後段に α-0.9 live export がある | heavy | 着手可能 |
...
| executable sample corpus | 94 | current-L2、practical alpha-1 first floors、bounded operational α-0.5 line は runnable。残る operational α gap は same-session hot-plug と live devtools | 着手可能 |

## current blockers

- same-session hot-plug runtime が未実装で、debug / auth / rate-limit / object / avatar-placeholder attach を α-0.5 session 上で live 観測できない
- devtools は α-0.5 local session summary と minimal host-I/O receipt までは live 化したが、route trace / membership timeline / witness relation / save-load timeline を live/session-bound に統合していない
