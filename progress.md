...
- stage 3:
  ...
  - next narrow step は、request-local `require:` / `ensure:` の fixed two-slot suite floor を helper-local / test-only structural compare にどこまで actualize するかを比較することである
...
2. stage 3 later branch の次段比較
   - request-local `require:` / `ensure:` の fixed two-slot suite floor を helper-local / test-only structural compare にどこまで actualize するかを比較する
