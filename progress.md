...
- **Phase 5** は `specs/examples/126...180` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory と symbolic retained bridge は `retained_archive_payload_ref` まで current first choice を切れており、**next later reopen** は `archive retention layout comparison` に置いている。
...
| Phase 5 | static analysis / type / theorem prover / async-control boundary | theorem-line later package close | とても重い | 自走可能 | `126...180` まで current package close。next reopen は archive retention layout |
...
1. **Phase 5 later reopen**
   - archive retention layout comparison を next promoted line に置く。
...
| static analysis / type / theorem prover / async-control boundary | 98% | 91% | 18% | 着手可能 | `126...180` まで current package close。next は archive retention layout |
...
1. **Phase 5 later reopen** として `archive retention layout comparison` を扱う
