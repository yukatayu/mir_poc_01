...
| Mirrorea future axis | 84% | 91% | 77% | helper-local / report-local preview に加え `mirrorea-core` minimal carrier ownership cut、`MembershipRegistry` / `PlaceCatalog` substrate、participant-place-kind-gated `LogicalPlaceRuntimeShell`、principal-derived `ParticipantPlace[{principal}]` shell-backed bootstrap / join / leave parity helper まで actualize 済み。public contract と emitted artifact family は後段 |
...
| `P14` hot-plug package-manager tranche | promoted next | helper/test/docs closeout hardening として `hotplug_lifecycle`、attach / detach canary、compatibility / activation / explicit TODO stop line を package-manager inventory surface に揃える | runtime crate 側の migration engine、rollback protocol、final public hot-plug ABI を premature に claim しない |
| `P15-P17` implementation tranche | later | projection / viewer / backend を段階的に widen する | `P14` helper-local package-manager closeout 前に emitted program / viewer / backend を collapse しない |
...
| `P11` logical multi-place runtime tranche | non-blocking follow-up later | current third cut は actualize 済み。helper-local emulator kept-later boundary の追加 tightening は repo-promoted line ではなく kept-later follow-up として残す |
...
| `P14` hot-plug package-manager tranche | 着手可能 | helper/test/docs closeout hardening として package-manager inventory surface を整理し、runtime migration / rollback claim を避けたまま queue を進められる |
| `P15-P17` implementation tranche | 後段依存 | `P14` helper-local package-manager closeout と hot-plug stop line の current line が先に要る |
