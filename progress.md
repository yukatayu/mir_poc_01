...
| operational product sample suite | operational product line | workflow-ready canonical suite, not final public product | `P-OPS-01` で `samples/product-alpha1/operational/`、`specs/26..27`、`plan/51..52`、`scripts/operational_product_samples.py` を追加し、`WorldCore -> MembershipChat -> SugorokuWorld` chain、shared attach packages、projection intent、portal/shard future inventory を bounded operational replay として同期した。`P-OPS-03` で `membership-chat` に bounded `EchoText` direct host boundary、`P-OPS-04` で `sugoroku-world` に bounded roll / publish / witness / handoff / stale membership reject runtime evidence と corresponding devtools evidence を追加した | projection packet/FFI schema、portal/world-link first cut |
...
| `Macro 7` | toolchain / developer surface / public operational interface | α-0.5 / α-0.8 / α-0.9 operational line、bounded practical α-1 workflow、product alpha release-candidate workflow、operational product suite Sugoroku bounded runtime evidenceまで到達。next は projection schema formalization / portal first cut / package authoring guide | heavy | 着手可能 |
...
- operational suite では projection packet/FFI schema、portal/world-link discrete handoff actualization、two-shard hard-boundary model-check が未着手
