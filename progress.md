...
- **Phase 6** は parser first tranche が `specs/examples/299...300`、checker/runtime first tranche が `specs/examples/301...302`、compile-ready verification / formal hook が `specs/examples/303...304` で fixed 済みであり、checkpoint drift suppression / mirror sweep も close 済みである。`specs/examples/305...306` により next reopen sequencing も fixed 済みであり、`specs/examples/307...308` により parser second tranche first package も attached-slot / predicate route として actualize 済みである。`specs/examples/309...310` により reserve formal tool binding inventory も fixed 済みであり、theorem-first concrete binding を first reserve、model-check side を second reserve に置いた。`mir-ast` には stage 1 / stage 2 carrier に加えて stage 3 declaration-side admit attached slot と shared isolated predicate fragment が入り、multiline attachment / request clause suite helper は support-local structural family に留めている。parser-side follow-up package、concrete tool actualization、final public crate surfaceは still later に残る。
- current main bottleneck は semantics の大崩れではなく、**Phase 6 parser-side follow-up package sequencing** をどこまで narrow に切るかである。
...
| Phase 6 | 74% | actual parser / checker / runtime commitment | parser-side follow-up package sequencing | 重い | 自走可能 | stage3 attached-slot / predicate route と formal reserve inventory まで fixed。次は shared single attachment frame の位置決め |
