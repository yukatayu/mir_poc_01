...
最終更新: 2026-04-06（try rollback AST helper の malformed static family timing まで整理）
...
- さらに current docs-only refinement として、`TryFallback` / `AtomicCut` を future dedicated AST structural helper に actualize するなら、parser/loader malformed source、AST-only floor、reason-row family と分ける dedicated carrier、runtime gate 非依存、複数の structural family、という entry criteria を先に満たす current judgment を固定した。
...
| Mir current L2 core semantics | 84% | 75% | 71% | 着手可能 | current task を回すには十分安定しており、`TryFallback` body 内 `AtomicCut` frontier と nested-place mismatch の runtime representative が actual corpus に入った。ただし final formalization はまだ先 |
...
| static analysis / type / theorem prover workstream | 82% | 76% | 71% | 着手可能 | first checker cut の local / structural floor に加え、static gate reasons をそのまま harness machine-check へ昇格できないことを確認し、additive optional `checked_reasons` を最小 dedicated carrier として実装した。typed code は stable cluster inventory と parametric shape 条件まで整理済みで、detached-side `reason_codes` は helper-local / reference-only mirror に留めつつ、display-only assist と readiness scan で corpus 横断の actual split を観察できる。さらに current stable cluster inventory 8 kind を覆う 9 fixture を optional `checked_reason_codes` に actualize し、coexistence scan を zero follow-up まで揃え、same-lineage / capability / missing-option checker cluster coverage を source-backed に確認し、same-lineage first checker spike、missing-option second checker spike、capability third checker spike、shared family checker support helper を固めた。そのうえで generic checker-side shared family entry は current phase では切らず、family facade 維持で止め、`TryFallback` / `AtomicCut` structural floor は fourth spike に premature actualize せず docs/runtime representative に留めた。さらに future dedicated AST structural helper の entry criteria、structural malformed source placement、runtime representative 優先の malformed-family non-actualization、helper-local compare contract まで docs-only で固定した。duplicate cluster、final public checker cut と shrink policy はまだ OPEN |
...
- **ある程度自律的に「追加し、回し、比較し、次へ進む」状態まで**: あと **2〜4** task 程度
- 想定する中身:
