...
- legacy Phase 6 front-half compile-ready checkpoint も fixed 済みであり、`mir-ast` / `mir-semantics` / `mir-runtime` の narrow actual path、tool-neutral formal hook、proof-notebook first pilot、plain bridge sketch first actualization までは source-backed である。
...
- tool-neutral formal hook から `proof_notebook_review_unit` を作る first theorem-side pilot はあるが、compare-ready metadata と concrete theorem / model-check binding はまだ later である。
...
| `Macro 5` | 34% | static reasoning / theorem / model-check bridge | early | 一部自走可能 | tool-neutral formal hook、review-unit pilot、plain bridge sketch はあるが concrete tool binding は later |
...
| contracts / static gate / formal hook / proof notebook first bridge | `S4-S5` | tool-neutral formal hook、review-unit pilot、plain bridge sketch first actualizationがある | compare-ready metadata、concrete theorem / model-check binding、public checker line | 自走可能 |
...
1. **Macro 5 / theorem-side compare-ready bridge sketch second reopen**
   - plain bridge sketch first actualization の次として、compare-ready minimum を narrow に比較する。
...
- compare-ready bridge sketch の minimum
