...
- product alpha-1 release-candidate workflow と operational suite first cut に加えて installed-binary adoption probe、grammar / ABI scoping、shipped-surface hardening は入ったが、それより広い installed distribution shape はまだ未固定であり、WAN / distributed durable save-load はその後段に残る
- operational suite では product-side broader public distribution narrowing が current queue にあり、broader multi-message room-chat widening と portal/shard starter revisit は later decision として残る
...
- WAN / distributed durable save-load / engine adapter scope は broader public distribution narrowing 後の later gate のまま
