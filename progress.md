...
| Phase 5 | static analysis / type / theorem prover / model checker boundary | fourth inventory package close | とても重い | 着手可能 | `specs/examples/126...` で 4-way split、`specs/examples/127...` で proof-obligation matrix と mixed handoff sketch、`specs/examples/128...` で mixed row default / split threshold / emitter defer、`specs/examples/129...` で theorem first / protocol second / runtime later の pressure order まで固定。next は theorem line の narrow actualization comparison |
...
- **inventory line**: Phase 5 fourth package close / later reopen candidate
...
2. Phase 5 current package は checkpoint close として維持し、theorem line を first practical candidate にしたうえで mixed row default を壊さない narrow actualization だけを later pressure が出たときに reopen する
...
| static analysis / type / theorem prover / async-control boundary | 67% | 54% | 14% | 着手可能 | `specs/examples/126...` で 4-way split、`specs/examples/127...` で proof-obligation matrix と mixed handoff sketch、`specs/examples/128...` で mixed row default と boundary-specific split / actual emitter の reopen threshold、`specs/examples/129...` で theorem first / protocol second / runtime later の pressure order まで current package close。next は theorem line の narrow actualization comparison |
...
2. Phase 5 を reopen する場合は、theorem line を first practical candidate に固定したまま、mixed row default を壊さずにどこまで theorem-side narrow actualization を切るかだけを narrow に選ぶ
