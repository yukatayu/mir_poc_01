...
- **Phase 5** は `specs/examples/126...187` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory と symbolic retained bridge は `retained_payload_body_materialization_carrier_payload_ref` まで current first choice を切れており、**next later reopen** は `actual bless-side / update-side row split comparison` に置いている。
...
| Phase 5 | static analysis / type / theorem prover / async-control boundary | theorem-line later package close | とても重い | 自走可能 | `126...187` まで current package close。next reopen は actual bless-side / update-side row split |
...
1. **Phase 5 later reopen**
   - actual bless-side / update-side row split comparison を next promoted line に置く。
...
| static analysis / type / theorem prover / async-control boundary | 98% | 93% | 18% | 着手可能 | `126...187` まで current package close。next は actual bless-side / update-side row split |
...
1. **Phase 5 later reopen** として `actual bless-side / update-side row split comparison` を扱う
