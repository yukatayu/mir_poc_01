...
- Mirrorea future lane:
  `TermSignature registry / debug output` を helper-local / report-local first cut として close し、next semantic carrier package を `LayerSignature system` と読む
...
| Mirrorea future axis | 61% | 71% | 24% | queue は維持したまま `TermSignature` first cut を通し、next は `LayerSignature` と `MessageEnvelope` |
...
| layer composition / auth / visualization / projection / hot-plug | `S2` | package queue、stop line、phase 8 representative skeleton family、`TermSignature` first cut と `--debug signatures` を actualize | LayerSignature、envelope、viewer、projection、AttachPoint actualization |
