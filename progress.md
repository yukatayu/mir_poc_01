...
- **Phase 2** は broad mainline では closeout 寄りに入っている。parser-free PoC、detached validation loop、fixture authoring の operational baseline は成立済みであり、残りは compile gate / retention policy / docs mirror closeout である。
...
- current main bottleneck は semantics の大崩れではなく、**Phase 2 / 4 / 5 closeout sweep**、**formal tool binding 前の proof / model-check handoff 整理**、**Phase 6 front-half compile path actualization** である。
...
| Phase 6 前半 compile-ready minimal actual PoC | 35% 前後 | 6 package 前後 / 3〜6週 | docs / test-only spike / parser-free harness / reconnect freeze と Phase 1 closeout はあるが public crates actualization はこれから |
...
2. その後に **Phase 4 / 5 closeout sweep** を順に進め、self-driven scope を phase-complete snapshot へ揃える
