...
- current promoted next line:
  `P13` network transport minimal alpha。current safest first cut は helper-local `process_boundary` closeout であり、`loopback_socket` parity、subprocess JSON bridge、stale epoch/incarnation reject、typed transport failure family、observer-safe redacted route trace を current floor として tighten する
- next reopen point:
  `P14` hot-plug package-manager tranche
- still later:
  `P14-P17` implementation tranche、`P18` public API / parser grammar gate
...
| `P13` network transport minimal alpha | promoted next | helper-local `process_boundary` closeout として `loopback_socket` parity、subprocess JSON bridge、stale epoch/incarnation reject、typed transport failure family、observer-safe redacted route trace を tighten する | real socket/broker、crypto session、durable replay/commit、continuous shared runtime state across process を premature に claim しない |
| `P14-P17` implementation tranche | later | hot-plug / projection / viewer / backend を段階的に widen する | `P13` helper-local process-boundary closeout 前に責務を collapse しない |
...
| `P13` network transport minimal alpha | 着手可能 | helper-local `process_boundary` closeout として transport canary floor を tighten し、real transport runtime / session / replay claim を避けたまま queue を進められる |
| `P14-P17` implementation tranche | 後段依存 | `P13` helper-local process-boundary closeout と transport stop line の current line が先に要る |
