...
- next near-term path は、source-sample emitted verification artifact wiring を入口とする sample-visible theorem/model-check line（emitted route + sample-facing summary）を進め、その次段で `docs-first I/O / host-facing port boundary`（working label）を整理する line に置く。
...
| `Macro 5` | 64% | static reasoning / theorem / model-check bridge | early-active | 一部自走可能 | proof notebook review-unit current cut、first actualization gate、actualization sequencing comparison、first actual carrier は fixed 済み。current line は emitted route + sample-facing summary の sample-visible line |
...
| theorem / verifier bridge | 87% | 82% | 61% | early-active | formal hook、proof notebook first bridge、first actual model-check carrier はある。nextは emitted route + sample-facing summary の sample-visible line |
...
1. **Macro 5 / source-sample emitted verification artifact wiring**
   - current authored sample octet と verification ladder を theorem/model-check side artifact route に狭く接続する。
2. **Macro 5 / sample-facing theorem/model-check evidence summary and bless/review flow**
   - sample code から見える theorem/model-check evidence と current bless/review flow を narrow に揃える。
3. **Macro 7 / docs-first I/O / host-facing port boundary**
