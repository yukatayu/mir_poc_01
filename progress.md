...
- **Phase 5 small decidable core / proof / async-control boundary** は、`specs/examples/126-current-l2-small-decidable-core-and-proof-boundary-inventory.md` から `specs/examples/172-current-l2-theorem-line-attachment-blob-ready-retained-file-body-threshold.md` までで theorem-line later package の current first choice を段階的に伸ばし、現時点では `retained_file_body_ref` までを symbolic retained bridge に寄せられるところまで整理した。したがって current package は theorem-line later package close とみなし、next later reopen は **actual archive materialization comparison** の comparison に寄せてよい。
...
2. Phase 5 current package は checkpoint close として維持し、actual archive materialization comparison が必要になったときだけ later pressure で reopen する
...
2. Phase 5 を reopen する場合は、actual archive materialization をどこまで theorem-line bridge に寄せるかを先に narrow に扱う
