...
- Mirrorea future lane:
  `TermSignature registry / debug output` と `LayerSignature system` を helper-local / report-local first cut として close し、next semantic carrier package を `MessageEnvelope / Auth seam` と読む
...
| Mirrorea future axis | 63% | 73% | 30% | queue は維持したまま `TermSignature` / `LayerSignature` first cut を通し、next は `MessageEnvelope` と `VisualizationProtocol` |
...
| layer composition / auth / visualization / projection / hot-plug | `S2-S3` | package queue、stop line、phase 8 representative skeleton family、`TermSignature` / `LayerSignature` first cut、`--debug signatures` / `--debug layers` を actualize | envelope、viewer、projection、AttachPoint actualization |
