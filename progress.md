...
最終更新: 2026-04-06（stage 1 parser spike input surface / helper naming まで反映）
...
- current parser boundary 主線では、actual stage 1 parser spike の private helper 配置と compare surface に加えて、input surface は test inline string、`decl_guard_slot` internal carrier は dedicated wrapper + owned `surface_text`、private helper family は `current_l2_stage1_parser_spike_support` を第一候補にする docs-only cut まで揃った。
...
| parser / syntax finalization 準備 | 66% | 71% | 24% | 着手可能 | first parser cut に入れてよい semantic cluster の inventoryに加え、actual parser spike を切るなら chain / declaration structural floor、`TryFallback` / `AtomicCut` structural floor、request / admissibility cluster の checker-led staged order が source-backed に見えた。さらに stage 1 では declaration-side guard slot を opaque attached slot に留め、predicate fragment parse / option-local `admit` / request clause parse は later stage に残す cut、parser-side opaque slot carrier から current fixture schema の `OptionDecl.lease` へ thin lowering bridge を使う minimal handoff、parser-side naming は `decl_guard_slot` を第一候補にし bridge は option-level structural transfer として narrow に読む cut、actual spike の最小 smoke family は `e4` + `e7` に留めて `e3` を later-stage contrast に残す cut、actual spike 自体は `crates/mir-ast/tests/support/` 配置の private helper + lowered fixture-subset compare から始める cut、そして actual implementation 前の input surface / internal carrier / helper naming も inline text + dedicated wrapper + repo-scoped private helper family まで docs-only で固定できた。ただし final grammar、exact lexical choice、predicate completion、actual parser API は未決 |
