...
を docs-first に固めたうえで、typed family split、notebook-consumer threshold、model-check small-cluster projection keep/drop、order/handoff source-surface wording reserve、modality internalization trigger、stronger typed-surface threshold framing、theorem discharge transport / public-contract later-gate framing、model-check property-language / tool-binding later-gate framingまで fixed にした line である。
...
installed-binary / fairness operational profile は mixed gate に残した line である。
...
- room-profile / fairness-replay / packaging mixed-gate boundary の docs-first hardening
...
4. shared-space final fairness / replay operational profile mixed gate
5. public operational CLI installed-binary / packaging success-criteria mixed gate

この順序だと、

- theory-lab line を mainline と混ぜずに boundary / pilot / framing closeout fixed まで進められる
- execution lane も duplicate pair actualization cut まで narrow に保てる
- reserve integration line も hidden promotion なしで packaging / fairness mixed-gate boundary へ進められる
