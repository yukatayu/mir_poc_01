...
1. public operational CLI packaging reserve note
2. shared-space fairness / replay strengthening reserve note
3. request / predicate / `try` cluster typed-surface reserve note
4. admissible evidence contraction note
5. tool-binding stop-line refresh
6. order / handoff emitted-artifact schema reserve note
7. guarded-vs-MDTT/MTT reduction timing note
8. malformed duplicate-cluster source-sample widening comparison

この順序だと、

- theory-lab line を mainline と混ぜずに second-order reserve として前進できる
- execution lane も duplicate cluster widen cut まで narrow に保てる
