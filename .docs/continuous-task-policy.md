...
- ただし過剰頻度にはしない。平均感覚としては **おおむね 1 時間に 1 回程度** を目安にし、数分ごとの細切れ通知は避ける。
- package close 後に **そのまま user 入力なしで続行できる** ときは、`progress` に留めて次の package へ進む。
- `complete` は、連続 task 全体に対する最終返答の直前に 1 回だけ送る。
  - user 依頼の scope を完了したとき
