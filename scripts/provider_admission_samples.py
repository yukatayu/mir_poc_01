...
NON_CLAIMS = [
    "provider admission is bounded inventory/admission evidence only",
    "native execution stays disabled by default",
    "WASM rows stay inventory-only until explicit sandbox admission exists",
    "viewer/diagnostic provider is not a world-semantics owner",
]
VALIDATION_FLOOR = [
...
def _run_provider_admission(path: Path, request: Path, provider: Path, input_value: int) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            f"provider admission example did not return JSON for `{path}`: {completed.stderr}"
        ) from error
...
def _summary(payload: dict[str, Any]) -> dict[str, Any]:
    split = payload.get("local_split_report") or {}
    target_reports = split.get("target_reports") or []
    launched_targets = sorted(
        row["target_id"]
        for row in target_reports
        if row.get("launched_entry_transitions")
    )
    passive_targets = sorted(
        row["target_id"]
        for row in target_reports
        if not row.get("launched_entry_transitions")
    )
    return {
        ...
        "execution_admitted": payload.get("execution_admitted"),
        ...
        "local_split_launched_targets": launched_targets,
        "local_split_passive_targets": passive_targets,
...
def _generated_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "local_split": {
            "accepted": split.get("accepted"),
            "launch_mode": split.get("launch_mode"),
            "target_execution_kinds": {
                row["target_id"]: row["execution_kind"]
                for row in (split.get("target_reports") or [])
            },
        },
    }
...
def run(sample_id: str) -> dict[str, Any]:
    ...
    if row is None:
        raise ValueError(f"unknown provider admission sample `{sample_id}`")

    actual_payload = _run_provider_admission(
...
def format_pretty(payload: Any) -> str:
    if isinstance(payload, list):
        lines = ["PROVIDER ADMISSION SAMPLES"]
        for row in payload:
