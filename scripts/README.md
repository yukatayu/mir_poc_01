...
- Surface Mir helper family has a P-SURF-01 parser floor, P-SURF-02 indexed-state semantic checker floor, and P-SURF-03 elaboration evidence floor
  ...
  - current actualized elaboration rows are `ELAB-01/02/04/05/06/07/08`: cross-locus read/write remote requests, generated edges, source spans, obligations, read/write underdeclared generated failure-row rejection, unsupported-statement rejection, and nested-place read placement
  ...
  - this helper floor does not claim auto communication completion, runtime execution, source patch activation, final grammar/API, or package artifact authority
