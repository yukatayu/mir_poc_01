...
- product alpha-1 CLI/schema first cut now has a Rust CLI surface
  ...
  - `run-membership-chat`, `run-sugoroku`, `export-devtools`, `release-check`, and `check-all` now include bounded operational semantic checks for direct text host-I/O, Sugoroku runtime evidence, and schema-backed projection inventory rather than only command exit status
