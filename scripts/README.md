...
- Surface Mir helper family has a P-SURF-01 parser floor and P-SURF-02 indexed-state semantic checker floor
  ...
  - future anchors remain `cargo test -p mir-semantics --test surface_to_core_elaboration -- --nocapture`, `cargo test -p mir-runtime --test source_patch_hotplug -- --nocapture`, and `cargo test -p mirrorea-cli --test surface_mir_cli -- --nocapture`
  - this helper floor does not claim generated Core IR, runtime execution, source patch activation, final grammar/API, or package artifact authority
