...
- Full System V1 helper family now has parser-floor and typed-checker-floor commands plus later planned commands
  ...
  - together they validate parser acceptance/rejection, path-aware unresolved import rejection, expression/statement spans, typed IR lowering, type/scope/import/effect/failure/capability rows, structural summaries, and sample/expected matrix consistency without changing the Product Alpha `package.mir.json` front door
  - they do not claim final public grammar, final typed IR API, interpreter, runtime execution, or package artifact generation
