...
- product alpha-1 CLI/schema first cut now has a Rust CLI surface
  ...
  - `operational_product_samples.py` is the orchestration helper for `samples/product-alpha1/operational/`; it keeps `mirrorea-alpha` as the canonical entrypoint and bundles `list`, `check-all`, `run-world-core`, `run-membership-chat`, `run-sugoroku`, `run-portal-worldlink`, `attach-layers`, `transport-local`, `transport-docker`, `export-devtools`, `build-native-bundle`, and `release-check`
  - `run-membership-chat`, `run-sugoroku`, `run-portal-worldlink`, `export-devtools`, `release-check`, and `check-all` now include bounded operational semantic checks for direct text host-I/O, Sugoroku runtime evidence, portal discrete handoff evidence, and schema-backed projection inventory rather than only command exit status
