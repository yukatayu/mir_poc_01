...
- `check_source_hierarchy.py`
  required root docs / specs / plan / support directory が存在するかを見る structural check。current line では `specs/13..43`、`plan/39..68`、Surface Mir guides、`samples/alpha/`、`samples/product-alpha1/README.md`、`samples/product-alpha1/demo/README.md`、`samples/product-alpha1/demo/package.mir.json`、`sub-agent-pro/alpha-0/`、`sub-agent-pro/alpha-1/`、`sub-agent-pro/product-alpha1-001/`、`sub-agent-pro/full-system-completion-001/` も structural presence の対象に入る。`P-SURF-00B` で追加した `specs/39..43` / `plan/64..68` は docs/spec source-authority rebaseline の正本として structural presence の対象に入るが、Surface sample/script roots are planned until later implementation packages. 文書内容、stale wording、normative consistency、report template completeness は判定しない。
- `validate_docs.py`
  required documentation scaffold、`progress.md` / `tasks.md` required snapshot heading order、numbered report、report template closeout headings、latest numbered report の required heading presence / order、empty required section、unresolved update-status placeholder を確認する scaffold check。current line では snapshot docs、`samples/README.md` / `scripts/README.md`、`samples/alpha/README.md`、`samples/product-alpha1/README.md`、`samples/product-alpha1/demo/README.md`、`samples/product-alpha1/demo/package.mir.json`、Surface Mir guides、`plan/39..68`、`specs/13..43` も required scaffold に入る。Surface sample/script roots remain planned and are not validator-required until implementation packages create them. historical report 全体の semantic validation、active/current wording lint、sample execution、Cargo validation は別 command の責務。
...
- Surface Mir helper family is planned, not active, after `P-SURF-00B`
  - planned anchors are `python3 scripts/surface_mir_samples.py matrix --format json`, `python3 scripts/surface_mir_samples.py check-all --format json`, and `python3 scripts/surface_mir_release_check.py --format json check-all --out /tmp/mirrorea-surface-release`
  - planned cargo anchors are `cargo test -p mir-ast --test surface_mir_parser -- --nocapture`, `cargo test -p mir-semantics --test surface_to_core_elaboration -- --nocapture`, `cargo test -p mir-semantics --test indexed_state_semantics -- --nocapture`, `cargo test -p mir-runtime --test source_patch_hotplug -- --nocapture`, and `cargo test -p mirrorea-cli --test surface_mir_cli -- --nocapture`
  - these commands must not be reported as runnable until later packages create the scripts/tests
  - the future helper family must accept canonical `S { ... }`, reject `S[ ... ]`, preserve `.mir` files as semantic source authority, expose generated Core IR / communication / publish / observe, and keep role claims separate from capability grants
