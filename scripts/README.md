...
- Full System V1 helper family now has parser-floor, typed-checker-floor, bounded runtime commands, PoseGraph runtime commands, bounded projection IR + boundary-schema commands, bounded same-binary local role-split commands, and bounded provider-admission commands plus one later planned release-check command
