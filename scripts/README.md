...
- product alpha-1 CLI/schema first cut now has a Rust CLI surface
  ...
  - `check-all` also reports machine-readable `widening_queue_scope`, keeping room-chat reopening and portal/shard starter reopening non-promoted while advancing the next promoted comparison to `broader_sugoroku_reopening`
