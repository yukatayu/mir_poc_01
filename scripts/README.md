...
- product alpha-1 CLI/schema first cut now has a Rust CLI surface
  ...
  - repo-owned operational roots and layer package inputs are passed to nested `mirrorea-alpha` commands as repo-relative `samples/...` argv so transient helper reports do not depend on the host checkout path
...
- Full System V1 helper family now has parser-floor, typed-checker-floor, bounded runtime commands, bounded source-first operational-suite commands, PoseGraph runtime commands, bounded projection IR + boundary-schema commands, bounded same-binary local role-split commands, bounded provider-admission commands, bounded renderer-pose commands, and a bounded release-check command
  ...
  - provider and renderer generated reports are committed bridge evidence: helpers compare fresh projections against those files, do not rewrite them, fail closed on a missing/mismatched report or inconsistent accepted/rejected exit code, and return exit status 2 for a failed standalone `run` or `check-all`
  - parser, checker/runtime, PoseGraph, and projection/local-split rows also require the nested command's exit code to be 0 for an accepted payload and 2 for an expected rejection or violation; a helper `run` succeeds only when that code and all expected evidence agree
  ...
  - release-check acceptance explicitly excludes C-distributed conformance and real transport / multi-process distributed execution; the local role split remains same-binary evidence only
  ...
  - they do not claim C-distributed conformance, real transport or multi-process distributed execution, final public grammar, final typed IR API, final effect ABI, final packet/FFI transport semantics, final server/client binary split, arbitrary native/WASM execution, distributed cut/save execution, final public viewer/devtools family completion, final installer/archive hardening, or package artifact generation
...
- `proof_first_durable_dispatch_check.py test <existing-work-directory>`
  nonproduction Linux reservation/crash experiment. New private files and actual
  child processes test synchronization-before-dispatch, restart, cooperative
  exclusion, malformed history and counterexamples. It records actual callback
...
- `env/mirrorea_storage_env.sh`
  mounted external workdir 前提の env export surface。`MIRROREA_WORKDIR`、`CARGO_TARGET_DIR`、`CARGO_HOME`、LLVM staging path、mount/ownership/writable status を返し、`findmnt --mountpoint` による exact mountpoint 判定で `MIRROREA_WORKDIR_MOUNTED` を出す。`--ensure-dirs` は unmounted default root への heavy dir 作成を拒否する
- `storage/`
...
- `tests/`
  includes repo-local runner/unit tests plus `test_discord_notify_skill.py`,
  which imports `.agents/skills/discord-report/scripts/discord_notify.py` and
  mocks webhook delivery to verify file-based summary / next-step inputs.
...
W4のqualified owner・codec・応答profile・完了用処理枠の一般証明は、既存の
