...
class PracticalAlpha1AvatarTests(unittest.TestCase):
    ...
    def test_repo_cli_arg_relativizes_repo_owned_package_dir(self) -> None:
        package_dir = REPO_ROOT / "samples/practical-alpha1/packages/av-a1-01-placeholder-avatar-runtime"
        self.assertEqual(
    ...
    def test_repo_cli_arg_keeps_external_path_absolute(self) -> None:
        external = Path("/tmp/mirrorea-external-avatar-package")
        self.assertEqual(runner.repo_cli_arg(external), str(external))

    def test_build_avatar_report_uses_repo_relative_package_arg(self) -> None:
        ...
        with mock.patch.object(runner.subprocess, "run", return_value=completed) as mocked_run:
            payload = runner._build_avatar_report(package_dir)
