...
def filter_capability_rows(rows: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    if not rows:
        return []
    return [
        row
        for row in rows
        if isinstance(row, dict) and row.get("kind") in CAPABILITY_KINDS
    ]
...
def status_for_rows(
    ...
    if not fixture_rows and actual_rows:
        return "fixture_capability_rows_missing", 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    fixture_rows = filter_capability_rows(read_fixture_checked_reason_codes(fixture))
    _, actual_reason_rows = read_actual_reason_code_candidates(artifact)
    actual_rows = filter_capability_rows(actual_reason_rows)

    status, exit_code = status_for_rows(fixture_rows, actual_rows)

    print(f"fixture: {fixture_path}")
    print(f"artifact: {artifact_path}")
    print("cluster: capability_strengthening_floor")
