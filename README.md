...
Surface Mir alpha は、この closed Full System V1 floor の上に置く user-facing source authority line です。Canonical place-scope syntax は `S { ... }` で、`S[ ... ]` は sugar としても採用しません。`[]` は配列・Map・indexed state・role instance head の value-level indexing に残します。`P-SURF-01` で parser floor、`P-SURF-02` で indexed-state semantic checker floor、`P-SURF-03` で Surface-to-Core elaboration evidence floor、`P-SURF-04` で generated `MessageEnvelope` / visible publish / observe / `VisibilityDenied` evidence floor が actualize 済みです。`P-SURF-05` では `crates/mir-semantics::surface_role_admission`、`surface_role_admission_check`、`samples/full-system-v1-surface/role-admission/`、`ROLE-01..04` により、role claim、join admission request、accepted verdict、admission witness、capability grant-backed accepted write、missing-grant write rejection、stale membership rejection、optional package/runtime hash metadata が report-level evidence として actualize されました。Surface Mir は user-facing source、Core Mir は elaboration target であり、通信・publish・observe は Surface から自動生成して Core IR / devtools に明示します。`state player[p: Participant]: Player` は S-owned Participant-indexed map で、key は authority ではありません。role claim は authority ではなく、authority は admission locus からの capability grant です。source patch hot-plug は direct eval ではなく parse / typecheck / elaborate / admit / activation_cut pipeline として扱います。新しい正本 / repository memory は `specs/39..43` と `plan/64..68` です。current promoted next package は `P-SURF-06 source patch hot-plug` です。
...
- Surface Mir runtime implementation beyond the P-SURF-05 role-admission report-level evidence floor
