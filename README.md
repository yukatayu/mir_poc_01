...
- **Operational alpha theory-freeze / session-runtime line**
  ...
  現在の repo は evidence closeout と first-floor carriers を多数持ち、`P-A1-19` で same-session α-0.5 session carrier まで actualize しましたが、typed external host-I/O direct execution lane、same-session hot-plug runtime、session-bound full devtools export はまだ未 actualize です。
