...
- `hands-on docs / closeout` も close してあり、`NET-01` helper-local loopback preview、`NET-02..05` helper-local canary、avatar widened representative slice、hot-plug helper-local lifecycle canary まで actualize しました。`cross-package sweep` 後に `FAIRY-05` residual reacquire design review を docs-first で閉じ、sample 自体は planned のままにしつつ、explicit state timeline / anchor switch evidence が要ることと exact carrier bundling が `UNRESOLVED` であることだけを固定したので、current next queue は Typed external boundary / adapter executable widening です。public avatar / visualization API を示唆するものではありません。
