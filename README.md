...
repo が主として維持しているのは、Mir current-L2 の **repo-local alpha-ready current layer**、Mirrorea Spaces の **product alpha release-candidate workflow**、その次段の **canonical operational product sample suite**、現在 rebaseline 済みの **Mir Computational Core docs/spec line**、closed **Full System V1 source-first bounded release-check line**、そして次に promoted された **Surface Mir brace syntax / source-authority roadmap** です。
...
Surface Mir alpha は、この closed Full System V1 floor の上に置く user-facing source authority line です。Canonical place-scope syntax は `S { ... }` で、`S[ ... ]` は sugar としても採用しません。`[]` は配列・Map・indexed state・role instance head の value-level indexing に残します。`P-SURF-01` で `crates/mir-ast::surface_alpha`、`surface_mir_alpha_parse`、`samples/full-system-v1-surface/syntax/`、`scripts/surface_mir_samples.py` が parser floor として actualize され、`S { ... }` / `Role[instance] { ... }` / `state` / `when` / `join` / record literal / expected parser rejection を確認できます。`P-SURF-02` では `crates/mir-semantics::surface_indexed_state`、`surface_indexed_state_check`、`samples/full-system-v1-surface/indexed-state/`、`IDX-01..05` が indexed-state semantic checker floor として actualize され、owner locus、Participant keyspace、key-not-authority rejection、stale key rejection、retained-savepoint compaction rejection、nested-place ambient-authority rejection を確認できます。`P-SURF-03` では `crates/mir-semantics::surface_to_core_elaboration`、`surface_to_core_elaborate`、`samples/full-system-v1-surface/elaboration/`、`ELAB-01/02/04/05/06/07/08` が Surface-to-Core elaboration evidence floor として actualize され、cross-locus read/write が explicit Core IR remote request、generated edge、source span、obligation row を持つこと、read/write underdeclared generated failure row が reject されること、unsupported statements が silent-drop されないことを確認できます。`P-SURF-04` では generated `MessageEnvelope`、visible field publish / observe rows、`VisibilityDenied` failure-row containment、private / non-visible field rejection を Core IR に出すようになり、`ELAB-03/09/10` を加えて 24 Surface rows を確認できます。Surface Mir は user-facing source、Core Mir は elaboration target であり、通信・publish・observe は Surface から自動生成して Core IR / devtools に明示します。`state player[p: Participant]: Player` は S-owned Participant-indexed map で、key は authority ではありません。role claim は authority ではなく、authority は admission locus からの capability grant です。source patch hot-plug は direct eval ではなく parse / typecheck / elaborate / admit / activation_cut pipeline として扱います。新しい正本 / repository memory は `specs/39..43` と `plan/64..68` です。current promoted next package は `P-SURF-05 role admission capability grant` です。
...
- Surface Mir runtime implementation beyond the P-SURF-04 generated communication elaboration evidence floor
...
- Surface Mir source operational suite beyond the current parser + indexed-state + elaboration evidence floors
