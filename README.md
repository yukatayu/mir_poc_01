...
Surface Mir alpha は、この closed Full System V1 floor の上に置く user-facing source authority line です。Canonical place-scope syntax は `S { ... }` で、`S[ ... ]` は sugar としても採用しません。`[]` は配列・Map・indexed state・role instance head の value-level indexing に残します。Surface Mir は user-facing source、Core Mir は elaboration target であり、通信・publish・observe は Surface から自動生成して Core IR / devtools に明示します。`state player[p: Participant]: Player` は S-owned Participant-indexed map で、key は authority ではありません。role claim は authority ではなく、authority は admission locus からの capability grant です。source patch hot-plug は direct eval ではなく parse / typecheck / elaborate / admit / activation_cut pipeline として扱います。新しい正本 / repository memory は `specs/39..43` と `plan/64..68` です。current promoted next package は `P-SURF-01 surface brace parser` です。
...
- Surface Mir parser / checker / runtime implementation
