...
- `hands-on docs / closeout` も close してあり、`NET-01` helper-local loopback preview、`NET-02..05` helper-local canary、avatar widened representative slice、hot-plug helper-local lifecycle canary まで actualize しました。`cross-package sweep` により active evidence / planned family / mixed gate の切り分けも揃い、current next queue は `FAIRY-05` residual reacquire design です。
