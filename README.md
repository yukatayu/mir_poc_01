...
Surface Mir alpha は、この closed Full System V1 floor の上に置く user-facing source authority line です。Canonical place-scope syntax は `S { ... }` で、`S[ ... ]` は sugar としても採用しません。`[]` は配列・Map・indexed state・role instance head の value-level indexing に残します。`P-SURF-01` で parser floor、`P-SURF-02` で indexed-state semantic checker floor、`P-SURF-03` で Surface-to-Core elaboration evidence floor、`P-SURF-04` で generated `MessageEnvelope` / visible publish / observe / `VisibilityDenied` evidence floor、`P-SURF-05` で role admission / capability grant report-level evidence floor が actualize 済みです。`P-SURF-06` では `crates/mir-runtime::surface_source_patch_hotplug`、`mirrorea-alpha check-source` / `parse-source` / `elaborate-source` / `patch-source` / `export-core-ir`、`samples/full-system-v1-surface/source-patch/`、`PATCH-01..04` により、source patch が parse / typecheck / elaborate / compatibility / admission を通り、HotPlugRequest、HotPlugVerdict、Core IR diff、activation_cut を出すこと、rejected patch が runtime mutation しないこと、direct eval しないことを alpha evidence として actualize しました。Surface Mir は user-facing source、Core Mir は elaboration target であり、通信・publish・observe は Surface から自動生成して Core IR / devtools に明示します。`state player[p: Participant]: Player` は S-owned Participant-indexed map で、key は authority ではありません。role claim は authority ではなく、authority は admission locus からの capability grant です。source patch hot-plug は direct eval ではなく parse / typecheck / elaborate / admit / activation_cut pipeline として扱います。新しい正本 / repository memory は `specs/39..43` と `plan/64..68` です。current promoted next package は `P-SURF-07 source operational suite` です。
