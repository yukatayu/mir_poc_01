...
operational suite には、active runnable roots とは別に `templates/world-core-starter/`、`templates/membership-chat-starter/`、`templates/sugoroku-world-starter/` と external developer 向け authoring guide も追加されており、template-only starter catalog を使った `author -> check -> run-local` の入口を bounded に再現できます。current starter catalog は intentionally `SugorokuWorld` で止め、portal/shard authoring は active executable roots を study/copy boundary にしつつ `future/` blueprints は non-executable inventory に留めます。portal/shard future line では `gradient-observation.profile.json` による docs-first profile inventory を保持したまま、separate `two-shard-gradient-observation/` root で observer-only widening の bounded same-session runtime cut も actualize しました。さらに `P-OPS-23` で helper-reported `sugoroku_scope` を追加し、current `SugorokuWorld` carrier は bounded deterministic same-session roll / publish / witness / handoff / stale-membership-reject scenario に留まり、interactive turn choice surface、broader negative-row catalog、networked multi-participant control は未定義と固定しました。`P-OPS-25` では helper-reported `widening_queue_scope` を更新し、room-chat reopening、portal/shard starter reopening、broader Sugoroku reopening は current line ではすべて non-promoted、later user-final distribution decision が next promoted comparison であることを machine-readable に固定しました。`P-OPS-26` ではさらに helper-reported `user_final_decision_scope` を追加し、current delivery unit は developer-built binary + generated host launch bundle、current catalog scope は bounded product alpha-1 narrow showcase、broader final distribution / final shared-space catalog breadth は user-spec-required gate であることを machine-readable に固定しました。backend については `native host launch bundle` だけが actualized で、WASM / LLVM は docs-first inventory に留めます。
...
- **Product/Public-ready Mirrorea Spaces alpha-1 line**
  規範判断は `specs/25-product-alpha1-public-boundary.md`、repository memory は `plan/50-product-alpha1-public-boundary-roadmap.md` に置きます。`P-A1-25` は、bounded workflow と product/public-ready alpha-1 の差を recut し、alpha `U1` defaults として Rust CLI entrypoint、versioned `package.mir.json`、native process + local/Docker host target、non-final viewer、native launch bundleを採ることを固定しました。`P-A1-26` は `mirrorea-cli` / `mirrorea-alpha` と `samples/product-alpha1/demo/` の versioned package schema first cut を追加し、`check` だけを実装、他 command family は structured unsupported diagnostic を返す状態にしました。これは product-ready 実装ではなく、`P-A1-27..31` の promoted line を開く entrypoint/schema floor です。
  `P-A1-27` は `crates/mir-runtime::product_alpha1_session` と `mirrorea-alpha run-local` / `session` / `attach` を追加し、`samples/product-alpha1/demo/` を local file-backed same-session carrier 上で runtime plan、core fabric envelope validation、typed host-I/O `AddOne` observation、debug-layer hot-plug lifecycle まで進められるようにしました。これは product alpha local same-session runtime floor であり、local/Docker transport command、save/load/quiescent-save execution、viewer、native bundle、release validation はまだ後段です。
  `P-A1-28` は同じ product session carrier に bounded message recovery rows、`mirrorea-alpha save` / `load`、および bounded local `quiescent-save` を追加しました。R0 local save/load と R2 quiescent-save は local session store first cut として再現できますが、local/Docker transport command、viewer、native bundle、release validation、R3/R4 durable distributed save/load はまだ後段です。
  ...
  `P-A1-31` は `mirrorea-alpha demo` と `scripts/product_alpha1_release_check.py check-all` を追加し、source-backed debug/auth/rate-limit layer attach、deferred object/avatar-preview attach、local/Docker transport、concrete non-final devtools/view、local save/load、bounded quiescent-save、native host launch bundle を documented release-candidate workflow として再現可能にしました。Docker を skip した場合は partial local probe であり release-candidate readiness ではありません。これは product alpha release-candidate readiness であり、final public product / grammar / ABI / WAN / distributed durable save/load / arbitrary native execution ではありません。`P-OPS-17` はその上で `scripts/product_alpha1_installed_binary_check.py` を追加し、built `mirrorea-alpha` binary と generated native host launch bundle を current first public-ish adoption candidate として probe できるようにしました。`P-OPS-18` はさらに current hardening target を versioned `package.mir.json`、documented `mirrorea-alpha` command family、native host launch bundle replay surface に絞りました。`P-OPS-19` はその narrowed front door のうち current shipped surface を built-binary `check` / `build-native-bundle` / `demo`、bundle replay `run.sh check` / `run.sh view`、および bundled CLI / package root / `manifest.json` / `launch.json` / `run.sh` / `README.md` / observer-safe supporting artifacts に限定し、other bundled reports や admin/debug session-store artifacts を compatibility promise の外へ明示しました。`P-OPS-20` はさらに broader public distribution を narrowing し、current delivery unit は developer-built `mirrorea-alpha` binary + locally generated native host launch bundle のみであり、archive / installer / system package / auto-update / hosted-service shape はまだ定義しないと固定しました。これは queue narrowing であり、final public packaging / textual grammar / Rust library ABI freeze ではありません。
  `P-OPS-01` は `samples/product-alpha1/operational/`、`specs/26`、`plan/51`、`scripts/operational_product_samples.py` を追加し、`WorldCore -> MembershipChat -> SugorokuWorld` の package/import chain、shared attach packages、save/load / transport / devtools / native host bundle の bounded operational replay、projection intent、portal/shard future inventory を 1 つの canonical suite としてまとめました。`P-OPS-06` ではその上に `portal-worldlink/` bounded same-session discrete handoff root を追加し、`future/portal-worldlink/` blueprint は保持したまま runtime evidence を actualize しました。`P-OPS-07` ではさらに `two-shard-hard-boundary/` bounded same-session hard-authority handoff root を追加し、`future/two-shard-hard-boundary/` と `spatial-shard-future.profile.json` は retained blueprint inventory として残しました。`P-OPS-15` では `two-shard-gradient-observation/` bounded observer-only gradient root を separate runnable root として追加し、`future/gradient-observation.profile.json` は non-executable inventory のまま保持しました。
  `P-OPS-03` はその suite 上で `MembershipChat` の bounded direct text host boundary を actualize し、`P-OPS-13` では current runtime lane を bounded room-oriented `ChatText("hello room") -> "room#lobby message accepted: hello room"` へ widen しました。`P-OPS-21` ではさらに helper-reported `room_chat_scope` を追加し、current lane が bounded single-message room-oriented `ChatText` に留まり、multi-message / transport-coupled / room-history / stdio shapes は未定義であると固定しました。`P-OPS-22` では続けて helper-reported `portal_shard_starter_scope` を追加し、validated starter catalog が `templates/sugoroku-world-starter/` で止まり、portal/shard authoring は active executable roots を study/copy boundary に使う current line を machine-readable に固定しました。`P-OPS-23` では helper-reported `sugoroku_scope` を追加し、current `SugorokuWorld` carrier は bounded deterministic same-session roll / publish / witness / handoff / stale-membership reject scenario に留まり、interactive turn choice surface、broader negative rows、networked multi-participant control は未定義であると固定しました。`P-OPS-25` ではさらに helper-reported `widening_queue_scope` を更新し、room-chat reopening、portal/shard starter reopening、broader Sugoroku reopening は current line では non-promoted、later user-final distribution decision が next promoted comparison であることを machine-readable に固定しました。`P-OPS-26` ではさらに helper-reported `user_final_decision_scope` を追加し、その next comparison は active self-driven widening ではなく、broader distribution / final catalog breadth に関する user-spec-required gate であると固定しました。`run-local` と observer-safe devtools export から同じ lane / carrier を確認できますが、これは final room-chat service や interactive multiplayer game completion ではありません。
  `P-OPS-04` はさらに `SugorokuWorld` に bounded same-session roll / publish / witness / handoff / stale membership reject scenario を actualize し、`run-local` / session / devtools export / helper closeout から同じ observer-safe runtime evidence を確認できるようにしました。`P-OPS-05` は `deployments/projection/projection.profile.json` を schema-backed inventory として `check`、runtime plan、observer-safe devtools projection panel、helper `release-check` / `check-all` に通し、packet / FFI / target summary を executable alpha line の外側で検証可能にしました。これは final interactive game runtime、final server/client binary split、direct LLVM backend claim ではありません。
