...
operational suite には、active runnable roots とは別に `templates/world-core-starter/`、`templates/membership-chat-starter/`、`templates/sugoroku-world-starter/` と external developer 向け authoring guide も追加されており、template-only starter catalog を使った `author -> check -> run-local` の入口を bounded に再現できます。current starter catalog は intentionally `SugorokuWorld` で止め、portal/shard authoring は active executable roots を study/copy boundary にしつつ `future/` blueprints は non-executable inventory に留めます。portal/shard future line では `gradient-observation.profile.json` により observer-only widening の reading も docs-first に固定しました。backend については `native host launch bundle` だけが actualized で、WASM / LLVM は docs-first inventory に留めます。
