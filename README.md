...
- `hands-on docs / closeout` も close してあり、`NET-01` helper-local loopback preview、`NET-02..05` helper-local canary、avatar widened representative slice、hot-plug helper-local lifecycle canary、typed external boundary synthetic preview helper、projection / placement helper/report-local preview floor まで actualize しました。さらに `P2` Typed external boundary residual planned family review も close し、`EXT-01` / `EXT-02` / `EXT-05` の indirect anchor / reopen criterion / kept-later gate を fixed しました。current next queue は `P3` Projection / placement residual emitted-program gate で、その次が `P4` `TermSignature` hardening です。public avatar / visualization / adapter / projection API を示唆するものではありません。
