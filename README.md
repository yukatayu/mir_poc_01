...
- **Mirrorea Spaces practical alpha-1 line**
  ...
  ただし、これは promoted work queue であり、active canonical runnable root への昇格を意味しません。2026-05-05 時点の latest package closeout は `P-A1-20` typed host-I/O direct execution lane です。
...
- **Operational alpha theory-freeze / session-runtime line**
  ...
  `P-A1-19` と `P-A1-20` により、same-session α-0.5 session carrier と typed external `AddOne` direct execution lane が actualize され、bounded operational α-0.5 local observable runtime は揃いました。same-session hot-plug runtime と session-bound full devtools export は引き続き未 actualize です。
