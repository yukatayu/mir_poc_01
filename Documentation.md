...
- **Mirrorea Spaces practical alpha-1 line**
  ...
  最初の narrow front-door cut として、`samples/practical-alpha1/` と `crates/mir-ast::practical_alpha1` の limited `package.mir.json` loader floor を current repo state で持ちます。これは library-first parse/load floor であり、final textual grammar、reusable checker/runtime API、product-like command surface ではありません。
