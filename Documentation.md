...
- helper-local checker payload row-detail / row-body / supported-kind-summary threshold:
  prototype / sample 実行時に source-side IFC trio `p10 / p11 / p12` を `actual_checker_payload_row_detail_threshold`、`actual_checker_payload_row_body_threshold`、`actual_checker_payload_supported_kind_summary_threshold` として見せ、`payload_row_family_ref + row_source_ref + row_reason_kind`、`row_body = { selected_option_ref, visibility_target_ref }`、`payload_row_family_ref + supported_kind_scope + supported_kind_refs` を row-detail / row-body / supported-kind-summary bridge として sample-local helper threshold に留める current cut がある
