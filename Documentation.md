...
manifest 済みです。二つの sidecar は input に限られ、得られた result も scoped
  ...
  `captureSubset` helper の全入力決定可能性だけを、OBL-003 を前進させない L3 候補として選んだ。
  結果 command の前に別コミットで登録する。`plan/173-local-predicate-constructive-decidability-selection.md`
