...
- **Full System V1 source-first line**
  `specs/33..38`、`plan/58..63`、`progress.md`、`tasks.md` が current roadmap snapshot を担います。current promoted package は `P-FSV1-03 full V1 release check`、次の promoted package は `P-FSV1-99 final audit` です。ここでは Product Alpha-1 を final product に昇格せず、textual Mir、typed IR、bounded effectful runtime、PoseGraph runtime、bounded pose save/devtools、projection IR + boundary schemas + bounded local role split、bounded provider admission、renderer pose backend、source-first operational suites、portal/shard/gradient source suites、release check へ順に進めます。
