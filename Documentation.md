...
- `hands-on docs / closeout` も close し、avatar representative slice も active にし、hot-plug helper-local lifecycle canary、transport helper-local canary、projection / placement helper/report-local preview floor も actualize しました。phase 9 executable widening は `EXT-03` / `EXT-04` の thin synthetic preview helper cut まで actualize し、sample 自体は planned のままに保っています。さらに `P2` Typed external boundary residual planned family review と `P3` Projection / placement residual emitted-program gate も close し、`EXT-01` / `EXT-02` / `EXT-05` の indirect anchor / reopen criterion / kept-later gate、projection validity report minimum、generated artifact reserve policy を fixed しました。current next package は `P4` `TermSignature` hardening で、その次が `P5` `LayerSignature` hardening です。
