...
- **operational product sample suite**
  `specs/26` / `plan/51` で定義した `WorldCore -> MembershipChat -> SugorokuWorld -> PortalWorldLink` suite、shared attach packages、deployment/projection inventory、future shard inventory。current line では `MembershipChat` の bounded `EchoText` direct host boundary、`SugorokuWorld` の bounded roll / publish / witness / handoff / stale membership reject scenario、`projection.profile.json` の schema-backed target / packet / FFI inventory、そして `PortalWorldLink` の bounded same-session discrete handoff cut が `check`、runtime plan、observer-safe devtools / helper closeout に actualize 済み。`future/portal-worldlink/` blueprint は残す
