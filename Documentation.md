...
- `VerificationLayer` composition は、finite-index checker / theorem / model-check / runtime policy / visualization lane を typed layer として束ねる方向で読みます。
...
- `VisualizationProtocol` の first cut も close してあり、Sugoroku helper の `visualization_views` / `telemetry_rows` / `--debug visualization` と clean near-end report / closeout の report-local `VisualizationView` / `TelemetryRow` inventory を追加しました。
- performance telemetry も information-bearing effect として扱い、typed telemetry と label / authority / redaction を意識した carrier に留めます。
...
- avatar fairy follow は `samples/clean-near-end/avatar-follow/` と `scripts/avatar_follow_samples.py` で active representative slice を持ちます。
  ...
  これを reopen する前に explicit state timeline / anchor switch evidence が要ること、visibility-return witness をどの carrier へ載せるかは `UNRESOLVED` であること、という current line だけを固定しました。public avatar / visualization API を示唆するものではありません。
