...
- **Mir Computational Core rebaseline**
  `specs/28..31` / `plan/53..56` で定義し直した docs/spec line。Product Alpha-1 の operational floor は保持するが、current typed external `AddOne` を Mir-owned computation の証拠とは読まない。`P-COMP-01` で `samples/product-alpha1/computational/` と `scripts/mir_computational_samples.py` の planned-only scaffold は actualize 済みで、次の自走 line は PoseGraph / projection / engine front-half と、その後の computation ownership implementation half である。
