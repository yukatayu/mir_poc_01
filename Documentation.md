...
- **Mir Computational Core rebaseline**
  `specs/28..31` / `plan/53..56` で定義し直した docs/spec line。Product Alpha-1 の operational floor は保持するが、current typed external `AddOne` を Mir-owned computation の証拠とは読まない。`P-COMP-01` の `samples/product-alpha1/computational/` / `scripts/mir_computational_samples.py`、`P-POSE-01` の `samples/product-alpha1/posegraph/` / `scripts/posegraph_samples.py`、`P-PROJ-01` の `samples/product-alpha1/projection/` / `scripts/projection_boundary_samples.py`、`P-ENG-01` の `samples/product-alpha1/engine-adapter/` / `scripts/engine_adapter_boundary_samples.py` は planned-only scaffold として actualize 済みで、次の自走 line は computation ownership / no-split-frame implementation half である。
