...
- `Network transport` の docs-first plan も close してあり、`plan/22-network-transport-roadmap.md` に loopback / reconnect / failure matrix と stop line を置きました。
...
- `hands-on docs / closeout` も close し、次の package は `Network transport` executable widening、その後の avatar reopen / hot-plug executable widening です。
