...
- **Mir Computational Core rebaseline**
  `specs/28..31` / `plan/53..56` で定義し直した docs/spec line。Product Alpha-1 の operational floor は保持するが、current typed external `AddOne` を Mir-owned computation の証拠とは読まない。front-half scaffold は維持しつつ、`P-COMP-02` により `samples/product-alpha1/computational/add-one-pure-mir/` と `scripts/mir_computational_samples.py` が one executable Mir-owned row を持つようになった。PoseGraph / projection / engine-adapter は引き続き planned-only scaffold / inventory で、次の自走 line は `P-COMP-03` / `P-COMP-04` / `P-POSE-02` である。
