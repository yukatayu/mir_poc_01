...
- helper-local checker payload row-detail / row-body threshold:
  prototype / sample 実行時に source-side IFC trio `p10 / p11 / p12` を `actual_checker_payload_row_detail_threshold` と `actual_checker_payload_row_body_threshold` として見せ、`payload_row_family_ref + row_source_ref + row_reason_kind` と `row_body = { selected_option_ref, visibility_target_ref }` を row-detail / row-body bridge として sample-local helper threshold に留める current cut がある
