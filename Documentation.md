...
- **operational product sample suite**
  `specs/26` / `plan/51` で定義した `WorldCore -> MembershipChat -> SugorokuWorld` suite、shared attach packages、deployment/projection intent、future portal/shard inventory。current line では `MembershipChat` の bounded `EchoText` direct host boundary に加えて、`SugorokuWorld` の bounded roll / publish / witness / handoff / stale membership reject scenario まで actualize 済み。portal/shard は planned-only
