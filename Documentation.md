...
- `hands-on docs / closeout` も close し、avatar representative slice も active にし、hot-plug helper-local lifecycle canary と transport helper-local canary も actualize しました。さらに `FAIRY-02` visibility-loss fallback を active helper canary に昇格させ、`cross-package sweep` で active evidence / planned family / mixed gate を recut したので、次の package は `FAIRY-05` residual reacquire design です。
