...
- `hands-on docs / closeout` も close し、avatar representative slice も active にし、hot-plug helper-local lifecycle canary と transport helper-local canary も actualize しました。さらに `FAIRY-02` visibility-loss fallback を active helper canary に昇格させたので、次の package は cross-package sweep です。
