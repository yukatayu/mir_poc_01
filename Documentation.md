...
- `Typed external boundary / adapter` の docs-first sample plan も close してあり、phase 9 `EXT-01..05` planned family を `samples/not_implemented/typed-external-boundary/` に置きました。
...
- `HotPlug Patch / AttachPoint` の docs-first plan も close してあり、`plan/21-hotplug-attachpoint-roadmap.md` に compatibility / activation / migration stop line を置きました。
- `Network transport` の docs-first plan も close してあり、`plan/22-network-transport-roadmap.md` に loopback / reconnect / failure matrix と stop line を置きました。さらに `NET-01` helper-local loopback preview も actualize し、`--transport loopback_socket` で same-process envelope parity を確認できます。
...
- `hands-on docs / closeout` も close し、avatar representative slice も active にしたので、次の package は hot-plug executable widening、その後の transport `NET-02..05` です。
- avatar fairy follow は `samples/clean-near-end/avatar-follow/` と `scripts/avatar_follow_samples.py` で active representative slice を持ちます。
  `samples/not_implemented/avatar-fairy-follow/` には `FAIRY-02` / `FAIRY-05` の residual planned family だけを残します。
