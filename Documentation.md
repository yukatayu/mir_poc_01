...
- **Full System V1 source-first line**
  `specs/33..38`、`plan/58..63`、`progress.md`、`tasks.md` が current roadmap snapshot を担います。current promoted package は `P-ENG-02 provider admission`、次の promoted package は `P-ENG-03 renderer pose backend demo` です。ここでは Product Alpha-1 を final product に昇格せず、textual Mir、typed IR、bounded effectful runtime、PoseGraph runtime、bounded pose save/devtools、projection IR + boundary schemas + bounded local role split、provider admission へ順に進めます。
