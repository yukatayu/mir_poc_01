...
現在 repo は repo-local alpha-ready current layer、current-scope evidence、practical alpha-1 first-floor evidence に加えて、bounded operational α-0.5 / α-0.8 / α-0.9 と bounded practical α-1 integrated workflow carrier まで actualize 済みです。`P-A1-25` で product/public-ready alpha-1 の境界と alpha defaults は固定し、`P-A1-26` で `mirrorea-alpha check` と versioned product package schema first cut を追加しました。`P-A1-27` では `mirrorea-alpha run-local` / `session` / `attach` と product same-session carrier first cut を追加し、local runtime plan、core fabric envelope validation、typed host-I/O observation、debug-layer hot-plug lifecycle を同じ session file に保持できるようにしました。ただし product alpha-1 release validation、local/Docker transport command behavior、quiescent-save、viewer、native bundle、final public product はまだ別 gate です。
