...
- **operational product sample suite**
  `specs/26` / `plan/51` で定義した `WorldCore -> MembershipChat -> SugorokuWorld` suite、shared attach packages、deployment/projection inventory、future portal/shard inventory。current line では `MembershipChat` の bounded `EchoText` direct host boundary、`SugorokuWorld` の bounded roll / publish / witness / handoff / stale membership reject scenario、そして `projection.profile.json` の schema-backed target / packet / FFI inventory が `check`、runtime plan、observer-safe devtools projection panel に actualize 済み。portal/shard は planned-only
