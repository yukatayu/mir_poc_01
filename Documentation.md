...
- **operational product sample suite**
  `specs/26` / `plan/51` で定義した `WorldCore -> MembershipChat -> SugorokuWorld` suite、shared attach packages、deployment/projection intent、future portal/shard inventory。current line では `MembershipChat` の bounded `EchoText` direct host boundary まで actualize 済み。portal/shard は planned-only
