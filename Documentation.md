...
- **Mirrorea Spaces practical alpha-1 line**
  ...
  current repo state では、`samples/practical-alpha1/` と `crates/mir-ast::practical_alpha1` の limited `package.mir.json` loader floor に加え、distinct lowered IR を通る `crates/mir-ast::practical_alpha1_checker` と `scripts/practical_alpha1_check.py` の first checker floor を持ちます。これは library-first parse/load + checker floor であり、final textual grammar、full `specs/18` typed-checking completion、runtime plan execution、reusable runtime API、product-like command surface ではありません。
