...
- **Full System V1 source-first line**
  `specs/33..38`、`plan/58..63`、`progress.md`、`tasks.md` が current roadmap snapshot を担います。current promoted package は `P-PROJ-03 boundary schemas`、次の promoted package は `P-PROJ-04 server/client local split` です。ここでは Product Alpha-1 を final product に昇格せず、textual Mir、typed IR、bounded effectful runtime、PoseGraph runtime、bounded pose save/devtools、projection IR、provider admission へ順に進めます。
