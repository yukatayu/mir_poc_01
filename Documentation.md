...
- **Surface Mir alpha source-authority line**
  `specs/39..43` / `plan/64..68` で定義した new promoted roadmap。Canonical place-scope syntax は `S { ... }` であり、`S[ ... ]` は sugar としても採用しない。Surface Mir は user-facing source、Core Mir は elaboration target であり、通信・publish・observe は自動生成して Core IR / devtools に明示する。`package.mir.json` は alpha artifact に留まり、semantic source authority は `.mir` files に戻す。`P-SURF-00B` は docs/spec rebaseline であり、parser/runtime/helper implementation claim ではない。
...
- Surface Mir brace syntax / source-authority docs rebaseline, with `P-SURF-01 surface brace parser` as the next promoted package
...
- **Surface Mir alpha line**
  `specs/39..43`、`plan/64..68`、`progress.md`、`tasks.md` が current promoted Surface roadmap snapshot を担います。current package is `P-SURF-00B` docs/spec rebaseline, and next promoted package is `P-SURF-01 surface brace parser`. Canonical place-scope syntax is `S { ... }`; `S[ ... ]` is rejected and is not sugar.
...
- Surface Mir parser / checker / runtime / helper implementation beyond the current docs/spec rebaseline
