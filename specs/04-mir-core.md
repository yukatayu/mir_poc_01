...
- current L2 で `documented lineage annotation` に要求する最小 surface form は、ちょうど 1 本の fallback edge を飾る edge-local annotation である。option 単体の属性や global lineage ID ではなく、「どの predecessor option からどの successor option へ same-lineage continuation を主張しているか」を書く最小注記として扱う。
- その annotation が最低限持つべき情報は次の 3 つだけである。
  - predecessor option の参照
  - successor option の参照
  - successor が predecessor と同じ logical access path / semantic lineage を継続する、という affirmative claim
- 最終的な reserved keyword、punctuation、serialization は current L2 では固定しない。以下では説明用記法として `lineage(A -> B)` を使うが、これは最小 surface form の**例示**であって、規範的な最終 token ではない。
- `declared access target` は各 option に付く別の declared information であり、`documented lineage annotation` の内部へ埋め込んで置き換えてはならない。same-lineage の static 証拠として admissible なのは、predecessor / successor の双方が `declared access target` を持ち、かつ edge-local な `documented lineage annotation` がそのちょうど 1 本の edge を指している場合だけである。
- したがって annotation 単独では access path の anchor を欠くため足りず、`declared access target` 単独でも semantic lineage continuation の明示を欠くため足りない。current L2 はこの両方を要求する。
- current L2 は、option-local tag、chain-level blanket annotation、global lineage identifier のような追加書式を要求しない。将来 sugar として許す余地はあるが、最小 surface form には含めない。
...
- 以下の表記例も説明用であり、最終 surface syntax を固定しない。
...
- 上の `valid` は、edge-local annotation が実際の predecessor / successor edge を正しく指し、両端に `declared access target` があるため、same-lineage の最小 static 証拠として admissible である。
- 上の `malformed` は、annotation が自分の飾る edge ではなく別の successor を参照しており、declared information だけで same-lineage continuation の claim が壊れているため static rejection に寄せる。
- 上の `underdeclared` は、両端に `declared access target` はあるが edge-local annotation がないため、same-lineage continuation を証明できない。current L2 では surface-level static error である。
- **未決定**: `documented lineage annotation` の最終 keyword / punctuation / serialization をどうするか。また edge-local な最小 form から option-local / chain-level sugar をどこまで許すか。
- **未決定**: same-place / cross-place 文脈で同じ annotation shape をどこまで共有するか。
