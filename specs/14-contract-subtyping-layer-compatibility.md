...
## runtime-mirror evidence boundary

current alpha-local line では、runtime-sensitive positive rows の一部に限って
helper-local `runtime_mirror` evidence を使ってよい。

- これは alpha-local / helper-local / non-public mirror evidence である
- `expected_static.checked_reason_codes` や
  `expected_acceptance.checked_acceptance_rows` とは別 carrier として扱う
- mirror scope は `alpha-runtime-mirror-floor` に限る
- mirror は target sidecar の `runtime_mirror` と source runtime sidecar の
  `status` / `claims` / `current_validation.mode` / `mirrors` /
  `expected_runtime` を照合して判断する
- mirror は `VAR-*` sample 自体が parser/runtime で実行されたことを意味しない
- mirror は final public layer checker、final public layer-attachment ABI、
  production rate-limit/auth/redaction completion を意味しない

current runtime-mirror floor で admissible なのは次だけである。

- `VAR-08` rate-limit declared failure valid
  - source authority は `LI-04`
  - declared-failure attach + preview `Reject(RateLimited)` の mirror に限る
- `VAR-11` redaction layer valid
  - source authority は `LI-01`
  - accepted debug attach + redacted post-attach trace の mirror に限る
- `VAR-13` auth layer contract update valid
  - source authority は `LI-03`
  - explicit contract-update path の mirror に限る
