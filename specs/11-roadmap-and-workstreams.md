...
- phase6 request-clause-suite publicization threshold helper mirror:
  ...
  - Package 89 phase6 request-clause-suite publicization actualization close、Package 90 phase6 perform-head structural carrier actualization next
...
## 2026-04-27 Mirrorea future-runtime integration addendum

current active floor は引き続き clean near-end suite と Sugoroku world vertical slice である。
その上で、次の promoted line は Mirrorea future-axis の docs-first / repo-local package 列に置く。

- standard I/O を core primitive にしない
- external world を typed effect adapter で扱う
- authentication を transport に潰さない
- visualization / telemetry を typed / permissioned effect として扱う
- system-wide source から place-specific program へ projection できる性質を保つ
- runtime hot-plug と compatibility check を principal line に置く
...
P20. `mir-runtime` hot-plug orchestration skeleton first tranche
  - historical `P20` closeout memory。`crates/mir-runtime/src/hotplug_runtime.rs` に dedicated `HotPlugRuntimeSkeletonReport`、consumer-side `assemble_hotplug_runtime_skeleton_report()`、example `build_hotplug_runtime_skeleton_report()` を actualize し、admitted request/verdict carrier と existing substrate の thin runtime/report assembly を current line に actualize した。helper-local lifecycle / sample-grounded IDs / attach-detach view-telemetry IDs は preview ownership に残し、completed engine、rollback、durable migration、distributed activation ordering、final public hot-plug ABI は kept-later に残す。
R7. post-`P20` hot-plug next-package inventory
  - historical `R7` closeout memory。`plan/35-post-p20-hotplug-next-package-inventory.md` と companion docs により、post-`P20` kept-later lane を smallest plausible package cuts に分け、`P21` runtime-crate hot-plug completed-engine narrow cut を next narrow implementation line として固定した。package-level reopen next は exact label 未固定のまま `rollback / durable migration`、`distributed activation ordering`、`final public hot-plug ABI` の later family に grouped して残す。
P21. runtime-crate hot-plug completed-engine narrow cut
  - current close 済み。`HotPlugRuntimeEngineState` / `HotPlugRuntimeEngineReport`、consumer-side `assemble_hotplug_runtime_engine_report()`、example `build_hotplug_runtime_engine_report()` により、admitted request/verdict carrier と existing substrate の上に canonical runtime-side engine state progression を narrow に actualize した。rollback / durable migration / distributed activation ordering / final public hot-plug ABI は同じ tranche に混ぜない。
post-`P21` rollback / durable migration family
  - current self-driven first recommendation。exact package label は intentionally unfixed のまま保ち、`rollback / durable migration` family を keep-one-family vs split-again criteria つきで docs-first に harden し、`distributed activation ordering` は second recommendation、`final public hot-plug ABI` は post-`P18` mixed gate / `U1` hold line の third recommendation に残す。
post-`P21` distributed activation ordering family
  - current self-driven second recommendation close 済み。exact package label は intentionally unfixed のまま保ち、`activation_cut` widening criteria、multi-place / multi-server ordering、durable activation commit、transport-collapse stop line、public ABI deferred line を docs-first に harden した。current remaining third recommendation は final public hot-plug ABI である。
...
2026-04-29 時点で `P0`、`P1`、`P2`、`P3`、`P4`、`P5`、`P6`、`P7`、`P8`、`P9`、`P10`、`P12` current first cut と `P13` / `P14` / `P15` / `P16` / `P17` current first-cut closeout、`P19`、`P20`、`P21`、`R7` は close し、`P11` current third cut は actualize 済み、`P18` repo-side first cut も close 済みであり、post-`P21` later family のうち first recommendation `rollback / durable migration` と second recommendation `distributed activation ordering` は docs-first close 済み、current remaining third recommendation は final public hot-plug ABI である。

## 2026-04-27 phase-sample-progress and storage foundation addendum

current repo では、phase / layer ごとの進捗を prose だけでなく runnable sample で追う。
その current dashboard は `samples_progress.md` とし、phase 0〜16 について次を最低限そろえる。
...
進捗% は evidence-backed にする。

- `1%` = sample ID / goal が存在する
- `50%` = primary positive sample が通る
- `90%` = E2E / regression / closeout validation が通る
- `100%` = implementation + positive/negative + debug/visualization + docs + report + tests + `progress.md` / `tasks.md` / `samples_progress.md` 更新 + git commit/push

E2E は natural layer composition として成立しなければならない。
internal helper を順に呼ぶだけの thick fake wrapper を E2E として扱わない。

storage-sensitive work は、small VPS の root disk を build cache / LLVM / generated artifact で既成事実化しない。
external workdir が使える場合はそこへ heavy disposable artifact を置き、repo source と committed docs/report は detachable storage に閉じ込めない。

### phase sequence summary

0. repository memory and decision boundary
1. Mir current L2 semantics
2. parser-free PoC / detached validation loop
3. parser boundary and first checker cut
4. shared-space membership / room boundary
5. small decidable core and proof boundary
6. compile-ready minimal actualization
7. Sugoroku runtime attach
8. avatar fairy follow / fallback anchor
9. typed external boundary / adapter
10. MessageEnvelope / authentication seam
11. TermSignature / LayerSignature
12. projection / placement
