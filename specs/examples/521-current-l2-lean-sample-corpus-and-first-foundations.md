...
`representative theorem quartet actual Lean execution` 到達後の next closeout debt として、
...
1. `samples/lean/foundations/`
   ...
   - current package では
     ...
     を置く
2. `samples/lean/current-l2/`
   - representative current-L2 sample に対する generated Lean theorem stub
   - current package では
...
- `foundations/` は actual small proofs を含む。
  - これは IFC / label-model first fragment と proof-skeleton / obligation-shape first fragment を current mechanization-ready core として読むための floor である。
- `current-l2/` は generated Lean stub を含む。
...
さらに `samples/lean/current-l2/` により、
representative theorem quartet の current generated theorem text を repo 内に保存してよい。
...
- representative theorem quartet generated Lean corpus
