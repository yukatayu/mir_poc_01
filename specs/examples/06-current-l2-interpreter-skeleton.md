...
- E1 direct-target ensure failure variant
  - `e10-perform-on-ensure-failure`
    - direct `PerformOn` で effect が success-side carrier を返しても、request-local `ensure` が unsatisfied なら tentative commit を適用せず `explicit_failure` に落ちる
    - この分岐では `Reject` や formal `non_admissible_metadata` を fabricated しない
- E3 via-chain ensure continuation variant
  - `e11-perform-via-ensure-then-success`
    - admitted option が success-side carrier を返しても request-local `ensure` が unsatisfied なら tentative commit を破棄し `perform-failure` を記録する
    - later same-lineage write-capable option が残っていれば via-chain evaluation を継続し、後段 success へ到達しうる
