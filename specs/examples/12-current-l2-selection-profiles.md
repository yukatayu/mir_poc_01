...
### profile helper が public behavior として持つもの

- `SelectionRequest` / `SelectionProfile` の carrier
- primitive selection の逐次合成
- `profile_name`
- selected counts と selected bundle shape を含む profile summary

### profile helper が thin delegation に留めるもの
...
- human-friendly alias の提示や alias 解決
...
profile helper は、selected batch 実行を下位 helper に委譲した結果として、少なくとも次を exact compare 済みで扱ってよい。
