...
provider placement の practical cut を current phase で 1 段整理した後の次 narrow step は、**control-plane separated causal carrier を authoritative room side line に reopen する threshold** を比べることであり、その threshold judgment 自体は `specs/examples/125-shared-space-control-plane-carrier-threshold.md` に切り出した。
