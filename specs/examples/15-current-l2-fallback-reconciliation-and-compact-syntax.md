...
explicit edge-row family の内部比較で A2 を first choice にしたあと、次に決めるべきなのは「その判断をどこまで mirror へ広げるか」である。current L2 では次の 3 境界だけを比べれば足りる。

### 1. docs-only 維持

- `specs/examples/01` と `specs/examples/15` だけで A2 first choice を述べ、`specs/examples/00` の representative code block は A1 / A2 混在のまま残す。
- 利点:
  - mirror 更新コストは最小である。
  - A1 shorthand を実例で見せ続けられる。
- 欠点:
  - representative examples 本体を先に読む読者には、A1 が依然として first-choice rendering に見えやすい。
  - outer / inner wrapper 誤読を prose だけで抑える形になり、A2 を選んだ価値が code sample に現れにくい。

### 2. limited rollout

- `specs/examples/01` と `specs/examples/15` の判断を保ったまま、`specs/examples/00` のうち fallback / preference chain 自体を主題にする representative code block を A2 first choice へ揃える。
- current repo では少なくとも E3、E3 variant、E4、E6 がこの対象に入る。
- E7 / E8 は parser-free fixture と `specs/examples/15` の比較材料としては使うが、representative programs 本体へ無理に昇格させない。
- 利点:
  - edge-local lineage と request-evaluation boundary の見せ方を、読者が最初に触れる representative code block で揃えられる。
  - rollback / cut / `Reject` 境界を誤読させにくい rendering を、chain-heavy な例にだけ集中して出せる。
  - fixture schema、interpreter、host harness、tests を触らずに済む。
- 欠点:
  - A1 shorthand の実例は `specs/examples/01` の最小 snippet へ寄り、representative examples 本体からは減る。

### 3. broader rollout

- A2 を representative examples 全体だけでなく、fixture catalog、PoC helper docs、さらには fixture JSON / tests の表示 mirror にまで広げる。
- 利点:
  - rendering の統一感は最も高い。
- 欠点:
  - notation rollout の task 境界を越えやすく、machine-check surface まで「書式の都合」で触る圧が強くなる。
  - fixture / test は current L2 では meaning carrier と public behavior coverage が主目的であり、companion notation の polished rendering と同期させる必然が弱い。

### rollout の暫定判断

current L2 では **limited rollout を採るのが最小で自然**である。

- A2 は docs-only の比較候補ではなく、representative examples の first-choice rendering へ広げてよい。
- ただし広げる先は fallback / preference chain を主題にする representative code block に限る。
- E7 / E8 は machine-check と comparison prose の根拠として十分であり、今回それ自体を representative programs 本体へ追加する必要はない。
- fixture JSON、host plan sidecar、interpreter、tests は notation rollout の対象外に留める。

この判断により、current L2 companion notation の中心は次のように揃う。

- `specs/examples/01`
  - A2 を first choice、A1 を shorthand と定義する。
- `specs/examples/15`
  - A2 / A1 / A3 の比較と rollout boundary を説明する。
- `specs/examples/00`
  - fallback / preference chain を主題にする representative code block は A2 で示す。
- parser-free PoC fixture / tests
  - machine-check surface を保ち、notation rollout では触れない。
...
- A2 の rollout は representative examples のうち chain-heavy な code block までに留め、fixture schema / interpreter / tests には広げない。
