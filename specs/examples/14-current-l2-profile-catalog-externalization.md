...
- 比較対象は `specs/examples/13-current-l2-profile-catalog.md` に列挙した current L2 named profile alias set である。
...
- `crates/mir-semantics/src/harness.rs` の hard-coded preset table から `ProfileCatalog::aliases()` と `ProfileCatalog::resolve()` を導く。
- alias は既存 `SelectionRequest` / `SelectionProfile` に直接解決する。
- tests は machine-check に必要な public behavior だけを検証し、docs の具体的な alias 列挙は `specs/examples/13-current-l2-profile-catalog.md` に寄せる。
...
- current L2 では、`specs/examples/13-current-l2-profile-catalog.md` に列挙した current alias set の規模なら hard-coded table のままで十分である。
