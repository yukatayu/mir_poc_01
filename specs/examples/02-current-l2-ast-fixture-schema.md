...
- optional `checked_reason_codes`
  - stable static malformed / underdeclared cluster に限った additive optional typed carrier
  - current harness が present 時だけ fail-closed compare してよい
- optional `checked_try_rollback_structural_verdict`
  - dedicated `TryFallback` / `AtomicCut` AST structural helper first tranche の helper-local verdict carrier
  - current enum は `no_findings` / `findings_present`
  - `expected_static.verdict` を置き換えず、`run_bundle()` machine-check core にも上げない
- optional `checked_try_rollback_structural_findings`
  - dedicated `TryFallback` / `AtomicCut` AST structural helper first tranche の helper-local finding row list
  - current row shape は `subject_kind` / `finding_kind`
  - current first tranche で actualize 済みなのは
    - `TryFallback` / `missing_fallback_body`
    - `AtomicCut` / `disallowed_fallback_placement`
  - detached validation loop では static gate artifact loop の helper-local smoke family にだけ接続し、shared detached carrier や public checker API にはまだ上げない
...
current L2 の最小 fixture set は次の 24 本とする。
...
| `e15-malformed-duplicate-chain-declaration.json` | E15 | visible chain 名の duplicate による malformed static stop |
| `e16-malformed-missing-chain-head-option.json` | E16 | visible chain head option 欠落による malformed static stop |
| `e17-malformed-missing-predecessor-option.json` | E17 | visible predecessor option 欠落による malformed static stop |
| `e18-malformed-missing-successor-option.json` | E18 | visible successor option 欠落による malformed static stop |
| `e19-malformed-target-mismatch.json` | E19 | declared access target mismatch による malformed static stop |
| `e20-malformed-late-capability-strengthening.json` | E13 late-edge variant | earlier read-to-read continuation の後に later read-to-write escalation を置いた malformed static stop |
| `e23-malformed-try-fallback-missing-fallback-body.json` | E23 | empty `fallback_body` を `missing_fallback_body` として読む dedicated try/rollback structural helper first tranche |
| `e24-malformed-atomic-cut-fallback-placement.json` | E24 | `fallback_body` 内 `AtomicCut` を `disallowed_fallback_placement` として読む dedicated try/rollback structural helper first tranche |
