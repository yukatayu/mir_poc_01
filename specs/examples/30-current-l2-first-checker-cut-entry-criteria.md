...
- `AtomicCut` event の存在と frontier update の runtime gate を区別して読むこと
...
- `place_anchor == current_place` の dynamic gate に依存する frontier update 成否
- frontier update 後の restore scope が whole-store snapshot か place-local snapshot か、という runtime/proof boundary
