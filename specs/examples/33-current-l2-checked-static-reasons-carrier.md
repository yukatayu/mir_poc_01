...
## initial adoption candidate

current fixture corpus で最初に `checked_reasons` を採用しやすいのは、
actual static gate wording が 1 本に安定しており、fixture 側 `reasons` を explanation として残したい
static-only fixture である。
...
- `checked_reasons`
  - actual static gate wording の fail-closed bridge

という split が自然に成立する。

逆に、current valid fixture 群の多くでは actual static gate `reasons` が空であり、
fixture 側 `reasons` だけが explanatory prose を担っている。
この場合は `checked_reasons = []` を機械的に足すより、field を absent のままにして
`verdict` only machine-check を維持する方が自然である。
...
- detached static gate artifact にある helper-local / reference-only `reason_codes` mirror を、first-class typed source に昇格させるか
...
- initial corpus adoption は `e4` / `e5` のような static-only malformed / underdeclared fixture から始めるのが自然であり、current corpus では `e12` / `e13` まで stable cluster を広げてよい
- actual static gate `reasons` が空な valid fixture では、`checked_reasons` を無理に `[]` で足さず absent のままにする方が current L2 には合う
- detached static gate artifact 側に helper-local / reference-only `reason_codes` mirror を持たせてもよいが、これは `checked_reasons` を置き換える typed carrier ではない
