...
- `LIF-11`
  requires anchor/deletion outcome semantics
  - inherited access path が upstream anchor disappearance 後にどう degrade するか
  - deleted object lifetime extension ではなく inherited access-path outcome として扱う
- `LIF-13`
  requires selected-option snapshot semantics
  - snapshot は current selected option だけを capture する
  - full chain hidden inheritance や object-identity merge として扱わない
- `LIF-15`
...
`LIF-11/13/15` を helper-local acceptance row、runtime-mirror row、
