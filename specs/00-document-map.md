...
- `docs/research_abstract/`
  ...
  - `surface_mir_alpha_01.md` は、Surface Mir brace/source-authority rebaseline、P-SURF-01 parser floor、P-SURF-02 indexed-state semantic checker floor、P-SURF-03 Surface-to-Core elaboration evidence floor、P-SURF-04 generated communication evidence floor、P-SURF-05 role admission evidence floor、P-SURF-06 source patch hot-plug evidence floor、P-SURF-07 source operational evidence floor、P-SURF-08 static devtools diagnostics evidence floor、P-SURF-99 final audit closeout、canonical `S { ... }`、no `S[ ... ]` sugar、indexed state、role admission、source patch hot-plug を reader-facing に短く読むための summary である。
  ...
  - `avatar_fairy_follow_plan_01.md` は、phase 8 active representative slice / residual planned family / historical prototype の境界を reader-facing に読む summary である。
...
- `docs/hands_on/`
  ...
  - `operational_package_authoring_01.md` は、`templates/world-core-starter/`、`membership-chat-starter/`、`sugoroku-world-starter/` から bounded operational package を author / check / run-local するための landing page である。
  ...
  - `surface_mir_alpha_01.md` は、Surface Mir alpha source-authority roadmap、canonical `S { ... }` place-scope syntax、P-SURF-01 parser commands、P-SURF-02 indexed-state checker commands、P-SURF-03/P-SURF-04 elaboration/generated-communication commands、P-SURF-05 role admission commands、P-SURF-06 source patch commands、P-SURF-07 source operational commands、P-SURF-08 static devtools diagnostics commands、P-SURF-99 audit closeout、stop lines を読む landing page である。
  - `source_patch_hotplug_01.md` は、Surface source patch hot-plug を direct eval ではなく parse / typecheck / elaborate / admit / activation_cut pipeline として確認する landing page である。
...
- `faq_011.md`
  - 2026-04-20 時点の current explanation refresh であり、
    `faq_010.md` 以後の actual progress を反映した上で、
    「どこまで何が終わっているか」
    「二大問題を completely solved / language implementation complete と読めるか」
    「全体像に対して今どこにいるか」
    「何を答えればどこまで自走できるか」
...
- `specs/40-indexed-state-semantics.md`
  ...
  - key は owner/authority ではなく、join/leave/tombstone/compaction/save-load constraints と nested-place request boundary を整理する。
...
- `specs/examples/522-current-l2-ifc-secret-valid-invalid-foundation-and-japanese-lean-corpus-sync.md`
  - Package 56 の reopened IFC line を `CurrentL2IfcSecretExamples.lean` まで進め、secret-key valid/invalid concrete example と、日本語 Lean corpus explanation の `foundations/` / active `clean-near-end/` / archived old corpus split を current mechanization-ready floor として整理する補助文書。
...
- `specs/examples/523-current-l2-source-side-ifc-authority-prototype-pair-and-representative-lean-sample-set-widening.md`
  - Package 56 の source-side explicit authority lineについて、historical prototype anchor `p10 / p11` と current clean-near-end compare floor `01 / 02` を切り分け、checker-adjacent IFC line の authority-success / authority-miss memory を整理する補助文書。
  - final typed source principal や final IFC syntax や final public verifier contract を固定するものではなく、source-side IFC authority pair の archived/current split を読むための anchor である。
...
- `specs/examples/566-current-l2-finite-index-first-layer-capture-lifetime-cost-actualization.md`
  - historical Phase 5 / Package 92 closeout memory として、pre-clean-near-end prototype anchor `p15 / p16` と current clean-near-end typing compare floor `04 / 05` を切り分け、finite-index first strong typing line の capture / lifetime / simple cost current readingを helper-local checker summary とともに整理する補助文書。
- `specs/examples/567-current-l2-lean-first-formal-skeleton-hardening-after-finite-index-widening.md`
  - historical Phase 5 / Package 93 closeout memory として、`samples/lean/foundations/`、active `samples/lean/clean-near-end/` generated stub corpus、archived `samples/lean/old/2026-04-22-pre-clean-near-end/current-l2/` appendix の役割差を保ったまま、`CurrentL2FiniteIndexFirstLayer.lean` と `p15 / p16` widening memory を整理する補助文書。
...
- `specs/examples/602-current-l2-authoritative-room-runnable-scenario-loop-hardening.md`
  - historical Phase 6 / Package 129 closeout memory として、historical `emit-scenario problem2` helper memory と Problem 2 bundle doc 側の runnable-scenario step memory を保持し、representative pair `p07 / p08`、reserve route `p09`、negative pair `p13 / p14` を repo-local output dir に materialize していた scenario-loop memory として narrow に読む補助文書。
- `specs/examples/603-current-l2-problem1-executable-residual-reopen-sync.md`
  - historical Phase 6 / Package 130 closeout memory として、historical `emit-theorem problem1` reopen-order memory と `reopen-map problem1` / `lane problem1-final-public-seams` / Problem 1 bundle doc 側の historical doc memory を保持し、`check-source-sample` → `emit-theorem problem1` → lane summary の historical reopen order を narrow に読む補助文書。
- `specs/examples/604-current-l2-problem2-executable-residual-reopen-sync.md`
  - historical Phase 6 / Package 131 closeout memory として、historical `emit-scenario problem2` reopen-order memory と `reopen-map problem2` / `lane problem2-final-public-seams` / `residuals` / Problem 2 bundle doc 側の historical doc memory を保持し、`emit-scenario problem2` → lane summary → residual summary の historical reopen order を narrow に読む補助文書。
...
- `specs/examples/606-current-l2-reserve-integration-entrypoint-summary-sync.md`
  - historical Phase 6 / Package 133 closeout memory として、historical `reserve` helper memory を保持し、once-through closeout summary 後の theorem-first external pilot / `auditable_authority_witness` / `delegated_rng_service` / model-check second-line reserve を helper-local reopen summary memory として narrow に読む補助文書。
...
- `specs/examples/609-current-l2-theorem-first-external-pilot-summary-index-actualization.md`
  - historical Phase 6 / reserve package closeout memory として、historical `emit-theorem problem1` output dir memory に `pilot-summary.md` / `pilot-summary.json` を保持し、theorem-first external pilot を notebook-first summary-index memory として narrow に読む補助文書。
- `specs/examples/610-current-l2-auditable-authority-witness-reserve-package-summary-index-actualization.md`
  - historical Phase 6 / reserve package closeout memory として、historical `emit-reserve auditable-authority-witness` と `target/current-l2-guided/reserve-packages/auditable-authority-witness/package-summary.md|json` memory を保持し、`auditable_authority_witness` package を `p07 / p08 / p05` 単独 summary-index memory として narrow に読む補助文書。
- `specs/examples/611-current-l2-delegated-rng-service-reserve-package-summary-index-actualization.md`
  - historical Phase 6 / reserve package closeout memory として、historical `emit-reserve delegated-rng-service` と `target/current-l2-guided/reserve-packages/delegated-rng-service/package-summary.md|json` memory を保持し、`delegated_rng_service` package を `p09 / p07 / p08` 単独 summary-index memory として narrow に読む補助文書。
