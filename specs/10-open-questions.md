...
- ただし step signal 名、predicate / effect oracle の最小 API、admissible option の explicit failure が chain exhaustion したときの exact carrier detail、`Approximate` / `Compensate` を parser-free 最小 interpreter の step semantics にどの時点で織り込むかは **未決定** である。
