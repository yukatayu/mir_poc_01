...
- next open cluster は次である。
  ...
  3. `MessageEnvelope` / `AuthEvidence` / transport insertion seam の最小 shape をどう置くか。
     ...
     - 2026-04-28 時点で `P6` `MessageEnvelope / AuthEvidence` seam hardening は current line に上げてよい。
       helper `message_envelope_scope = representative_slice`、runtime `message_envelope_scope = clean_near_end_canonical_inventory`、
       `transport_medium` / `transport_seam` split、`emitter_principal`、`freshness_checks`、
       shared `auth_evidence_lanes = kind / subject / issuer / bindings / notes`、
       helper active `transport_mediums = local_queue / loopback_socket`、runtime canonical `transport_seams = provider_boundary / audit_trace_boundary`
       までは current line に上げてよい。
     - 未決なのは final public `AuthEvidence` kind、session / signature protocol、real network transport への widening、
  ...
  5. projection / placement mobility をどこまで docs-first に固定するか。
     ...
     - 2026-04-27 時点で helper/report-local preview floor までは actualize した。
       `plan/20-projection-and-placement-roadmap.md` により、system-wide source / place-specific program distinction、
       server / participant / adapter / visualizer split、validity checklist、helper/report-local preview floor、stop line までは current line に上げてよい。
       Sugoroku helper `projection_view` と clean near-end runtime report-local `cross_place_projection` により、
       place split と provider placement inventory を evidence-oriented に読む current floor も actualize してよい。
     - 未決なのは final projection IR、generator、optimizer、equivalence checker、emitted place program public contract である。
  ...
  6. hot-plug `Patch Req Prov Δ` / `AttachPoint` の最小 surface と compatibility check をどこまで current line に上げるか。
     ...
     - 2026-04-28 時点で `R2` docs-first closeout として、
       ...
       までは current line に上げてよい。
     - 2026-04-28 時点の `R4` docs-first closeout として、
       `plan/32-hotplug-real-migration-rollback-boundary.md` により
       `activation_cut != distributed_activation_ordering`、
       ...
       network replay / storage detach / runtime detach lifecycle は collapse しない、
     ...
     - 2026-04-28 時点の `R5` docs-first closeout として、
       ...
       という line までは current docs に上げてよい。
     - 2026-04-28 時点の `R6` docs-first closeout として、
       `plan/34-runtime-crate-hotplug-carrier-admission-cut.md` により
     ...
     - historical recommendation at `R7` close time は、`P21` runtime-crate hot-plug completed-engine narrow cut を next narrow implementation line に固定し、exact later package label はまだ fixed せずに `rollback / durable migration`、`distributed activation ordering`、`final public hot-plug ABI` の later family に grouped して残すことであった。current repo state では `P21` も close 済みであり、exact next label は intentionally unfixed のまま later family grouping に戻している。current self-driven first recommendation は `rollback / durable migration` family hardening である。
  ...
  8. `P18` public API / parser grammar gate で、repo-side inventory と actual final freeze の境界をどこまで current docs に固定するか。
     ...
     - post-`P18` hold line の option inventory としては、first shipped public surface を `parser/checker/runtime/verifier first`、`adapter/viewer/projection/hot-plug first`、`two-step split` で比較してよい。ただし actual final freeze は packaging / host target choice の後に残す。
...
43. public operational CLI installed-binary promotion と packaging success criteria をどこで mixed gate と読むか。
   ...
   - 2026-04-27 時点で docs-first backend/LLVM guardrail は `plan/23-compiler-backend-llvm-guardrail-roadmap.md` に actualize した。mounted external workdir、`CARGO_TARGET_DIR` / `CARGO_HOME` binding、LLVM path readiness、non-destructive probe floor までは current line に上げてよい。
   - option inventory として、packaging shape は `CLI / library / engine-adapter / hybrid`、host integration target は `browser / native process / engine / mixed` で比較してよい。
