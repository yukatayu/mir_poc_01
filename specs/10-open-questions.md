...
48. final-layer closeout queue をどこまで actual package として戻すか。
   ...
   - 未決なのは、secret-key / IFC valid-invalid / authority / label-flow の source-side trioを beyond first checker fragment へどこまで広げるか、checker-hint / diagnostics 側へどこまで mirror するか、delegated provider carry-over と CLI surface preview の先で broader theorem/model-check/order-handoff corpus widening をどこで止めるかである。
