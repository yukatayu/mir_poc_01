...
- `P11` logical multi-place runtime tranche は current active package であり、`MembershipRegistry`、`PlaceCatalog`、participant-place-kind-gated `LogicalPlaceRuntimeShell` が `crates/mirrorea-core` に actualize され、`WorldState` / `PlaceRuntime` / `MessageQueue` / `SugorokuState` は kept-later boundary に残っていること
