...
`SUG-01` と `SUG-09` の副読本です。`run 01_runtime_attach_game --debug hotplug` と
`run 09_detach_todo --debug hotplug` を使うと、`AttachPoint[SugorokuGame#1]`、
compatibility、activation cut、`detach_request#1`、post-detach rejection、migration deferred を
...
- `Phase 14 hot-plug / detach`
  - `SUG-01` と `SUG-09` で helper-local lifecycle canary を actualize したが、`SUG-09` は preview / stop-line 用であり completion evidence ではない
