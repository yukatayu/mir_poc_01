...
までは external-contract-facing handoff row として足してよい。ただし actual external contract は still 後段に残す。
...
- external-contract-facing handoff row の次段として actual external contract をどの field で切るか
