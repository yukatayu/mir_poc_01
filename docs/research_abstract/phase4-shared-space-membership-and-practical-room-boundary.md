...
その後 `specs/examples/365...366` により identity / auth layering reopen、`specs/examples/373...374` により admission / compile-time visibility reopen、`specs/examples/375...376` により authority / resource ownership reopen も docs-first boundary package として fixed 済みであり、shared-space docs-first follow-up checkpoint はここで一旦 close と読むのが current reading である。shared-space line はここから先、model-check / host-facing / room-profile note と並走する adjacent boundary line として扱うのが自然である。
