...
その後 `specs/examples/365...366` により identity / auth layering reopen、`specs/examples/373...374` により admission / compile-time visibility reopen、`specs/examples/375...376` により authority / resource ownership reopen、`specs/examples/424` により room-profile / host-binding bridge-only note、`specs/examples/432` により fairness / replay strengthening reserve note も docs-first boundary package として fixed 済みであり、shared-space docs-first follow-up checkpoint はここで一旦 close と読むのが current reading である。shared-space line はここから先、final operational catalog / fairness operational profile later gate を残す adjacent boundary line として扱うのが自然である。
