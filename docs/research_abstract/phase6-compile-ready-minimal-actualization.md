...
- source target only row `e3` / `e21` の authored-set widen timing
...
actual parser-to-`Program` lowering first cut も fixed 済みであり、`mir_runtime::current_l2::lower_current_l2_fixed_source_text` を helper-local lowerer に置き、current authored quartet `e1` / `e2` / `e4` / `e23` を semantic `Program` + optional stage 1 / stage 2 bridge evidence へ fail-closed に落とす current cut を採った。
...
- widened authored-row order は `e1 -> e21 -> e3` に置き、`e1` は actual authored row として閉じ、`e3` は admit-family / theorem-side guard を伴う third slot に残す
