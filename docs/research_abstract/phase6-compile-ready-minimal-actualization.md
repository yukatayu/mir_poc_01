...
actual parser-to-`Program` lowering first cut も fixed 済みであり、`mir_runtime::current_l2::lower_current_l2_fixed_source_text` を helper-local lowerer に置き、current authored quintet `e1` / `e2` / `e21` / `e4` / `e23` を semantic `Program` + optional stage 1 / stage 2 bridge evidence へ fail-closed に落とす current cut を採った。
