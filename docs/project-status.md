...
| 研究 lifecycle | T-RESEARCH-001..033 は pre-delegation evidence。WRK-0001 は theory/02 の有限 index 許容範囲を existing Lean fragment で再現する committed L3 pre-registration であり、結果は未実行。L2 selection は将来の owner trust configuration まで fail-closed である。 | `mirrorea_canon/working/WRK-0001-finite-index-boundaries.md`; `plan/156-t0-t2-research-autonomy-envelope.md`; `plan/158-standing-bounded-autonomy.md`; `plan/159-wrk-evidence-commit-integrity-recut.md` |
