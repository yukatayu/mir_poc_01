...
| Consistent-cut literal boundary | WRK-0023 は theory/04 の event-only cut closure と `send -> receive` から sent-event membership が遡ることを scratch Lean で確認した。表示済み定義に channel-state の表現関係は含まれない。これは checkpoint/state carrier、checker、OBL の選択・反証ではない。 | `plan/wrk-0023-consistent-cut-channel-state-boundary.md`; `docs/reports/2429-wrk0023-consistent-cut-evidence.md` |
