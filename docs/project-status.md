...
| semantic kernel | P004/008/012/013/015 の方向は記録済み。WRK-0028--0035 は各々の限定結果を保持し、Plan 204 は fixed line の no-candidate を保持する。Plan 206 は個別 C7 factorization の common coarsening failure を示す countermodel を pre-registration 候補に選別した。C1/C6 も既存の限定 evidence に留まる | C7 は source rule でなく L3 boundary。C7-CUM-PRE の登録後検査以外、C3/C4/C5 本体と C0-D/C1/C2-B/C6 は semantic selection 又は duplicate-evidence risk のため deferred |
...
- Oracle の independent review と local source check により、WRK-0028 は common
  ...
  を要する。Plan 204 は同一 fixed-presentation line を scoped `no-candidate` とし、Plan 205 から WRK-0035 は C7 factorization criterion の range-only L3 evidence を retained した。Plan 206 は個別 C7 check の common coarsening failure を L3 pre-registration 候補に選別した。
