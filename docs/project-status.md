...
| 5 | value / occurrence | P017 X1 relation-state envelope | X1 は記録済み。WRK-0040 は V1/R1 cross-locus read の五つの collapse を predicate-only で検出するために登録済み・未実行。unique pending binding、owner-service/receipt-use 分離、authority、save/load、observation の positive schema/rule/proof は未選択 |
