...
| Consistent-cut literal boundary | theory/04 の event-only cut closure と `send -> receive` から sent-event membership を導くか、及び `channel state carries it` を代替とする表示済み表現関係があるかを WRK-0023 が登録後に検査する。checkpoint carrier、checker、OBL は選ばない。 | `mirrorea_canon/working/WRK-0023-consistent-cut-channel-state-boundary.md`; `docs/reports/2428-wrk0023-consistent-cut-registration.md` |
