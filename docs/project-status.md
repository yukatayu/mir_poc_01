...
| semantic kernel | P004/008/012/013/015 の方向は記録済み。WRK-0028--0037 は限定結果を保持し、Plan 213 は全十セルの fiberwise relational comparison だけを次候補にした | WRK-0038 は未実行の scope correction 対象。carrier は選ばず、A/B の identity/pending/receipt/save-load は owner/Canon selection を待つ。C4/C5 と C0-D/C1/C6 は deferred |
...
| 5 | value / occurrence | P012 V1/R1/SW1/conditional A2 の pending/facet/correlation | Plan 213 は B2-OPAQUE と同じ有限表を全十 supplied key ごとに relation-first presentation と比較する候補のみを選んだ。pending/reply/receipt/load の Canon carrier は選ばない。C3/C4/C5 本体は ordinary Canon design boundary |
