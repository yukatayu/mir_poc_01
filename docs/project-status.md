...
W4では公開状態と各processの使用可能状態の対応を先に検査しています。外部の非productionモデルで全値の一致・既存Session遷移・検査から使用までの区間を一般証明し、実parser由来の17文を局所モデルで確認しました。W3追試と既存実process/QUIC46件も通過。最新のfresh検査では92moduleの全所有宣言・source/反例検査が通過。Oracle3の指摘を受け、公開手順からの待機結果の代替計算を拒否する規則も機械検証しました。第5回Oracleまで回収し、呼出し先だけを変える候補の不足を確認しました。外部の後続候補では、実sourceの明示配置を参照の別名・再取得・解放・取消しへ保持し、17文と4結果を局所モデルで検査しました。一般のSession遷移でsource区分とcaller識別を保存しますが、退出したcallerからの開始を防ぐ現在性・実process所有の条件は別途必要です。第6回Oracleは回収済みで、元sourceの宣言を変換後の宣言で補ってしまう不具合を実際に再現し、修正しました。外部候補では、全入口での残余sourceの型・待機位置・結果変数の新鮮性と、有限ownerデータへの変換による検査結果の一致も一般証明しました。第7回Oracleでこの差分と物理接続候補をreview中です。実owner・配置・個別process状態との対応と新しい実network証拠は未完了です。W5以降・Plan250/I3-4は未開始、正式THM/OBL受理も未更新です。
