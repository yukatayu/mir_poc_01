...
| Cross-boundary claim integrity | 定理・義務・境界 ID の参照は台帳／registry と整合し、台帳外の proof-complete claim はない。存在 DAG、patch DAG、stream fallback の THM-002 接続は、現時点では後段 package が carrier/relation を選ぶべき方向であり、証明済み結果ではない。SCN-04 compaction と BND-006 は後段の operational constraint であって core primitive を要請しない。 | Report 2285; `plan/156-t0-t2-research-autonomy-envelope.md` |
...
| Surface v0 grammar closure | owner record pending | PROPOSAL-004 の A/B/C。current recommendation は A: Participant-only の既存語彙閉包であり、custom keyspace は later に残す。採択前に parser/AST/keyword を実装しない。 |
