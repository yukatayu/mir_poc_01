...
| WRK-0022 correction | pushed registration 後の bare `lean` command が `samples` module prefix を解決できず first source compile で `frozen`; source は復元済みで、failure-row / determinism / OBL の結論ではない。 | `plan/wrk-0022-write-cross-failure-generation-boundary-falsifier.md` |
