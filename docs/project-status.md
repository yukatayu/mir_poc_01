...
- `PROPOSAL-003`、`PROPOSAL-004`、`PROPOSAL-008`、`PROPOSAL-009`、`PROPOSAL-010`、`PROPOSAL-011`、`PROPOSAL-012` は owner/canon
...
| OBL-001 Core/write correspondence; elaboration/trace closure | PROPOSAL-009 / PROPOSAL-012 pending | 将来の proof-facing package は THM-001 の既存 `c` write 条件を直接表明するか、同条件を導く明示 correspondence を持つ output view にするかを選ぶ。別に `V` (read 値フロー)、`R` (成功 read 受領)、`S` (成功 write service)、`A` (admission) の semantic family を PROPOSAL-012 で選ぶまで、`G_e` dependency row 以外の runtime mapping を証明モデルに仮定しない。 |
