...
| 観点 | 状態 | 根拠 |
| --- | --- | --- |
| active frontier | **M5 shared formal model/metatheory**。次は M6 Surface | `plan/247-mir-theory-v0-i1plus-current-roadmap.md` |
...
| proof / scenarios | General OBL-001..028 は `intentionally-deferred`; finite M3 OBL-029..032 は `lean-proved`、033 は `model-checked-bounded`、034 は `runtime-monitored`; finite M4 OBL-035..039 は `lean-proved`; SCN-01..10 official status は不変、SCN-11/12 は milestone pressure scenario | `mirrorea_canon/theory/11-metatheory-ledger.md` |
| M0--M4 close/non-effect | M4 は owner-held relation、consumer-local coherent projection、semantic/presentation fallback 分離、fresh reacquire、privacy/split-frame boundaryを有限 evidenceで閉じた。general relation theorem、M6 Surface、M8 runtime、general save/load/patch、official conformance、I1、public API/ABI/wire、production deployment は動かさない | `docs/reports/2581-mir-theory-v0-i1plus-milestone-0-bootstrap.md`, `docs/reports/2582-mir-theory-v0-i1plus-milestone-1-constitution.md`, `docs/reports/2583-mir-theory-v0-i1plus-milestone-2-t0-g0-semantic-assertions.md`, `docs/reports/2584-mir-theory-v0-i1plus-milestone-3-evaluation-materialization.md`, `docs/reports/2585-mir-theory-v0-i1plus-milestone-4-maintained-relation-projection.md` |
...
有限 relation calculusを、trusted Lean OBL-035..039、13 focused Rust tests、full
`mir-semantics` test、test-target clippy、Oracle advisory、independent reviewで閉じた。M5 の
direct blocker は、M1--M4 の evidence を一つの non-opaque shared modelへ対応付けることであり、
direct consumer は M6 Surface である。

## 現在の停止線

- M5 は M1--M4 の Surface fragment、Core、Config、Step、WellFormed、Trace、Diagnostic、
  relation state、cut/save interfaceを一つの non-opaque modelへ統合するまで、M6 grammarや
  checker/runtimeを開始しない。`plan/247-mir-theory-v0-i1plus-current-roadmap.md`
...
現在の M5 を止める owner decision はない。以下だけが escalation 条件であり、通常の
