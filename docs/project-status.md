...
### WRK-0022 outcome correction

WRK-0022 は未実行ではない。pushed registration 後の登録済み bare `lean`
command が `samples` module prefix を解決できず、最初の source compile で
`frozen` になった。追加 source は復元済みであり、この結果は failure-row、
elaboration determinism、OBL の結論ではない。上の論理仕様行に残る未実行記述は
この correction により置換される。`plan/wrk-0022-write-cross-failure-generation-boundary-falsifier.md`
...
- `PROPOSAL-003`、`PROPOSAL-004`、`PROPOSAL-008`、`PROPOSAL-010`、`PROPOSAL-011`、`PROPOSAL-012` は owner/canon
