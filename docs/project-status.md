...
| 実装 / 運用 | alpha / Surface / operational roots は限定 LAB evidence。Full System V1 helper は accepted/rejected の内側 CLI 終了コード整合を確認し、committed provider/renderer generated evidence は実行時に書き換えず fresh report と比較する。一方、reviewer-found の operation-capability binding、duplicate record field、composite equality の三つの未テスト checker gap は次の bounded LAB maintenance package で修正するまで依拠しない。release bundle は C-distributed conformance / 実 transport / multi-process execution を非主張とする。 | `samples_progress.md`; `scripts/README.md`; `tasks.md`; `mirrorea_canon/plan/01-phases.md` |
