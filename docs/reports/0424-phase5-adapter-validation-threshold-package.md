# 0424 — phase5 adapter-validation threshold package

## Objective

Phase 5 theorem-line later package の次段として、exchange-ready retained bridge に `adapter_validation_ref` をどこまで current first choice として足してよいかを比較し、environment-specific invocation surface をどこまで後段に残すかを docs-first で固定する。

## Scope and assumptions

- 対象は `proof_notebook` first bridge に限る。
- current 前提は `specs/examples/153-current-l2-theorem-line-adapter-linked-exchange-rule-threshold.md` までである。
- current task では actual notebook exchange rule body、concrete file / blob exchange protocol、environment-specific consumer invocation surface は固定しない。
...
1. `specs/examples/154-current-l2-theorem-line-exchange-ready-adapter-validation-threshold.md` を追加し、
   - exchange-ready retained bridge を terminal cut にする案
   - `adapter_validation_ref` だけを足す案
   - `adapter_validation_ref + consumer_invocation_surface_ref` をまとめて足す案
   を比較した。
2. current judgment として、exchange-ready retained bridge に `adapter_validation_ref` だけを足し、environment-specific invocation surface / concrete runtime coupling は still 後段に残す cut を採用した。
...
4. next later reopen を `adapter-validation threshold` から `invocation-surface threshold` へ差し替えた。
...
- resource check:
  - `df -h .` → `/dev/vda2 99G / 92G used / 2.8G avail / 98%`
  - `free -h` → `Mem 960Mi / 775Mi used / 65Mi free / 184Mi available`
- timestamp:
  - `date '+%Y-%m-%d %H:%M %Z'` → `2026-04-10 01:40 JST`
- post-package validation:
  ...
  - `git diff --check` → 無出力
  - `git status --short --branch` → `Documentation.md`、`specs/00-document-map.md`、`plan/11`、`plan/12`、`plan/13`、`plan/17`、`plan/90`、`progress.md`、`tasks.md`、`docs/research_abstract/phase5-small-decidable-core-and-proof-boundary.md` に modified、`docs/reports/0424-phase5-adapter-validation-threshold-package.md`、`docs/reports/0425-review-phase5-adapter-validation-threshold-package.md`、`specs/examples/154-current-l2-theorem-line-exchange-ready-adapter-validation-threshold.md` に untracked 差分が残る状態

## What changed in understanding

- theorem-line later package は `exchange_rule_ref` で terminal cut にするより、`adapter_validation_ref` までを symbolic ref として current first choice に含める方が自然である。
- ただし `adapter_validation_ref` を environment-specific invocation surface や concrete runtime coupling と結びつけるのは still 早く、actual notebook exchange rule body / invocation surface / concrete file or blob exchange は次段の later reopen に残すべきである。
- したがって next later reopen の名前も `adapter-validation threshold` ではなく `invocation-surface threshold` に切り替えるのが正確である。

## Open questions

- `consumer_invocation_surface_ref` を theorem-line retained bridge にどこまで足すか。
- actual notebook exchange rule body と adapter validation を同じ reopen package に置くべきか。
- `proof_assistant_adapter` pressure を invocation-surface line より先に practical reopen に上げる条件があるか。
