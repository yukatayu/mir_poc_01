...
- `git commit --no-gpg-sign -m "admit と lease の観測面の表記を補正する"`
  - `[main dff6ec9] admit と lease の観測面の表記を補正する`
...
### 6.5 specification-body commits

- 仕様本文の commit は `09ff3a8` `admit と lease の観測面を整理する` と `dff6ec9` `admit と lease の観測面の表記を補正する` で記録した。
