# 0872 Package 119 Problem 1 model-check public-contract split helper
...
- Problem 1 model-check public-contract residual を typed residual / theorem residual から切り離して読むための
...
- current target は model-check public-contract split helper actualization に限定する。
- first settled property language、
  final public checker artifact、
  final public verifier contract は今回の scope に入れない。
- typed residual と theorem residual は kept-separate として保持する。
...
1. model-check public-contract split helper の current reading を確認した。
2. `samples/problem-bundles/problem1-typed-theorem-model-check.md` に
   `model-check public-contract split の入口` section を追加した。
3. split package status を更新し、
   model-check public-contract split を close 済みの helper/doc cut として読めるようにした。
4. guided sample tests と sample bundle doc tests に
   model-check split command / manifest / doc wording を追加した。
5. spec / snapshot / roadmap / traceability を Package 119 closeout reading に同期した。
...
- RED:
  - `python3 -m unittest scripts.tests.test_problem_sample_bundles`
  - Problem 1 guide に model-check split helper section がなく失敗した。
- GREEN:
  - `python3 scripts/current_l2_guided_samples.py split problem1 model-check-public-contract`
...
- GREEN:
  - `python3 -m unittest scripts.tests.test_current_l2_guided_samples scripts.tests.test_problem_sample_bundles`
  - model-check split helper tests と Problem 1 guide updated doc tests を含めて通過した。

## 9. What changed in understanding

- model-check public-contract residual は typed / theorem residual と並べると混線しやすいが、
  `split` helper と Problem 1 bundle doc の専用 section を持たせると reopen point が読みやすい。
- `p06` representative だけでなく `p10 / p11 / p12 / p15 / p16` を supporting set として明示したほうが、
  row-local carrier first / checker-artifact route first の current cut を説明しやすい。

## 10. Open questions

- Problem 1 の split package closeout 自体はここで一巡したが、
  final public theorem/model-check/verifier contract は still later である。
- Problem 2 source wording / witness-provider public-shape residual は still next package であり、
  今回は untouched に残した。

## 11. Suggested next prompt

- Problem 1 split trio が揃ったので、
  Problem 2 source wording / emitted schema split を same style の narrow helper/doc package に落としてください。
