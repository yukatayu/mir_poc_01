...
focused regression では、`terminal_outcome = explicit_failure`、`perform-failure`、formal metadata 空だけでなく、
`DirectStyleEvaluator` の final `place_store` が空のままであることも確認した。
