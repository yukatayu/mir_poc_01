...
- `SL-A1-*` fixtures are parser/loader inputs plus `alpha_local_checker_input`; `SL-A1-01/02` additionally carry `alpha_local_runtime_input` and `alpha_local_save_load_input` for the runtime-backed save/load branch, while `SL-A1-03` stays checker-backed as an exact save-load preflight reject row.
...
- `alpha_local_save_load_input` currently carries practical save/load lanes such as `scenario_kind`, `required_base_terminal_outcome`, saved-frontier history/owner requirements, `resumed_dispatch_program`, `resumed_envelope`, and `post_restore_membership_advances`; this input is used by the runtime-backed `SL-A1-01/02` branch only.
...
- `SL-A1-01/02` still require a positive checker floor, a distinct runtime-plan boundary, and one exact practical local-runtime frontier; they do not bypass the practical front-door or collapse into the Alpha-0 Stage-B save/load supporting subset.
- `SL-A1-03` reuses an exact rejected practical checker report as a save-load preflight reject row and does not build a runtime frontier, saved local frontier, or distributed checkpoint attempt.
...
- `SL-A1-01/02/03` are non-final practical local save/load rows only; they are not distributed durable save/load, stale witness/stale lease non-resurrection completion, queue/channel/transport persistence, product prototype, or final public save-load ABI completion.
