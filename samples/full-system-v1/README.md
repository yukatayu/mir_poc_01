...
- `provider-adapter/` is actualized for `P-ENG-02` as bounded provider-admission evidence with 5 executable rows, generated `provider-admission-report.json`, viewer-diagnostic inventory admission, WASM inventory-only admission, over-capability rejection, missing rollback policy rejection, native-disabled rejection, and the `mirrorea-alpha admit-provider-v1` CLI surface.
