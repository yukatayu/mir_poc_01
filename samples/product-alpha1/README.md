...
- `operational/templates/` has the `P-OPS-09` validated authoring starters. `world-core-starter/` is `template_only`, passes `check` and `run-local`, and is documented by `docs/hands_on/operational_package_authoring_01.md`; it is not counted as an active operational sample root.
