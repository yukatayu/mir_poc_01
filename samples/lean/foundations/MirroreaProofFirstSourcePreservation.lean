...
namespace MirroreaProofFirst.SourcePreservation
...
theorem control_preserves (s : State p a) (member : Fin a) (place : Fin p) (principal : Nat)
  ...
  | some result =>
      rw [h] at accepted
      cases accepted
      exact CompositionMachine.manage_preserves _ _ _ _ _ _ _ valid h

theorem resume_preserves (m : CompositionMachine.Machine p a) (t : InvocationBoundary.Ticket)
    (next : CompositionMachine.Machine p a × Int) (valid : CompositionMachine.Invariant m)
    (accepted : CompositionMachine.resume m t = some next) : CompositionMachine.Invariant next.1 := by
  ...
  | some value =>
      ...
      | some result =>
          rw [hf] at accepted
          cases accepted
          exact CompositionMachine.finish_preserves _ _ _ _ valid hf
              ...
              | some started =>
                  ...
                  | some completed =>
                      exact bind_preserves {s with machine := completed.1,nextRequest := s.nextRequest+1}
                        ...
                        (resume_preserves _ _ _ (CompositionMachine.start_preserves _ _ _ _ _ _ _ _ valid hs) hr)
...
end MirroreaProofFirst.SourcePreservation
