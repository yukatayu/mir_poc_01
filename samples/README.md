...
- `alpha/` は current alpha-local theory-freeze / checker/runtime roadmap 用の scaffold であり、expected-verdict sidecar を伴う
  ...
  - `cut-save-load/` は `alpha_cut_save_load_samples.py` により `CUT-04` local-only save/load bridge を actualize し、selected negative rows の checker floor と併存する
  - `visualization/` は `alpha_visualization_samples.py` により `VIS-01/03/06/07/08/10/11` を dedicated Stage-E subset runner として actualize したが、`VIS-02/04/05/09/12` は planned-only のままであり、family 全体や final viewer API を completion 扱いしない
