...
- `full-system-v1/`
  ...
  - current executable scope is parser plus typed checker plus bounded effectful runtime plus bounded PoseGraph runtime/save-load/devtools evidence plus bounded projection IR/boundary-schema evidence; it does not claim final effect grammar, final packet/FFI transport semantics, executable server/client split, final devtools family completion, or generated package artifact completion
  ...
  - do not mark the whole root workflow-ready until later packages add executable local server/client role-run evidence, provider admission, and broader sample families
