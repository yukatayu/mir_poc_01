...
- `alpha/` と `practical-alpha1/` を混同しない
  ...
  - `scripts/practical_alpha05_session.py` はこの root を first bounded α-0.5 same-session carrier として consume し、`OA05-07` で one minimal typed external `AddOne` direct execution lane を same-session observer surface へ接続する。same-session hot-plug runtime は still later
