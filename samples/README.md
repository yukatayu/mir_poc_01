...
- `full-system-v1/`
  ...
  - current executable scope is parser plus typed checker plus bounded effectful runtime plus bounded PoseGraph runtime/save-load/devtools evidence plus bounded projection IR/boundary-schema evidence plus bounded local role-split evidence; it does not claim final effect grammar, final packet/FFI transport semantics, final server/client binary split, final devtools family completion, or generated package artifact completion
  - planned families still include world-core, membership-chat, sugoroku-world, and provider-adapter
  ...
  - do not mark the whole root workflow-ready until later packages add provider admission, broader source-first operational families, and release-check closure
