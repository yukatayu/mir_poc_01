...
- `alpha/` と `practical-alpha1/` を混同しない
  ...
  - current practical devtools export floor は `VIS-A1-01/02/04/05/06` に限られ、`VIS-A1-04` は exact practical hotplug reports から attach accepted boundary / membership snapshot / deferred detach boundary を export する observability widening に留まり、`VIS-A1-05` は exact `AV-A1-03` avatar preview report から rejected source lane / degraded roles / missing host capability を export する fallback degradation widening に留まる。membership timeline と retention/on-demand trace は later widening に残る
