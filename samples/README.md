...
- `product-alpha1/`
  ...
  - `operational/` is the `P-OPS-01` canonical operational product sample suite root with `WorldCore -> MembershipChat -> SugorokuWorld -> PortalWorldLink -> TwoShardHardBoundary`, the bounded `portal-worldlink/` and `two-shard-hard-boundary/` roots, shared attach packages, deployment/projection intent, retained portal/shard blueprint inventory, the `P-OPS-09` / `P-OPS-10` template-only authoring starter catalog under `operational/templates/`, the `P-OPS-11` docs-first `future/gradient-observation.profile.json`, and the `P-OPS-12` decision that keeps portal/shard authoring on active roots rather than starter duplicates
