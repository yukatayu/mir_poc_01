...
- `alpha/` と `practical-alpha1/` を混同しない
  ...
  - `scripts/practical_alpha08_session_hotplug.py` は同じ root を bounded α-0.8 same-session hot-plug carrier として再利用し、`OA08-01..10` で debug / auth / rate-limit / object preview / deferred detach の accepted/rejected/deferred lifecycle を same-session observer surface へ接続する。α-0.9 live/session-bound devtools export は still later
