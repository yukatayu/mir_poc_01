...
- `e1` / `e3` / `e21` は current matrix 上の target path を維持しつつ、authored-row widen sequencing は `e1 -> e21 -> e3` に置く。
...
- `python3 scripts/current_l2_source_sample_regression.py inventory` は current authored first trio の file presence と deferred row absence を確認する。
- `python3 scripts/current_l2_source_sample_regression.py regression --run-label <label> --artifact-root <root>` は current authored first trio の lowering / runner / ladder / formal-hook smoke bundle を流す。
