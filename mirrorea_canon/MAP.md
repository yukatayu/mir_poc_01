...
| WRK-0023 | L3-open | theory/04 の表示済み event-only Consistent(Kc) と `send -> receive` から send membership を literal Lean transcription で検査し、channel state を代替と読むための表示済み表現関係の有無を監査する登録済み L3 record。checkpoint carrier、checker、OBL は選ばない。`working/WRK-0023-consistent-cut-channel-state-boundary.md` |
