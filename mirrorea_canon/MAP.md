...
NORTH-STAR (軸)
   │
   ▼
adr/ (決定 ADR-0001..0038) ◄──── GLOSSARY (概念 CON-###)
   │
   ▼
...
theory/ (MirCore v0)                     mental-model/ (表と裏の対応)
  ...
  04 / 05 / 06 / 07 / 09 ──► 14 relation/projection ──► 15 shared model
...
- **実装筋**(toolchain を作る): spec/02..15 → architecture/03..04 → architecture/09 → scenarios → plan/01 の該当 Phase → plan/05 I3 boundary。
- **運用筋**(進め方を知る): plan/00..05 → meta/agent-instructions → adr/ADR-0012 → adr/ADR-0014 → adr/ADR-0015 → adr/ADR-0026 → adr/ADR-0027 → adr/ADR-0028 → adr/ADR-0029 → adr/ADR-0030 → adr/ADR-0031 → adr/ADR-0032 → adr/ADR-0033 → adr/ADR-0034 → adr/ADR-0035 → adr/ADR-0036 → adr/ADR-0037 → meta/proposals/PROPOSAL-041 → adr/ADR-0038。
...
PROPOSAL-040 / ADR-0037は同一のprivate source/Core-bound nine-case actual-process
canaryでcriteria 1--7がtieし、criterion 8 implementation/library maturityとcriterion 9
...
deferred replacement baseline、QUIC datagramはexcludedのままである。PROPOSAL-041 / ADR-0038でI3-1はcompleted、I3-2がsole activeである。OPEN-032はこのbounded programだけresolvedした。
...
| WRK-0042 | L3-open, not-promoted | P017 X1 の V1/R1 cross-locus read に限り、supplied fixture の owner-terminal-negative fact と owner-mutation fact が同時に成立する collapse を predicate-only finite countermodel で検査した。四 fixture の detector table は通ったが、failure semantics、mutation rule、transition、carrier、runtime は定義・選択しない。`working/WRK-0042-p017-x1-owner-negative-mutation-countermodel.md` |
| WRK-0043 | L3-open, not-promoted | P017 X1 の V1/R1 cross-locus read に限り、source-named M1 adverse condition の supplied fixture tag と owner-mutation fact が同時に成立する collapse を predicate-only 有限 countermodel で検査した。四 fixture forms の detector table は通ったが、validation/failure semantics、mutation rule、transition、carrier、runtime は定義・選択しない。`working/WRK-0043-p017-x1-m1-adverse-mutation-countermodel.md` |
| WRK-0044 | L3-open, not-promoted | P017 X1 の V1/R1 cross-locus read に限り、external rejection/no-observation を候補仮説とする candidate-local five-pair static relation-envelope presentation を Lean `--trust=0` で実行した。これは明示前提に対する local pending/non-sharing、terminal exclusion、named use、五 correspondence の静的 account のみであり、Canon schema、transition、identity、causal order、SaveObject、proof、runtime は定義・選択しない。`working/WRK-0044-p017-x1-minimum-relation-envelope-coherence.md` |
| WRK-0045 | L3-open, frozen / DEFER | P017 X1 の V1/R1 cross-locus read に限る A-Sigma `H_K-rs` conditional trace を Lean `--trust=0` で実行したが、同一 binding を二つの distinct branch が共有できる extracted-source countermodel が registered non-sharing falsifier を満たしたため凍結した。branch key、identity、Canon schema、transition、receipt、restore、runtime は定義・選択せず、B-Pi へ切替えない。`working/WRK-0045-p017-x1-k0-hk-rs-asigma-conditional-trace.md` |
| WRK-0046 | L3-open, not-promoted | P017 X1 K0 external-rejection branch に限り、opaque per-state `AcceptedSuccess` と disposable q-fibered `Spent` を用いる一つの finite linear experimental lineage を Lean `--trust=0` で実行した。ordinary-edge preservation と一つの supplied restore-edge preservation の下で at-most-one counted consume を条件付きに導き、preservation を外した A1 は exact two-consume control を構成した。これは Canon state/identity/transition/persistence、receipt、Core、Canon proof/OBL、runtime を定義・選択せず、Gate/Phase 又は official critical path を動かさない。`working/WRK-0046-p017-x1-k0-qf-ul-lift.md` |
