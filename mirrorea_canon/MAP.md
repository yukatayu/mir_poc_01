...
| WRK-0035 | L3-open, not-promoted | C7 の concrete source rule を選ばず、local parametric `erase`/`observe` に対する fiber constancy と range 上の pointwise unique observation を conditional lemma として検査する。choice、quotient、Mir carrier、source inference は定義・選択しない。`working/WRK-0035-c7-parametric-factorization.md` |
