...
| WRK-0036 | L3-open, not-promoted | C7 の concrete source rule を選ばず、individually fiber-constant な二つの local erasure とその common coarsening が paired observation を失う固定有限 countermodel を検査する。個別 check を同時 omission の許可へ合成しない negative guard に限り、choice、quotient、Mir carrier、source inference は定義・選択しない。`working/WRK-0036-c7-cumulative-erasure-countermodel.md` |
