...
| WRK-0017 | L3-open, not-promoted | exact current-L2 Lean foundation の二 constructor `Capability` に限り、任意 `CaptureSet` 間の proposition-valued `captureSubset lhs rhs ∨ ¬ captureSubset lhs rhs` theorem を明示局所消去で構成できるかを登録したL3 record。WRK-0016のvalue-declaration route、OBL-003、Line-1、checker、generic carrier、Canon carrier/APIは変更・主張しない。`working/WRK-0017-local-predicate-proposition-decidability.md` |
