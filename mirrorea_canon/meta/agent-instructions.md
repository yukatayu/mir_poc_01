...
status: L0-frozen
maturity: draft
depends_on: [adr/ADR-0012, adr/ADR-0014, adr/ADR-0015, adr/ADR-0026, plan/02-operating-model]
summary: agent の読込、active I2 Systems Foundation bounded autonomy、closed Mir v0/I1+、通常L3 research、review/owner escalation境界。
open_items: []
---
...
- ADR-0026 / PROPOSAL-029 の Mirrorea I2 Systems Foundation program では、current milestone
  ...
  挟まず次の SYS milestone へ進む。active roadmapは`LAB:plan/249`だけであり、
  closed ADR-0015 / Plan 247はregression baselineとして保持する。
...
- North Star の変更、authority/privacy/redaction/no-stale-resurrection 保証の弱化、
  ...
  public API/ABI/wire format の不可逆な固定、real transport のこのprogram内での
  選択・実装、production deployment/external publication は owner に留保する。
...
- active ADR-0026 program 外では ADR-0014 の reserved boundary と L3-only route を守る。

## 衝突時の挙動

ADR-0026 program 内の現行 Canon との矛盾は、PROPOSAL-029 を最新 owner direction
...
ADR-0026 program の normative result は通常手続きで Canon へ、evidence は milestone
