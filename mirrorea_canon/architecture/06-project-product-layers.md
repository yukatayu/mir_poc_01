...
status: L1-fixed
maturity: reviewed
depends_on: [root/north-star, root/design-constitution, arch/01-strata, arch/02-boundary-contracts, arch/05-satellites, adr/ADR-0034, adr/ADR-0035, adr/ADR-0040, plan/01-phases, adr/ADR-0043]
...
| real process/transport execution | `S4` realized over `S6` | `PL-2` over `PL-0` | bounded I3-3 evidence accepted under ADR-0043; owner-paused / no active semantic milestone; official I3 lifecycle unentered |
| package admission / View / provider | `S6` boundary consuming `S3--S5` facts | `PL-3` over `PL-0`, connected to `PL-2` | ALIGN-2 responsibility contract accepted; implementation inactive I5 |
...
accepted by `arch/07`; implementation remains the inactive I5 direct consumer.
