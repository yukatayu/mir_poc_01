...
- **Phase 5** は `specs/examples/126...185` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から symbolic retained bridge を `retained_payload_body_materialization_payload_ref` まで伸ばし、**next later reopen** を `actual retained payload body materialization carrier detail comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | actual retained payload body materialization carrier detail comparison | theorem-line retained bridge に carrier detail をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...185` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_payload_ref` までを current first choice に置ける。

#### 次に扱うべき narrow question

- `actual retained payload body materialization carrier detail` を theorem-side retained bridge に寄せるか
- それとも body materialization payload ref のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...185` で current package close。
