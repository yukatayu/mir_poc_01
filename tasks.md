...
- **Phase 5** は `specs/examples/126...199` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から theorem-side retained bridge を `retained_payload_body_materialization_external_contract_payload` まで伸ばし、**next later reopen** を `external-contract-payload-ready payload enrichment / second-consumer-pressure comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | external-contract-payload-ready payload enrichment / second-consumer-pressure comparison | theorem-line retained bridge に richer notebook payload と second consumer pressure をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...199` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_external_contract_payload` までを current first choice に置ける。

#### 次に扱うべき narrow question

- `retained_payload_body_materialization_external_contract_payload` の次段として richer notebook payload ないし second consumer pressure を theorem-side retained bridge にどこまで近づけるか
- それとも minimal consumer-specific payload のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...199` で current package close。
