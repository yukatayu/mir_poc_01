...
- **Phase 5** は `specs/examples/126...181` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から symbolic retained bridge を `archive_retention_layout_ref` まで伸ばし、**next later reopen** を `actual retained archive payload body family comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | actual retained archive payload body family comparison | theorem-line retained bridge に payload body family をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...181` で theorem-line retained bridge を段階的に伸ばし、現時点では `archive_retention_layout_ref` までを current first choice に置ける。

#### 次に扱うべき narrow question

- `actual retained archive payload body family` を theorem-side retained bridge に寄せるか
- それとも layout family ref のままで止めるか
