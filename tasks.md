...
- **Phase 5** は `specs/examples/126...194` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から symbolic retained bridge を `retained_payload_body_materialization_boundary_handoff_pair_ref` まで伸ばし、**next later reopen** を `boundary-specific-handoff-pair-ready actual handoff pair shape comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | boundary-specific-handoff-pair-ready actual handoff pair shape comparison | theorem-line retained bridge に actual handoff-facing pair shape をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- theorem-side symbolic retained bridge を、proof obligation / external handoff artifact の docs-first 比較として narrow に進める。
...
- `specs/examples/135...194` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_boundary_handoff_pair_ref` までを current first choice に置ける。

#### 次に扱うべき narrow question

- `retained_payload_body_materialization_boundary_handoff_pair_ref` の次段として actual boundary-specific handoff pair shape を theorem-side retained bridge にどこまで近づけるか
- それとも symbolic handoff pair ref のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...194` で current package close。
