...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | minimal-public-checker-command-surface-ready shared-output-contract comparison | public checker command surface を `command_surface_kind + family_facade_command_refs + public_checker_api_ref` minimal family-facade bundle に留めた current package を前提に、shared output contract をどこまで narrow に切るべきかを比較する。parser boundary は still later に残す | 中 | 1〜3 task / 2〜5日 | 自走可能 | **current promoted line** |
...
- public checker command surface line の次段として shared output contract をどこまで narrow に切るべきか

#### いま自走できる理由

- current line は docs-only cluster matrix / row core / evidence attachment / public checker payload / public checker API read relation / public checker entry criteria / public checker command surface threshold に留まり、shared output contract / final type system / actual theorem prover relation までは踏み込んでいない。
...
- Phase 5 current package は `specs/examples/126...280` で current package close。
