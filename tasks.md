...
- Plan 227 は P017 X1 の通常設計で何を一つの tuple として比較するかを固定した。
  ...
  とし、WRK-0045 はその question を source なしで登録した。Theory 04 は generic
  occurrence kind / receive rule を供給しない。次は単一候補 source の premise/falsifier
  design screen であり、source を急いで materialize しない。K1 は failure row の Canon
  gap。
