...
- **Phase 5** は `specs/examples/126...246` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から theorem-side retained bridge を `retained_payload_body_materialization_theorem_export_handoff_transport_family` まで伸ばし、**next later reopen** を `minimal-handoff-transport-family-ready handoff-transport-carrier-detail comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | minimal-handoff-transport-family-ready handoff-transport-carrier-detail comparison | symbolic handoff transport family の次段として handoff transport carrier detail を theorem-line retained bridge にどこまで近づけるかを narrow に整理する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...246` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_theorem_export_handoff_transport_family` まで current first choice に置ける。

#### 次に扱うべき narrow question

- `retained_payload_body_materialization_theorem_export_handoff_transport_family` の次段として、handoff transport carrier detail をどこまで narrow に retained bridge へ寄せるか
...
- Phase 5 theorem-line package は `specs/examples/126...246` で current package close。
