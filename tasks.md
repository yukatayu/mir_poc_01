...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | retained archive payload comparison | `proof_notebook` first bridge に `archive_bless_update_policy_ref` までを足したうえで、retained archive payload をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `179...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
...
- `specs/examples/178-current-l2-theorem-line-archive-member-family-ready-archive-member-body-compare-threshold.md` で、
  ...
  current first choice まで固定済みである。
- `specs/examples/179-current-l2-theorem-line-archive-member-body-compare-ready-archive-bless-update-policy-threshold.md` で、
  - archive-member-body-compare-ready retained bridge の次段では `archive_bless_update_policy_ref` までは足してよい
  - retained archive payload は second step に残す
  current first choice まで固定済みである。

#### この task で残ること

- retained archive payload をどこまで足すか
