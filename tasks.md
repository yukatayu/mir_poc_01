...
- actualize the renderer pose backend demo over the bounded PoseGraph + projection + provider floor without making the renderer a semantic owner.
- preserve `P-ENG-02` closed with bounded provider admission, accepted viewer-diagnostic / WASM inventory-only rows, explicit over-capability and missing-rollback rejection, native-disabled evidence, and generated provider-admission reports.
...
|---:|---|---|---|---|---|
| 1 | `P-ENG-03 renderer pose backend demo` | `Macro 7/8`, `S4 -> S5` | renderer receives pose snapshot | renderer stays non-semantic-owner | medium |
| 2 | `P-FSV1-01 source operational suite` | `Macro 8`, `S4 -> S5` | source-first WorldCore/MembershipChat/Sugoroku | check/run/devtools over source-first roots | heavy |
| 3 | `P-FSV1-02 portal/shard source samples` | `Macro 8`, `S4 -> S5` | source-first portal/shard/gradient | positive/negative portal/shard evidence | heavy |
| 4 | `P-FSV1-03 full V1 release check` | `Macro 0/7/8`, `S5 -> S6` | clean clone Full V1 workflow | release check, viewer, bundle, reports | heavy |
| 5 | `P-FSV1-99 final audit` | `Macro 0`, `S6` | claim/non-claim and docs cleanup | all validation recorded, report/commit/push done | medium |
...
| `Macro 3` | compile-ready minimal actualization | `P-MIR-01..04`、`P-POSE-03..04`、`P-PROJ-02..04`、`P-ENG-02` closed; next main implementation path is renderer pose backend |
...
| projection IR granularity | `P-ENG-03` and later | summary manifest / typed projection IR / deployment planner | typed projection IR is actualized; preserve that floor and keep planner later |
| packet/FFI payload schema granularity | `P-ENG-03` and later | payload-shape-only / effect-tag-rich schema / full transport protocol | keep the current payload-shape plus effect/failure/capability/authority preservation floor and defer transport semantics |
| local role-split carrier shape | `P-ENG-03` and later | same-binary local roles / paired local processes / Docker-pinned manifest roles | same-binary manifest-driven local roles are actualized; keep renderer/provider layering above that floor |
| renderer/provider pose snapshot seam | `P-ENG-03` | project raw runtime state / preserve typed pose snapshot surface / add later planner normalization | keep provider admission bounded and add renderer as a consumer of pose snapshots without widening semantic ownership |
| PoseGraph/projection preservation seam | `P-ENG-03..FSV1-02` | project raw runtime state / preserve typed boundary manifests / add later planner normalization | preserve typed manifests and boundary evidence first, then layer renderer and source-first operational lanes without widening semantic ownership |
...
| sample taxonomy audit | keep active, planned, generated, archive roots distinct | source hierarchy and relevant helper checks | only `samples/full-system-v1/computational/`、`avatar-pose/`、`projection/`、`server-client/`、`provider-adapter/` are evidence-closed; wider Full System V1 roots must not be workflow-ready before evidence |
