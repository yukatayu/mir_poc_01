...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | theorem-side minimum contract row comparison | theorem-side docs-only bridge を concrete theorem consumer に渡すなら、minimum contract rows をどこまで docs-only で書くかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `132...` までで current theorem-line package は close。reopen は concrete theorem consumer pressure が実際に要るとき |
...
- concrete theorem consumer bridge に必要な minimum contract rows をどこまで docs-only で書くか
