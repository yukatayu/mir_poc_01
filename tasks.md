...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | exchange-body threshold comparison | `proof_notebook` first bridge に `consumer_invocation_surface_ref` までを足したうえで、actual notebook exchange body / concrete runtime coupling をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `155...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
...
- exchange-body threshold をどこまで足すか
