...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | actual emitted notebook artifact threshold comparison | `proof_notebook` first bridge を docs-only derived view に留めたまま、path-ready retained bridge の上に actual emitted notebook artifact をどこまで足すかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `149...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
