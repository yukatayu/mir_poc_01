...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | actual archive bless / update policy comparison | `proof_notebook` first bridge に `archive_member_body_compare_ref` までを足したうえで、actual archive bless / update policy をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `178...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
...
- `specs/examples/166-current-l2-theorem-line-transcript-ready-materialized-runtime-handoff-threshold.md` で、
  ...
  current first choice まで固定済みである。
- `specs/examples/167-current-l2-theorem-line-materialized-ready-concrete-payload-threshold.md` で、
  - materialized-ready retained bridge の次段では `concrete_payload_ref` までは足してよい
  - concrete transcript body / actual serialized channel body は second step に残す
  current first choice まで固定済みである。
- `specs/examples/168-current-l2-theorem-line-payload-ready-concrete-transcript-threshold.md` で、
  - payload-ready retained bridge の次段では `concrete_transcript_body_ref` までは足してよい
  - actual serialized channel body は second step に残す
  current first choice まで固定済みである。
- `specs/examples/169-current-l2-theorem-line-transcript-body-ready-serialized-channel-body-threshold.md` で、
  - transcript-body-ready retained bridge の次段では `serialized_channel_body_ref` までは足してよい
  - actual emitted attachment blob / file body は second step に残す
- `specs/examples/170-current-l2-theorem-line-serialized-ready-emitted-attachment-body-threshold.md` で、
  - serialized-ready retained bridge の次段では `emitted_attachment_body_ref` までは足してよい
  - actual emitted attachment blob / file materialization は second step に残す
  current first choice まで固定済みである。
- `specs/examples/171-current-l2-theorem-line-attachment-body-ready-emitted-attachment-blob-threshold.md` で、
  - attachment-body-ready retained bridge の次段では `emitted_attachment_blob_ref` までは足してよい
  - actual retained file body / archive materialization は second step に残す
  current first choice まで固定済みである。
- `specs/examples/172-current-l2-theorem-line-attachment-blob-ready-retained-file-body-threshold.md` で、
  - attachment-blob-ready retained bridge の次段では `retained_file_body_ref` までは足してよい
  - actual archive materialization は second step に残す
  current first choice まで固定済みである。
- `specs/examples/173-current-l2-theorem-line-retained-file-body-ready-archive-materialization-threshold.md` で、
  - retained-file-body-ready retained bridge の次段では `archive_materialization_ref` までは足してよい
  - actual archive body / bundle family は second step に残す
  current first choice まで固定済みである。
- `specs/examples/174-current-l2-theorem-line-archive-materialization-ready-archive-body-bundle-threshold.md` で、
  - archive-materialization-ready retained bridge の次段では `archive_body_ref` までは足してよい
  - actual archive bundle family は second step に残す
  current first choice まで固定済みである。
- `specs/examples/175-current-l2-theorem-line-archive-body-ready-archive-bundle-threshold.md` で、
  - archive-body-ready retained bridge の次段では `archive_bundle_ref` までは足してよい
  - actual archive bundle manifest / member family は second step に残す
  current first choice まで固定済みである。
- `specs/examples/176-current-l2-theorem-line-archive-bundle-ready-archive-manifest-threshold.md` で、
  - archive-bundle-ready retained bridge の次段では `archive_bundle_manifest_ref` までは足してよい
  - actual archive bundle member family は second step に残す
  current first choice まで固定済みである。
- `specs/examples/177-current-l2-theorem-line-archive-manifest-ready-archive-member-family-threshold.md` で、
  - archive-manifest-ready retained bridge の次段では `archive_bundle_member_family_ref` までは足してよい
  - actual archive member body compare は second step に残す
  current first choice まで固定済みである。
- `specs/examples/178-current-l2-theorem-line-archive-member-family-ready-archive-member-body-compare-threshold.md` で、
  - archive-member-family-ready retained bridge の次段では `archive_member_body_compare_ref` までは足してよい
  - actual archive bless / update policy は second step に残す
  current first choice まで固定済みである。

#### この task で残ること

- actual archive bless / update policy をどこまで足すか
