...
|---:|---|---|---|---|---|
| 1 | broader operational template catalog | `Macro 7/8`, `S2` | `world_core` 以外の package kinds に対する template-only starter widening 順を決める | `membership_chat` / `sugoroku_world` / portal/shard のどこまで starter を増やすかを bounded に整理する | medium |
...
- recommended reopen point:
  broader operational template catalog
- recommendation reason:
  `P-OPS-08` で backend feasibility inventory が docs-first に actualize 済みになったため、次の最小 widening は `world_core` 以外の template-only starter をどこまで増やすかを bounded に整理し、external reuse を practical に広げることである
- stop line:
...
| post-two-shard widening order | `specs/27` と future suite line に影響 | broader starter catalog first / gradient observation profile first / room-chat widening first | current recommendation は broader starter catalog を先に置き、gradient observation runtime や broader room-chat widening はその後段に分ける |
