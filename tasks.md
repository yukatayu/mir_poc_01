...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | runtime-coupling threshold comparison | `proof_notebook` first bridge に `exchange_rule_body_ref` までを足したうえで、concrete runtime coupling / transport / failure body をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `156...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
