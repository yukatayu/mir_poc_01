...
- **Phase 5** は `specs/examples/126...189` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から symbolic retained bridge を `retained_payload_body_materialization_bless_update_row_pair_ref` まで伸ばし、**next later reopen** を `actual bless-side row / update-side row ref family comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | actual bless-side row / update-side row ref family comparison | theorem-line retained bridge に bless/update row ref family をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...189` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_bless_update_row_pair_ref` までを current first choice に置ける。

#### 次に扱うべき narrow question

- `actual bless-side row ref / update-side row ref family` を theorem-side retained bridge に寄せるか
- それとも bless/update row pair ref のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...189` で current package close。
