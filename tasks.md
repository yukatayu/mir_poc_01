...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | actual handoff emitter threshold comparison | `proof_notebook` first bridge に `emitted_artifact_ref` までを足したうえで、actual handoff emitter / consumer adapter policy をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `150...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
