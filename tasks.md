...
conditional account with explicit premises and no axioms. It remains
not-promoted and does not select a relation schema, transition, source form,
...
- Plan 227 は P017 X1 の通常設計で何を一つの tuple として比較するかを固定した。
  ...
  external rejection は候補仮説、no-observation は負の scope のままであり、vacuity と
  restore identity は falsifier として維持する。positive schema/rule/proof は ordinary
  Canon design work のままであり、card の common identity や lifecycle を Plan 側から
  導入しない。
