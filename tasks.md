...
- `scripts/README.md` also mirrors the current `check_source_hierarchy.py`
  ...
  `plan/39..137`, including the current G1 E-ROW / OBL addenda, remaining
  ...
  `plan/118..137`. This is documentation taxonomy maintenance only.
- `scripts/validate_docs.py` now rejects numbered `plan/*.md` files that exist
  ...
  cover current numbered `plan/00..137`. This keeps future plan-file additions
...
|---|---|---|---|
| `OBL-001 artifact annex template` | `G1` docs/advisory-only | prepare a non-applied annex template for a later OBL-001 `lean-stated` packet, mapping canon OBL-001 target, LAB artifact path / namespace / constant, OPEN-014 deferral, validation evidence, and non-claims, using `plan/137` | annex-template-only; no wrapper file, no canon edit, no ledger movement, no proposal submission, no status choice treated as accepted, no executable row, no Lean refinement |
| `G1 status packet shell with unresolved slots` | `G1` docs/advisory-only | prepare a draft packet shell that references the OBL-020 annex template but leaves requested status, ledger delta, artifact identity acceptance, and scope decisions unresolved | draft-shell-only; no proposal submission, no canon edit, no ledger movement, no status choice treated as accepted, no executable row, no Lean refinement |
