...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | theorem consumer class / attachment family comparison | minimum contract row core の先で、concrete theorem consumer class と `goal_text` / `proof_hint` / `consumer_hint` の attachment family をどこまで docs-only で比べるかを切る | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `133...` までで current theorem-line package は close。reopen は concrete theorem consumer pressure が実際に要るとき |
...
- concrete theorem consumer class を first bridge にどう置くか
- `goal_text` / `proof_hint` / `consumer_hint` の attachment family をどこまで docs-only で比べるか
