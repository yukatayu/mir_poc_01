...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | invocation-surface threshold comparison | `proof_notebook` first bridge に `adapter_validation_ref` までを足したうえで、environment-specific invocation surface / concrete runtime coupling をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `154...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
...
- invocation-surface threshold をどこまで足すか
