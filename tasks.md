...
- actualize source-first `world-core/`, `membership-chat/`, and `sugoroku-world/` operational roots over the now-closed parser/runtime/PoseGraph/projection/provider/renderer floor.
- preserve `P-ENG-03` closed with one accepted renderer pose delivery row, two blocked renderer rows, bounded provider admission, and generated renderer/provider reports.
...
|---:|---|---|---|---|---|
| 1 | `P-FSV1-01 source operational suite` | `Macro 8`, `S4 -> S5` | source-first WorldCore/MembershipChat/Sugoroku | check/run/devtools over source-first roots | heavy |
| 2 | `P-FSV1-02 portal/shard source samples` | `Macro 8`, `S4 -> S5` | source-first portal/shard/gradient | positive/negative portal/shard evidence | heavy |
| 3 | `P-FSV1-03 full V1 release check` | `Macro 0/7/8`, `S5 -> S6` | clean clone Full V1 workflow | release check, viewer, bundle, reports | heavy |
| 4 | `P-FSV1-99 final audit` | `Macro 0`, `S6` | claim/non-claim and docs cleanup | all validation recorded, report/commit/push done | medium |
...
| `Macro 3` | compile-ready minimal actualization | `P-MIR-01..04`、`P-POSE-03..04`、`P-PROJ-02..04`、`P-ENG-02..03` closed; next main implementation path is source operational suites |
...
| projection IR granularity | `P-FSV1-01` and later | summary manifest / typed projection IR / deployment planner | typed projection IR is actualized; preserve that floor and keep planner later |
| packet/FFI payload schema granularity | `P-FSV1-01` and later | payload-shape-only / effect-tag-rich schema / full transport protocol | keep the current payload-shape plus effect/failure/capability/authority preservation floor and defer transport semantics |
| local role-split carrier shape | `P-FSV1-01` and later | same-binary local roles / paired local processes / Docker-pinned manifest roles | same-binary manifest-driven local roles are actualized; keep operational suites above that floor |
| renderer/provider pose snapshot seam | `P-FSV1-01` and later | preserve typed pose snapshot surface / add later planner normalization | bounded renderer pose delivery is actualized; preserve non-semantic-owner discipline while operational suites consume the same seam |
| PoseGraph/projection preservation seam | `P-FSV1-01..FSV1-02` | project raw runtime state / preserve typed boundary manifests / add later planner normalization | preserve typed manifests and boundary evidence first, then layer source-first operational lanes without widening semantic ownership |
