...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | actual notebook runtime handoff actualization comparison | `proof_notebook` first bridge に `failure_wording_ref` までを足したうえで、actual runtime handoff actualization / emitted invocation receipt / runtime transcript family をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `162...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
...
- actual notebook runtime handoff actualization をどこまで足すか
