...
- Plan 227 は P017 X1 の通常設計で何を一つの tuple として比較するかを固定した。
  ...
  とした。Theory 04 は generic occurrence kind / receive rule を供給しない。次は fresh
  cut を pin する別 WRK preregistration であり、source を同じ commit に置かない。K1 は
  failure row の Canon gap。
