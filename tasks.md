...
- **実装面の現在地** は still uneven である。`mir-semantics` には parser-free current L2 minimal interpreter と harness があり、`cargo test -p mir-semantics --test current_l2_minimal_interpreter -- --list` で 46 tests が列挙できる。`mir-ast` には `crates/mir-ast/src/current_l2.rs` として stage 1 option/chain と stage 2 try/fallback structural floor の non-production carrier が入り、`cargo test -p mir-ast` が通る。一方で `mir-runtime` / `mir-lsp` の public `src/lib.rs` はまだ placeholder skeleton であり、actual checker / runtime compile path は薄い。
...
| Phase 6 前半 compile-ready minimal actual PoC | parser first tranche は actualize 済みで、checker/runtime bridge と formal hook が主残量 | **68% 前後** | 下の Task 1〜3 | **2〜3 package 前後 / 1〜3週** | current L2 subset に scope を絞った rough estimate |

### 進捗 68% 前後の根拠

- 進んでいるもの
  - semantic core、invariants、representative fixtures、parser-free harness、detached validation loop、Phase 3 staged parser spike の evidence、minimal parser subset freeze、parser-to-checker reconnect freeze、`mir-ast` stage 1 / stage 2 non-production carrier。
- まだ薄いもの
  - `mir-runtime` actual runtime bridge
  - parser -> checker -> runtime の compile path actualization
  - proof / model-check handoff の first tranche

## 次に着手すべき順番と rough estimate

| 順番 | phase | task package | 完了条件の要点 | rough weight | rough 所要 | 自走可否 |
|---|---|---|---|---|---|---|
| 1 | Phase 6 前半 | actual checker / runtime skeleton first tranche | `mir-semantics` / `mir-runtime` をまたぐ minimal compile path を actualize する | 重い | 1〜2 task / 4〜7日 | 自走可能 |
| 2 | Phase 6 前半 | compile-ready PoC verification and formal hook | cargo gate、smoke、proof/model-check first tranche を揃えて checkpoint close | 重い | 1〜2 task / 3〜6日 | 自走可能 |
| 3 | Phase 6 前半 | compile-ready checkpoint drift suppression / mirror sweep | specs / plan / progress / tasks / abstract の wording と current line を揃える | 中 | 1 task / 1〜2日 | 自走可能 |

## 自走で進める task package

### Task 1. Phase 6 前半 actual checker / runtime skeleton first tranche
...
### Task 2. Phase 6 前半 compile-ready PoC verification and formal hook
...
### Task 3. Phase 6 前半 compile-ready checkpoint drift suppression / mirror sweep
...
- 概要
  - Phase 6 parser first tranche の後で、どこまで current L2 source surface を widen するか。
- 何に影響するか
  - `mir-ast` public API
  - parser test corpus
  - checker / verifier handoff shape
- 主要な選択肢
  1. stage 1 / 2 carrier を compile-ready checkpoint minimum として維持し、stage 3 reconnect は second tranche に回す
  2. selected stage 3 / perform head を checker/runtime task の中で同時に widen する
  3. request head / clause suite / richer diagnostics まで一気に広げる
- current recommendation / 見解
  - **1 を推奨**
  - parser first tranche を fixed entry criteria にし、compile-ready checkpoint を閉じてから second tranche widen を比較する方が自然である。
...
- current recommendation / 見解
  - **Task 1 fixed 後の Task 2〜3 で 1 か 2+3 を narrow に選ぶ** のを推奨。
  - 現時点では checker/runtime surface がまだ薄いため、先に tool を固定すると surface が逆流しやすい。
