...
- **Phase 5** は `specs/examples/126...236` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から theorem-side retained bridge を `retained_payload_body_materialization_theorem_export_witness_aware_handoff_family` まで伸ばし、**next later reopen** を `minimal-witness-aware-handoff-family-ready handoff-witness-row-detail comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | minimal-witness-aware-handoff-family-ready handoff-witness-row-detail comparison | symbolic witness-aware handoff family の次段として actual handoff witness row detail を theorem-line retained bridge にどこまで近づけるかを narrow に整理する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...236` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_theorem_export_witness_aware_handoff_family` まで current first choice に置ける。

#### 次に扱うべき narrow question

- `retained_payload_body_materialization_theorem_export_witness_aware_handoff_family` の次段として、actual handoff witness row detail をどこまで narrow に retained bridge へ寄せるか
...
- Phase 5 theorem-line package は `specs/examples/126...236` で current package close。
