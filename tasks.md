...
| 36 | WRK-0014 same-carrier variance registration | Current: committed L3 pre-registration permits one standalone Lean check of the relation-inclusion direction needed for safety/coherence and for outcome existence. It must remain same-carrier and parameter-only; no concrete Canon relation, actual coverage/realizability, fairness, outcome representation, OBL, or theorem interface may be introduced. | Macro 1/5 reserve; execute the registered command after push, then either manifest conditional evidence or freeze on a semantic-neutrality falsifier |
...
| --- | --- | --- |
| OBL-020 / OBL-021 / OBL-001 / OBL-024 / OBL-025 | whether a scoped premise or relation is canon-compatible, sufficiently explicit, and non-reserved | historical source-adequacy evidence in `plan/156`; plan/171 records the LAB reading that the familywise wrapper cannot replace direct global OBL-020 without coverage, the OBL-021 coherence draft does not give totality, and WRK-0007's Result/write gap needs a future direct-`c` or explicit bridge. WRK-0014 now tests only the direction of a same-carrier bridge: Canon-to-model for universal safety/coherence, model-to-Canon for outcome existence. It cannot establish an actual bridge. ADR-0014 eligibility remains independent; P008 is an open BND-001 totality decision request. No `theory/11` movement |
