...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | named review unit の stronger bridge threshold comparison | `proof_notebook` first bridge を docs-only derived view に留めたまま、docs-only named review unit を stronger notebook bridge sketch へどこまで拡張するかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `139...` までで current theorem-line package は close。compare / bless flow は second step、`proof_assistant_adapter` pressure は second practical candidate に残す |
