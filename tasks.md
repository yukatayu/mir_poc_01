...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | review-linked bless bridge への retained-path / lifecycle threshold comparison | `proof_notebook` first bridge を docs-only derived view に留めたまま、review-linked bless bridge に retained notebook path / review session lifecycle をどこまで足すかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `143...` までで current theorem-line package は close。actual emitted artifact はさらに後段、`proof_assistant_adapter` pressure は second practical candidate に残す |
