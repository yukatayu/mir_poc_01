...
- **Phase 5** は `specs/examples/126...193` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から symbolic retained bridge を `retained_payload_body_materialization_bless_update_pair` まで伸ばし、**next later reopen** を `consumer-visible-pair-ready boundary-specific handoff pair comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | consumer-visible-pair-ready boundary-specific handoff pair comparison | theorem-line retained bridge に handoff-facing pair surface をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...193` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_bless_update_pair` までを current first choice に置ける。

#### 次に扱うべき narrow question

- consumer-visible pair の次段として boundary-specific handoff pair surface を theorem-side retained bridge に寄せるか
- それとも notebook consumer pair のままで止めるか
