...
- **Phase 5** は `specs/examples/126...198` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から theorem-side retained bridge を `retained_payload_body_materialization_external_contract` まで伸ばし、**next later reopen** を `actual-external-contract-ready consumer-specific external contract payload comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | actual-external-contract-ready consumer-specific external contract payload comparison | theorem-line retained bridge に consumer-specific external contract payload をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...198` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_external_contract` までを current first choice に置ける。

#### 次に扱うべき narrow question

- `retained_payload_body_materialization_external_contract` の次段として consumer-specific external contract payload を theorem-side retained bridge にどこまで近づけるか
- それとも actual external contract のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...198` で current package close。
