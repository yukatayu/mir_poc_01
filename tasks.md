...
scratch execution and manifest show that theory/04's displayed event-only
closure makes sent-event membership follow from `send -> receive`; the printed
predicate itself has no channel-state representation relation. The scratch
source stays outside the repository. This cannot define a checkpoint,
