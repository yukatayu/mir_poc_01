...
| 52 | WRK-0019 P-COMP-03 bounds direct-carrier registration | Registered only: one existing-lane non-production arrays-bounds `world` sidecar is pinned for a future Product Alpha package-path observation. No sidecar, candidate command, outcome, phase assertion, public error contract, helper/schema/validator/runtime/CLI change, or workflow claim exists yet. | Macro 2 parser-free substrate reserve; execute exactly the registered command only after this registration is committed and pushed. Freeze on any mismatch or reserved-surface pressure. |
