...
- **Phase 5** は `specs/examples/126...252` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から theorem-side retained bridge を `retained_payload_body_materialization_theorem_export_handoff_transport_receipt_row` まで伸ばし、**next later reopen** を `minimal-handoff-transport-receipt-ready handoff-transport-channel-body comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | minimal-handoff-transport-receipt-ready handoff-transport-channel-body comparison | symbolic handoff transport receipt row の次段として handoff transport channel body を theorem-line retained bridge にどこまで近づけるかを narrow に整理する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...252` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_theorem_export_handoff_transport_receipt_row` まで current first choice に置ける。

#### 次に扱うべき narrow question

- `retained_payload_body_materialization_theorem_export_handoff_transport_receipt_row` の次段として、handoff transport channel body をどこまで narrow に retained bridge へ寄せるか
...
- Phase 5 theorem-line package は `specs/examples/126...252` で current package close。
