...
- **Phase 5** は `specs/examples/126...191` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から symbolic retained bridge を `retained_payload_body_materialization_bless_update_dual_ref_bundle_ref` まで伸ばし、**next later reopen** を `actual bless-side row ref / update-side row strict dual-field comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | actual bless-side row ref / update-side row strict dual-field comparison | theorem-line retained bridge に bless/update strict dual-field shape をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...191` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_bless_update_dual_ref_bundle_ref` までを current first choice に置ける。

#### 次に扱うべき narrow question

- bless-side row ref / update-side row ref の strict dual-field shape を theorem-side retained bridge に寄せるか
- それとも dual-ref-bundle ref のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...191` で current package close。
