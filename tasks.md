...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | actual archive bundle member family comparison | `proof_notebook` first bridge に `archive_bundle_manifest_ref` までを足したうえで、actual archive bundle member family をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `176...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
...
- actual archive bundle member family をどこまで足すか
