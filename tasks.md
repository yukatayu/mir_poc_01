...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | transport / failure threshold comparison | `proof_notebook` first bridge に `runtime_coupling_ref` までを足したうえで、concrete transport protocol / failure body をどこまで theorem-side bridge に近づけるかを比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `157...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
