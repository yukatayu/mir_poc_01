...
| `E-ROW non-visibility singleton coverage check` | `G1` reserve | decide whether `MissingWitness` is enough as the singleton representative or whether `MissingCapability` / `RouteUnavailable` / `StaleMembership` need separate no-repair fixture rows before widening | either additional singleton fixtures pass helper checks, or `plan/89` records why one representative is enough; no repair widening or OBL-025 proof |
| `E-ROW non-visibility singleton repair prototype` | `G1` reserve | widen `suggested_repair[]` for the singleton no-repair boundary only after no-placeholder tests and the single-edit assumption are explicit | `ELAB-04/07` no-repair fences preserved; no ranking, multi-edit, whole-program success, or OBL-025 proof claim |
