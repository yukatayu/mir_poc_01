...
| 2 | Phase 5 checkpoint 後半 / later reopen 候補 | concrete notebook workflow pressure comparison | `proof_notebook` first bridge を docs-only derived view に留めたまま、stable notebook bridge sketch が必要になる workflow pressure の最小条件を比べる | 中〜重 | 0〜2 task | 一部自走可能 | `specs/examples/126...` から `137...` までで current theorem-line package は close。`proof_assistant_adapter` pressure は second practical candidate に残す |
