...
- **Phase 5** は `specs/examples/126...192` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から symbolic retained bridge を `retained_payload_body_materialization_bless_side_row_ref` / `retained_payload_body_materialization_update_side_row_ref` まで伸ばし、**next later reopen** を `strict dual-field ready consumer-visible pair comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | strict dual-field ready consumer-visible pair comparison | theorem-line retained bridge に consumer-visible pair surface をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...192` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_bless_side_row_ref` / `retained_payload_body_materialization_update_side_row_ref` までを current first choice に置ける。

#### 次に扱うべき narrow question

- strict dual field の次段として consumer-visible pair surface を theorem-side retained bridge に寄せるか
- それとも internal-only split のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...192` で current package close。
