...
- **Phase 5** は `specs/examples/126...197` までで theorem-line later package close である。small decidable core / proof / async-control boundary inventory から theorem-side retained bridge を `retained_payload_body_materialization_external_handoff_row` まで伸ばし、**next later reopen** を `external-contract-facing-handoff-row-ready actual external contract comparison` に置いている。
...
|---|---|---|---|---|---|---|---|
| 1 | Phase 5 後半 | external-contract-facing-handoff-row-ready actual external contract comparison | theorem-line retained bridge に actual external contract をどこまで近づけるかを narrow に比較する | 中〜重 | 1〜3 task / 2〜6日 | 自走可能 | **current promoted line** |
...
- `specs/examples/135...197` で theorem-line retained bridge を段階的に伸ばし、現時点では `retained_payload_body_materialization_external_handoff_row` までを current first choice に置ける。

#### 次に扱うべき narrow question

- `retained_payload_body_materialization_external_handoff_row` の次段として actual external contract を theorem-side retained bridge にどこまで近づけるか
- それとも external-contract-facing handoff row のままで止めるか
...
- Phase 5 theorem-line package は `specs/examples/126...197` で current package close。
